...
- **#2** continues as **many feature branches** (request-lifetime MM retrofitted for script **scope** lifetimes). Another discussed, not coded: object property names as `const afw_value_t *`. Script + JSON string-only. Pad: [`designs/issue-2-property-name-values.md`](designs/issue-2-property-name-values.md). Leftover object-key `as_utf8` cleanup rides that branch; `eq` docs-vs-C is noted there as **not** that branch.
