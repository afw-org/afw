...
| [#33](https://github.com/afw-org/afw/issues/33) | Mike, Jeremy | Review/Change Error Codes | Now that we can try/catch in Adaptive Script, we need to review current error codes and change any, or create new one… | On `issue-#33-error-codes`: map reordered; HTTP retargets; `throw` … `id` / `data`. Old `throw "…" { … }` deprecated → #172. `errorsThrown` review still open. | 2026-08-13 |
