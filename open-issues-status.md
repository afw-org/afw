...
| [#39](https://github.com/afw-org/afw/issues/39) | Mike | Support holes in list literals | Support holes in list literals and deal with all of the consequences of entries in lists being able to be undefined. There are a lot of functions that can traverse lists that need to deal with this properly like the… | **In progress on `issue-39-list-literal-holes`:** elision → dense undefined (script/relaxed); assign-append at length; `create_array(n)`; tests + `designs/array-semantics.md`. Strict JSON still no holes. | 2026-08-06 |
