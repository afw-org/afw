...
| [#33](https://github.com/afw-org/afw/issues/33) | Mike, Jeremy | Review/Change Error Codes | Now that we can try/catch in Adaptive Script, we need to review current error codes and change any, or create new one… | Best-effort on `issue-#33-error-codes`: map reordered (`none`/`general`/`throw` first); HTTP retargets; pad `designs/issue-33-error-codes.md`. Throw-with-id still open. | 2026-08-12 |
