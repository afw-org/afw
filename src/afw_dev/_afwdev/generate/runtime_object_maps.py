...
def write_c_map(fd, prefix, obj, options, onGetValueCFunctionNames):
    ...
    if runtime is not None:
        ...
        for name in sorted(propertiesNames):
            ...
            if propertyType.get('dataType') is None:
                ...
                #FIXME if options['core']:
                fd.write('        { &afw_data_type_' + propertyType.get('dataType') + '_direct },\n')
                #FIXME else:
                #FIXME     fd.write('        .unresolved_data_type_id = ' + s_ +  propertyType.get('dataType') + ',\n')
            ...
            if propertyType.get('dataTypeParameter') is not None and propertyType.get('dataType','') == 'array':
                ...
                #FIXME else:
                #FIXME     fd.write('        .unresolved_data_type_parameter_data_type_id = ' + s_ +  dataTypeParameter + ',\n')
            else:
                fd.write('        { NULL },\n')
    ...
    #FIXME     fd.write('    impl_runtime_rti_' + id + ',\n')
    #FIXME     fd.write('    impl_runtime_meta_' + id + ');\n')
