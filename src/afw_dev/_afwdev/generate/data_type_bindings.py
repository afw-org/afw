...
def write_h_section(fd, prefix, obj):
    ...
    if not special:
        ...
        fd.write(' * @param xctx of caller.\n')
        fd.write(' * @return (const afw_value_' + id + '_t *) or NULL if missing.\n')
        fd.write(' *\n')
        ...
        fd.write(' * @param xctx of caller.\n')
        fd.write(' * @return (const afw_value_' + id + '_t *) or NULL.\n')
        fd.write(' *\n')
        ...
        fd.write(' */\n')
        fd.write(declare + '(' + return_type + ')\n')
        fd.write(prefix + 'array_of_' + id + '_get_next_internal_source(\n')
        fd.write('    const afw_array_t *instance,\n')
        fd.write('    const afw_iterator_old_t * *iterator,\n')
        if needs_found:
            fd.write('    afw_boolean_t *found,\n')
        ...
        fd.write('\n/**\n')
        fd.write(' * @brief Add a ' + id + ' value to array of ' + id + '.\n')
        fd.write(' * @param instance of array.\n')
        ...
        fd.write('\n/**\n')
        fd.write(' * @brief Add a ' + id + ' internal to array of ' + id + '.\n')
        fd.write(' * @param instance of array.\n')
        fd.write(' * @param value to add.\n')
        fd.write(' * @param xctx of caller.\n')
        fd.write(' */\n')
        fd.write(declare + '(void)\n')
        fd.write(prefix + 'array_of_' + id + '_add_internal(\n')
        fd.write('    const afw_array_t *instance,\n')
...
def write_c_section(fd, prefix, obj):
    ...
    if not special:
        ...
        fd.write('            "Typesafe error: expecting \'' + id + '\' but "\n')
        fd.write('            "encountered \'%ku\'",\n')
        fd.write('            data_type_id);\n')
        fd.write('    }\n')
        ...
        fd.write('        return afw_value_get_reference(value, xctx);\n')
        fd.write('    }\n')
        if id == 'object':
            fd.write('    {\n')
            fd.write('        const afw_object_t *from;\n')
            ...
            fd.write('    {\n')
            fd.write('        const afw_array_t *from;\n')
        ...
        fd.write('            "encountered \'%ku\'",\n')
        fd.write('            data_type_id);\n')
        fd.write('        afw_error_processing_throw((xctx), afw_error_code_general);\n')
        fd.write('    }\n')
        fd.write('    return (const afw_value_' + id + '_t *)value;\n')
        fd.write('}\n')
        ...
        if needs_found:
            ...
            fd.write('    const afw_value_' + id + '_t *value;\n')
            fd.write('\n')
            fd.write('    value = afw_object_get_property_as_' + id +
                     '_source(object, property_name, source_z, xctx);\n')
            fd.write('    if (!value) {\n')
            ...
            fd.write('        return NULL;\n')
            fd.write('    }\n')
            fd.write('    return ' + amp_if_needed + 'value->internal;\n')
            fd.write('}\n')

        fd.write('\n/* Get next property as ' + id + ' value. */\n')
        fd.write(define + '(const afw_value_' + id + '_t *)\n')
        fd.write('afw_object_get_next_property_as_' + id + '_source(\n')
        fd.write('    const afw_object_t *object,\n')
        fd.write('    const afw_iterator_old_t * *iterator,\n')
        ...
        if needs_found:
            ...
            fd.write('    const afw_value_' + id + '_t *value;\n')
            fd.write('\n')
            fd.write('    value = afw_object_get_next_property_as_' + id +
                     '_source(object, iterator, property_name, source_z, xctx);\n')
            fd.write('    if (!value) {\n')
            fd.write('        *found = false;\n')
            fd.write('        return 0;\n')
            fd.write('    }\n')
    ...
    # array
    if not special:
        fd.write('\n/* Get next ' + id + ' value from array of ' + id + '. */\n')
        fd.write(define + '(const afw_value_' + id + '_t *)\n')
        fd.write(prefix + 'array_of_' + id + '_get_next_source(\n')
        fd.write('    const afw_array_t *instance,\n')
        fd.write('    const afw_iterator_old_t * *iterator,\n')
        ...
        fd.write('\n/* Get next ' + id + ' internal from array of ' + id + '. */\n')
        fd.write(define + '(' + return_type + ')\n')
        fd.write(prefix + 'array_of_' + id + '_get_next_internal_source(\n')
        fd.write('    const afw_array_t *instance,\n')
        fd.write('    const afw_iterator_old_t * *iterator,\n')
        ...
        fd.write('    const afw_value_' + id + '_t *value;\n')
        fd.write('\n')
        fd.write('    value = ' + prefix + 'array_of_' + id +
                 '_get_next_source(instance, iterator, source_z, xctx);\n')
        if needs_found:
            fd.write('    if (!value) {\n')
            fd.write('        *found = false;\n')
            fd.write('        return 0;\n')
        ...
        fd.write('    }\n')
        fd.write('    afw_array_setter_push_value(setter, &value->pub, xctx);\n')
        fd.write('}\n')

        fd.write('\n/* Add a ' + id + ' internal to array of ' + id + '. */\n')
        fd.write(define + '(void)\n')
        fd.write(prefix + 'array_of_' + id + '_add_internal(\n')
        fd.write('    const afw_array_t *instance,\n')
        fd.write('    ' + parameter_ctype + 'value,\n')
