...
/*
 * Adaptive Framework LMDB Adapter Metadata
 *
    ...
    afw_xctx_t *xctx)
{
    ...
    /* Create a subpool for metadata in adapter's pool. */
    p = afw_pool_create(adapter->pub.p, xctx);
...
/* Used by afw_lmdb_adapter_session() to retrieve metadata objects. */
    ...
    afw_xctx_t *xctx)
{
    ...
    return afw_array_create_unmanaged(xctx->p, xctx);
}
