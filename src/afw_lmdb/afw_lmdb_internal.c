    ...
    afw_xctx_t *xctx)
{
    ...
    afw_object_set_property(config, &afw_lmdb_self_s_updateTimeStamp, now, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    object_type_id = &afw_lmdb_self_s_internalConfig;

    dbi = afw_lmdb_internal_open_database(self, txn, 
        &afw_lmdb_self_s_Primary, MDB_CREATE, p, xctx);
    ...
    if (rc == MDB_NOTFOUND) {
        ...
        afw_object_set_property(config, &afw_lmdb_self_s_createTimeStamp, now, xctx);
        afw_object_set_property(config, &afw_lmdb_self_s_updateTimeStamp, now, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LMDB_BEGIN_TRANSACTION(session->adaptor, session, 0, false, xctx) {
        ...
        database = afw_utf8_concat(xctx->p, xctx, &afw_lmdb_self_s_Extern, 
