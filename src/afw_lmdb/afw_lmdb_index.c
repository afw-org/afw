    ...
    afw_xctx_t *xctx)
{
    ...
    if (object_type_id) {
        database = afw_utf8_concat(p, xctx, &afw_lmdb_self_s_Index, 
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_ADAPTOR_IMPL_LOCK_READ_BEGIN(adaptor) {
        indexes = afw_object_old_get_property_as_object(
            self->adaptor->internalConfig, &afw_lmdb_self_s_indexDefinitions, xctx);
        if (indexes) {
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_ADAPTOR_IMPL_LOCK_WRITE_BEGIN(adaptor) {
        afw_object_set_property_as_object(
            session->adaptor->internalConfig, &afw_lmdb_self_s_indexDefinitions, 
