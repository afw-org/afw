/* Interface afw_adaptor Implementation for vfs */
...
/**
 * @file afw_vfs_adaptor.c
 * @brief Implementation of afw_adaptor interface for afw_vfs
 */
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    adaptor = afw_adaptor_impl_create_cede_p(
            ...
            sizeof(afw_vfs_adaptor_internal_t),
            properties, p, xctx);
    p = adaptor->p;
...
/*
 * Implementation of method create_adaptor_session for interface
 * afw_adaptor.
 */
...
{
    afw_vfs_adaptor_internal_t *self =
        (afw_vfs_adaptor_internal_t *)instance;
    const afw_adaptor_session_t *session;

    /* Create session and return. */
    session = afw_vfs_adaptor_internal_session_create(
        self, xctx);
    return session;
}
