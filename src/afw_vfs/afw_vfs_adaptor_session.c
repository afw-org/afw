    ...
    afw_xctx_t *xctx)
{
    ...
    if (finfo.filetype == APR_DIR) {
        ...
        afw_object_set_property(object,
            &afw_vfs_self_s_isDirectory, afw_value_true, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    ctx.suffix = &afw_self_s_a_empty_string;
    ...
    if (!afw_utf8_equal(object_type_id, &afw_vfs_self_s__AdaptiveFile_vfs)) {
        callback(NULL, context, xctx);
        return;
    }
    ...
    if (adaptor_type_specific) {
        ...
        ctx.includeHidden = afw_object_old_get_property_as_boolean(
            adaptor_type_specific, &afw_vfs_self_s_includeHidden, &found, xctx);

        /* subdirectory */
        subdirectory = afw_object_old_get_property_as_string(
            adaptor_type_specific, &afw_vfs_self_s_subdirectory, xctx);
        if (subdirectory &&
            (
                afw_utf8_starts_with(subdirectory, &afw_self_s_a_slash) ||
                !afw_utf8_ends_with(subdirectory, &afw_self_s_a_slash) ||
                afw_utf8_contains(subdirectory, &afw_self_s_a_backslash) ||
                afw_utf8_contains(subdirectory, &afw_self_s_a_dot_slash)
            ))
        {
        ...
        ctx.suffix = afw_object_old_get_property_as_string(
            adaptor_type_specific, &afw_vfs_self_s_suffix, xctx);

        /* recursive */
        ctx.recursive = afw_object_old_get_property_as_boolean(
            adaptor_type_specific, &afw_vfs_self_s_recursive, &found, xctx);
    }
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_utf8_contains(object_id, &afw_self_s_a_dot_slash) ||
        afw_utf8_contains(object_id, &afw_self_s_a_backslash))
    {
        callback(NULL, context, xctx);
        return;
    }
    ...
    afw_xctx_t *xctx)
{
    ...
    *is_directory = false;
    ...
    if (afw_utf8_ends_with(object_id, &afw_self_s_a_slash)) {
        *is_directory = true;
    }
    ...
    afw_xctx_t *xctx)
{
    ...
    /* If regular file, get data property and write to file. */
        ...
        data = afw_object_get_property(object, &afw_vfs_self_s_data, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    data = afw_object_get_property(replacement_object, &afw_vfs_self_s_data, xctx);
    impl_write_data_to_file(self,
