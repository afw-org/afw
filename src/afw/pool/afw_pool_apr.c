...
#define AFW_IMPLEMENTATION_ID "apr"
#define AFW_IMPLEMENTATION_INF_LABEL impl_afw_pool_apr_inf
...
#define AFW_IMPLEMENTATION_ID "apr_multithreaded"
#define AFW_IMPLEMENTATION_INF_LABEL impl_afw_pool_apr_multithreaded_inf
    ...
    afw_xctx_t *xctx)
{
    ...
    inf = ((AFW_POOL_SELF_T *)parent)->thread
        ? &impl_afw_pool_apr_inf
        : &impl_afw_pool_apr_multithreaded_inf;

    if (((AFW_POOL_SELF_T *)parent)->thread) {
        self = impl_create((afw_pool_apr_self_t *)parent, inf, xctx);
    }
    else {
        IMPL_MULTITHREADED_LOCK_BEGIN(xctx) {
            self = impl_create((afw_pool_apr_self_t *)parent, inf, xctx);
    ...
    IMPL_PRINT_DEBUG_INFO_FZ(minimal,
        "afw_pool_apr_create " AFW_INTEGER_FMT,
    ...
    afw_xctx_t *xctx)
{
    ...
    self = impl_create(NULL, &impl_afw_pool_apr_inf, xctx);
    thread = apr_pcalloc(self->apr_p, size);
    self->thread = thread;
    thread->p = (const afw_pool_t *)self;

    IMPL_PRINT_DEBUG_INFO_FZ(minimal,
        "afw_pool_apr_create_thread " AFW_SIZE_T_FMT,
        size);
...
afw_pool_apr_internal_create_base_pool()
{
    ...
    self->pub.inf = &impl_afw_pool_apr_multithreaded_inf;
    self->apr_p = apr_p;
    ...
    afw_xctx_t *xctx)
{
    ...
    for (child = self->first_child; child; child = child->next_sibling) {
        afw_pool_apr_print_debug_info(indent + 2, &child->pub, xctx);
    }
}
