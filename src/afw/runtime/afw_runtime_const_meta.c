    ...
    afw_xctx_t *xctx)
{
    ...
    /* Parent paths. */
    if (afw_utf8_equal(property_name, &afw_self_s_parentPaths)) {
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!*iterator)
    {
        *iterator = (afw_iterator_t *)&afw_self_s_null;
        if (self->parent_paths) {
            if (property_name) {
...
impl_afw_object_has_property (
    ...
    afw_xctx_t *xctx)
{
    return (afw_utf8_equal(property_name, &afw_self_s_parentPaths)) ? true : false;
}
