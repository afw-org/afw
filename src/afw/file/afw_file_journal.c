    ...
    if (use_consumer) {
        ...
        if (advance_consumer_cursor) {
            afw_object_set_property_as_string(peer,
                &afw_self_s_advanceCursor, entry_object_id, xctx);
        }
        ...
        /* If use_consumer_cursors and not reissue, update peer cursors. */
            ...
            if (applicable) {
                afw_object_set_property_as_string(peer,
                    &afw_self_s_currentCursor, entry_object_id, xctx);
                afw_object_set_property_as_string(peer,
                    &afw_self_s_consumeCursor, entry_object_id, xctx);
                afw_object_set_property_as_dateTime(peer,
                    &afw_self_s_consumeStartTime, now, xctx);
                afw_object_remove_property(peer, &afw_self_s_advanceCursor,
                    xctx);
            }
