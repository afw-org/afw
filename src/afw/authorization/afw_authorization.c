    ...
    afw_xctx_t *xctx)
{
    ...
    if (!ctl)
    {
        afw_trace_fz(1, xctx->env->flag_index_trace_authorization_check_bypass,
            NULL, xctx,
            "[Authorization check requestId " AFW_UTF8_K_FMT_Q
            " mode " AFW_UTF8_K_FMT_Q
            " actionId " AFW_UTF8_K_FMT_Q
            " resourceId " AFW_UTF8_K_FMT_Q
                ...
                if (ah) {
                    ...
                    if (afw_utf8_equal(decision_id, afw_s_permit)) {
                        if (!ah->allow_permit_override) {
                            afw_trace_fz(1, ah->trace_flag_index,
                                ...
                                " actionId " AFW_UTF8_K_FMT_Q
                                " resourceId " AFW_UTF8_K_FMT_Q
                                " authorizationHandler " AFW_UTF8_K_FMT_Q
                    ...
                    if (result2) {
                        ...
                        afw_trace_fz(1, ah->trace_flag_index,
                            ...
                            " resourceId " AFW_UTF8_K_FMT_Q
                            " authorizationHandler " AFW_UTF8_K_FMT_Q
                            "] handler decision is " AFW_UTF8_K_FMT_Q,
                            (request_id),
                            (&((const afw_value_string_t *)xctx->mode)->internal),
                            (action_id),
                            (resource_id),
    ...
    AFW_CATCH_UNHANDLED{
            ...
            xctx->env->flag_index_trace_authorization_decision, xctx))
        {
            ...
            afw_trace_fz(1, xctx->env->flag_index_trace_authorization_decision_detail,
                ...
                " actionId " AFW_UTF8_K_FMT_Q
                " resourceId " AFW_UTF8_K_FMT_Q
                "] detail for indeterminate produced by " AFW_UTF8_K_FMT_Q
                ":\n\n" AFW_UTF8_K_FMT,
                (request_id),
                (&((const afw_value_string_t *)xctx->mode)->internal),
                (action_id),
    ...
    afw_trace_fz(1, xctx->env->flag_index_trace_authorization_decision,
        ...
        " actionId " AFW_UTF8_K_FMT_Q
        " resourceId " AFW_UTF8_K_FMT_Q
        "] final decision is " AFW_UTF8_K_FMT_Q
        " made by " AFW_UTF8_K_FMT_Q,
        (request_id),
        (&((const afw_value_string_t *)xctx->mode)->internal),
        (action_id),
        (resource_id),
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!instance) {
        ...
        if (!instance) {
            AFW_THROW_ERROR_FZ(general, xctx,
                "Authorization Handler " AFW_UTF8_K_FMT_Q
                " is not available",
                (authorization_handler_id));
        }
    }
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    if (anchor) {
        AFW_THROW_ERROR_FZ(general, xctx,
            AFW_UTF8_K_FMT " authorizationHandlerId " AFW_UTF8_K_FMT_Q
            " is already running",
            (source_location),
            (authorization_id));
    }
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!factory) {
        AFW_THROW_ERROR_FZ(general, xctx,
            "authorizationHandlerType " AFW_UTF8_K_FMT_Q
