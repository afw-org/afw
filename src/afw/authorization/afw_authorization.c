    ...
    afw_xctx_t *xctx)
{
    ...
    if (object) {
        self->core_authorization_check =
            afw_object_old_get_property_as_compiled_script(object,
                &afw_self_s_coreAuthorizationCheck,
                &afw_self_s_internal, NULL, p, xctx);

        self->initial_authorization_check =
            afw_object_old_get_property_as_compiled_script(object,
                &afw_self_s_initialAuthorizationCheck,
                &afw_self_s_internal, NULL, p, xctx);

        self->check_intermediate_mode = afw_object_old_get_property_as_boolean(
            object, &afw_self_s_checkIntermediateMode, &found, xctx);

        deny_if_not_applicable = afw_object_old_get_property_as_boolean(
            object, &afw_self_s_denyIfNotApplicable, &found, xctx);
    }
    ...
    afw_object_meta_set_object_type_id(not_applicable_object,
        &afw_self_s__AdaptiveAuthorizationResult_, xctx);
    afw_object_set_property(not_applicable_object, &afw_self_s_decisionId,
    ...
    afw_object_set_property_as_array(not_applicable_object,
        &afw_self_s_applicablePolicies, list, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    current_decider = &afw_self_s_none;
    final_decider = &afw_self_s_none;
    ...
    ah = NULL;
    AFW_TRY{
        afw_context_push_cb_variables(&afw_self_s_current,
        ...
        if (xctx->mode == afw_authorization_mode_id_core_value) {
            ...
            current_decider = &afw_self_s_coreAuthorizationCheck;
            (void)current_decider; /* In catch. Avoid "not used" error. */
            final_decider = &afw_self_s_coreAuthorizationCheck;
        }
        else if (ctl->initial_authorization_check) {
            check = ctl->initial_authorization_check;
            property_name = "initialAuthorizationCheck";
            current_decider = &afw_self_s_initialAuthorizationCheck;
            (void)current_decider; /* In catch. Avoid "not used" error. */
            final_decider = &afw_self_s_initialAuthorizationCheck;
        }
        ...
        if (check) {
            ...
            if (!result || afw_value_is_null(result)) {
                decision_id = &afw_self_s_notApplicable;
            }
            else {
            ...
            if (afw_utf8_equal(decision_id, &afw_self_s_permit)) {
                final_result = true;
            }
            else if (afw_utf8_equal(decision_id, &afw_self_s_deny)) {
                final_result = true;
            }
            else if (afw_utf8_equal(decision_id, &afw_self_s_notApplicable)) {
                final_result = false;
            }
            else {
                AFW_THROW_ERROR_FZ(general, xctx,
        ...
        if (!final_result && xctx->mode != afw_authorization_mode_id_core_value) {
            for (decision_id = &afw_self_s_notApplicable,
                result = NULL,
                ...
                if (ah) {
                    ...
                    if (result2) {
                        decision_id2 = afw_object_old_get_property_as_string(
                            ...
                            &afw_self_s_decisionId, xctx);
                        if (afw_utf8_equal(decision_id2, &afw_self_s_permit) ||
                            afw_utf8_equal(decision_id2, &afw_self_s_deny)) {
                            result = result2;
        ...
        if (!final_result) {
            current_decider = &afw_self_s_denyIfNotApplicable;
            (void)current_decider; /* In catch. Avoid "not used" error. */
            final_decider = &afw_self_s_denyIfNotApplicable;
    ...
    AFW_CATCH_UNHANDLED{
        ...
        afw_object_meta_set_object_type_id(obj,
            &afw_self_s__AdaptiveAuthorizationResult_, xctx);
        afw_object_set_property(obj, &afw_self_s_decisionId,
            afw_authorization_decision_id_indeterminate_value,
            xctx);
        result = afw_value_create_object(obj, p, xctx);
        decision_id = &afw_self_s_indeterminate;
    ...
    if (enforce) {
        if (!afw_utf8_equal(decision_id, &afw_self_s_permit)) {
            obj = afw_object_create(p, xctx);
            afw_object_set_property(obj, &afw_self_s_actionId,
                action_id_value, xctx);
            afw_object_set_property(obj, &afw_self_s_resourceId,
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_flag_environment_register_flag(
        &afw_self_s_a_flag_trace_authorization_detail,
        &afw_self_s_a_flag_trace_authorization_detail_brief,
        &afw_self_s_a_flag_trace_authorization_detail_description,
        &afw_self_s_a_flag_trace_detail,
        xctx);

    /* Register flag trace:authorization */
    afw_flag_environment_register_flag(
        &afw_self_s_a_flag_trace_authorization,
        &afw_self_s_a_flag_trace_authorization_brief,
        &afw_self_s_a_flag_trace_authorization_description,
    ...
    afw_flag_environment_register_flag(
        &afw_self_s_a_flag_trace_authorization_check_bypass,
        &afw_self_s_a_flag_trace_authorization_check_bypass_brief,
        &afw_self_s_a_flag_trace_authorization_check_bypass_description,
        &afw_self_s_a_flag_trace_authorization,
        xctx);
    ...
    afw_flag_environment_register_flag(
        &afw_self_s_a_flag_trace_authorization_check_detail,
        &afw_self_s_a_flag_trace_authorization_check_detail_brief,
        &afw_self_s_a_flag_trace_authorization_check_detail_description,
        &afw_self_s_a_flag_trace_authorization_detail,
        xctx);
    ...
    afw_flag_environment_register_flag(
        &afw_self_s_a_flag_trace_authorization_check,
        &afw_self_s_a_flag_trace_authorization_check_brief,
        &afw_self_s_a_flag_trace_authorization_check_description,
        &afw_self_s_a_flag_trace_authorization,
        xctx);
    ...
    afw_flag_environment_register_flag(
        &afw_self_s_a_flag_trace_authorization_decision_detail,
        &afw_self_s_a_flag_trace_authorization_decision_detail_brief,
        &afw_self_s_a_flag_trace_authorization_decision_detail_description,
        &afw_self_s_a_flag_trace_authorization_detail,
        xctx);
    ...
    afw_flag_environment_register_flag(
        &afw_self_s_a_flag_trace_authorization_decision,
        &afw_self_s_a_flag_trace_authorization_decision_brief,
        &afw_self_s_a_flag_trace_authorization_decision_description,
        &afw_self_s_a_flag_trace_authorization,
        xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!instance) {
        service_id = afw_utf8_concat(xctx->p, xctx,
            &afw_self_s_authorizationHandler,
            &afw_self_s_a_dash, authorization_handler_id, NULL);
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    authorization_id = afw_object_old_get_property_as_string(conf,
        &afw_self_s_authorizationHandlerId, xctx);
    if (!authorization_id) {
        AFW_THROW_ERROR_FZ(general, xctx,
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_environment_create_and_register_conf_type(
        &afw_self_s_authorizationHandler,
        impl_authorization_conf_type_create_cede_p,
        &afw_self_s_a_authorizationHandler_title,
        &afw_self_s_a_authorizationHandler_description,
        &afw_self_s_authorizationHandlerId,
        &afw_self_s_authorization_handler_id,
        &afw_self_s__AdaptiveAuthorizationHandler_,
        &afw_self_s_authorizationHandlerType,
        &afw_self_s_authorization_handler_type,
    ...
    afw_memory_copy(&self->service_type_id, &afw_self_s_authorizationHandler);
    self->conf_type = afw_environment_get_conf_type(
        &afw_self_s_authorizationHandler, xctx);
    if (!self->conf_type) {
        AFW_THROW_ERROR_Z(general, "conf_type must already be registered",
            xctx);
    }
    self->title = &afw_self_s_a_service_type_authorizationHandler_title;
    self->conf_type_object = afw_runtime_get_object(&afw_self_s__AdaptiveConfType_,
        &afw_self_s_authorizationHandler, xctx);
    afw_environment_register_service_type(&afw_self_s_authorizationHandler, self, xctx);
}
    ...
    afw_xctx_t *xctx)
{
    ...
    authorization_handler_type = afw_object_old_get_property_as_utf8(
        properties, &afw_self_s_authorizationHandlerType, p, xctx);
