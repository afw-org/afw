...
afw_adaptor_impl_push_qualifiers(
    ...
    afw_xctx_t *xctx)
{
    afw_xctx_qualifier_stack_qualifier_object_push(&afw_self_s_adaptor,
        adaptor->properties, true, xctx->p, xctx);
    afw_xctx_qualifier_stack_qualifier_push(&afw_self_s_adaptor, NULL, true,
    ...
    afw_xctx_t *xctx)
{
    ...
    adaptor->adaptor_type_id = afw_object_old_get_property_as_string(
        properties, &afw_self_s_adaptorType, xctx);
    ...
    afw_adaptor_impl_set_supported_core_object_type(adaptor,
        &afw_self_s__AdaptiveObjectType_, true, false, xctx);
    ...
    impl->journal_adaptor_id = afw_object_old_get_property_as_string(
        adaptor->properties, &afw_self_s_journalAdaptorId, xctx);
    ...
    impl->authorization = afw_object_get_property(
        properties, &afw_self_s_authorization, xctx);
    ...
    b = afw_object_old_get_property_as_boolean_deprecated(properties,
        &afw_self_s_isModelLocation, xctx);
    if (b) {
        impl->model_location = afw_model_location_create(adaptor, p, xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveTemplateProperties_, false, true, xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveModel_, true, true, xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveModelObjectTypes_, false, true, xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveModelObjectType_, false, true, xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveModelPropertyTypes_, false, true, xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveModelPropertyType_, false, true, xctx);
    }

    /** @fixme have way for extension to add these.
    If isPolicyLocation is true, provide appropriate object types.
    b = afw_object_old_get_property_as_boolean_deprecated(properties,
        &afw_self_s_isPolicyLocation, xctx);
    if (b) {
        impl->policy_location = afw_authorization_policy_internal_location_create(
            adaptor, p, xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveAuthorizationAdviceExpression_, false, true,
            xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveAuthorizationCombinerParameter_, false, true,
            xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveAuthorizationObligationExpression_, false, true,
            xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveAuthorizationPolicy_, true, true,
            xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveAuthorizationPolicyIssuer_, false, true,
            xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveAuthorizationPolicySet_, true, true,
            xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveAuthorizationRule_, false, true,
            xctx);
        afw_adaptor_impl_set_supported_core_object_type(adaptor,
            &afw_self_s__AdaptiveTemplateProperties_, false, true, xctx);
    }
...
impl_update_allow(
    ...
    afw_xctx_t *xctx)
{
    afw_object_set_property(object, &afw_self_s_allowEntity,
        (allow_entity) ? afw_value_true : afw_value_false, xctx);
    afw_object_set_property(object, &afw_self_s_allowAdd,
        (allow_write) ? afw_value_true : afw_value_false, xctx);
    afw_object_set_property(object, &afw_self_s_allowChange,
        (allow_write) ? afw_value_true : afw_value_false, xctx);
    afw_object_set_property(object, &afw_self_s_allowDelete,
        (allow_write) ? afw_value_true : afw_value_false, xctx);
}
    ...
    afw_xctx_t *xctx)
{
    ...
    if (e) {
        ...
        e->object = afw_runtime_get_object(&afw_self_s__AdaptiveObjectType_,
            object_type_id, xctx);
        if (!e->object) {
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_object_meta_set_ids(result, &adaptor->adaptor_id,
        &afw_self_s__AdaptiveObjectType_, objectType, xctx);
    afw_object_set_property_as_string(result, &afw_self_s_objectType,
        objectType, xctx);
    afw_object_set_property_as_boolean(result,
        &afw_self_s_allowAdd, afw_value_true, xctx);
    afw_object_set_property_as_boolean(result,
        &afw_self_s_allowChange, afw_value_true, xctx);
    afw_object_set_property_as_boolean(result,
        &afw_self_s_allowDelete, afw_value_true, xctx);
    ...
    afw_object_set_property_as_boolean(other_properties,
        &afw_self_s_allowWrite, afw_value_true, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    else if (object_type_id && afw_utf8_starts_with(object_type_id, &afw_self_s__Adaptive)) {
        ctx.impl_callback_context = &ctx;
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_utf8_starts_with(object_type_id, &afw_self_s__Adaptive)) {
        ctx.impl_callback_context = &ctx;
        ctx.impl_callback = impl_special_object_handling_cb;
        afw_adaptor_session_get_object(
            self->wrapped_session, impl_request,
