 ...
 *
 */
afw_boolean_t afw_adaptor_impl_index_object_type_applicable(
    ...
    afw_xctx_t        * xctx)
{
    ...
    objectTypes = afw_object_old_get_property_as_array(
        indexDefinition, &afw_self_s_objectType, xctx);
 ...
 */
afw_boolean_t afw_adaptor_impl_index_filter_applicable(
    ...
    afw_xctx_t         * xctx)
{
    ...
    filter = afw_object_old_get_property_as_string(
        indexDefinition, &afw_self_s_filter, xctx);
    if (filter) {
 ...
 *
 */
afw_boolean_t afw_adaptor_impl_index_option_case_insensitive(
    const afw_object_t * indexDefinition,
    afw_xctx_t        * xctx)
{
    ...
    options = afw_object_old_get_property_as_array(
        indexDefinition, &afw_self_s_options, xctx);
    ...
    if (options) { 
        ...
        while (option) {
            if (afw_value_is_string(option)) {
                ...
                if (afw_utf8_equal(option_str, &afw_self_s_case_insensitive_string))
                    return true;
            }
 ...
 *
 */
afw_boolean_t afw_adaptor_impl_index_option_unique(
    const afw_object_t * indexDefinition,
    afw_xctx_t        * xctx)
{
    const afw_array_t  * options;
    ...
    if (options) { 
        ...
        while (option) {
            if (afw_value_is_string(option)) {
                ...
                if (afw_utf8_equal(option_str, &afw_self_s_unique))
                    return true;
            }
 ...
 */
void afw_adaptor_impl_index_apply(
    ...
    afw_xctx_t        * xctx)
{
    ...
    if (options) {
        ...
        while (option) {
            if (afw_value_is_string(option)) {
                ...
                else if (afw_utf8_equal(option_str, &afw_self_s_unique))
                    unique = true;
            }
    ...
    if (operation == afw_adaptor_impl_index_mode_add) {
        afw_adaptor_impl_index_add(instance, object_type_id,
            object_id, key, value_string, unique, object->p, xctx);
    }

    else if (operation == afw_adaptor_impl_index_mode_delete) {
        afw_adaptor_impl_index_delete(instance, object_type_id,
 ...
 * adaptive function index routines, and by adaptor session
 * routines.
 */
afw_boolean_t afw_adaptor_impl_index_try(
    const afw_adaptor_impl_index_t * instance,
    const afw_utf8_t   * key,
    const afw_object_t * object,
    const afw_utf8_t   * object_type_id,
    const afw_utf8_t   * object_id,
    const afw_object_t * indexDefinition,
    afw_adaptor_impl_index_mode_t operation,
    afw_xctx_t        * xctx)
{
    ...
    if (!afw_adaptor_impl_index_filter_applicable(
    ...
    value_expression = afw_object_old_get_property_as_string(
        indexDefinition, &afw_self_s_value, xctx);
    if (value_expression) 
    {
    ...
    else if (afw_value_is_array(eval))
    {
        ...
        for (i = 0; index_values[i]; i++) {
            ...
            afw_adaptor_impl_index_apply(instance, indexDefinition,
...
void afw_adaptor_impl_index_open_definition(
    ...
    afw_xctx_t                    * xctx)
{
    ...
    if (options) {
        ...
        while (option) {
            if (afw_value_is_string(option)) {
                ...
                if (afw_utf8_equal(option_str, &afw_self_s_sort_reverse))
                    reverse = true;
                else if (afw_utf8_equal(option_str, &afw_self_s_integer))
                    integer = true;
                else if (afw_utf8_equal(option_str, &afw_self_s_unique))
                    unique = true;
            }
    ...
    if (objectType) {
        ...
        afw_adaptor_impl_index_open(indexer, NULL, key, 
            integer, unique, reverse, pool, xctx);
    }
}

AFW_DEFINE(void) afw_adaptor_impl_index_open_definitions(
    const afw_adaptor_impl_index_t * indexer,
    ...
    afw_xctx_t                    * xctx)
{
    ...
    while (indexDefinition) {
        afw_adaptor_impl_index_open_definition(indexer, key, 
 ...
 */
afw_boolean_t afw_adaptor_impl_index_cb(
    const afw_object_t * object,
    void               * context,
    afw_xctx_t        * xctx)
{
    ...
    // afw_xctx_scope_deprecated_variable_set(&afw_self_s_object, 
    ...
    if (afw_adaptor_impl_index_try(ctx->instance, key, object, 
...
/*
 * void afw_adaptor_impl_index_list()
 *
 * This routine will use adaptor interfaces to go through
 * and list indexes for a particular adaptorId.
 *
 */
AFW_DEFINE(const afw_object_t *) afw_adaptor_impl_index_list(
    ...
    afw_xctx_t      * xctx)
{
    ...
    session = afw_adaptor_session_get_cached(adaptorId, false, xctx);

    instance = afw_adaptor_session_get_index_interface(session, xctx);
    if (instance == NULL) {
        AFW_THROW_ERROR_Z(general,
            "Error: Cannot find index interface for adaptorId.", xctx);
    }
...
AFW_DEFINE(const afw_object_t *) afw_adaptor_impl_index_remove(
    const afw_utf8_t  * adaptorId,
    const afw_utf8_t  * key,
    const afw_pool_t  * pool,
    afw_xctx_t       * xctx)
{
    ...
    session = afw_adaptor_session_get_cached(adaptorId, true, xctx);

    indexer = afw_adaptor_session_get_index_interface(session, xctx);
    if (indexer == NULL) {
        AFW_THROW_ERROR_Z(general, 
    ...
    if (objectTypes) {
        ...
        do 
        {
            ...
            rc = afw_adaptor_impl_index_drop(indexer, 
            ...
            if (rc) {
                ...
                ctx.num_indexed = 0;
                ctx.num_processed = 0;
                ctx.mode = afw_adaptor_impl_index_mode_delete;

                /** @fixme xctx->p, session->p, or pool?  */
                afw_adaptor_session_retrieve_objects(session, NULL, NULL,
                    NULL, &ctx, afw_adaptor_impl_index_cb, NULL,
...
/*
 * void afw_adaptor_impl_index_create()
 *
 * This routine will create an index definition.
 *
 */
AFW_DEFINE(const afw_object_t *) afw_adaptor_impl_index_create(
    const afw_utf8_t  * adaptorId,
    const afw_utf8_t  * key,
    ...
    afw_xctx_t       * xctx)
{
    ...
    if (indexDefinition) {
        ...
        afw_object_set_property_as_string_from_utf8_z(result, &afw_self_s_message, 
    ...
    if (value)
        afw_object_set_property_as_string(indexDefinition,
            &afw_self_s_value, value, xctx);

    if (objectType)
        afw_object_set_property_as_array(indexDefinition,
            &afw_self_s_objectType, objectType, xctx);
    ...
    if (options)
        afw_object_set_property_as_array(indexDefinition,
            &afw_self_s_options, options, xctx);
    ...
    ctx.num_indexed = 0;
    ctx.num_processed = 0;
    ctx.mode = afw_adaptor_impl_index_mode_add;
    ctx.test = test;
    ...
    transaction = afw_adaptor_session_begin_transaction(session, xctx);
    ...
    if (retroactive) {
        ...
        afw_adaptor_session_retrieve_objects(session, NULL, NULL,
            NULL, &ctx, afw_adaptor_impl_index_cb, NULL, pool, xctx);
    }

    /* now, tell the adaptor to add the new indexDefinition for configuration */
    ...
    afw_adaptor_transaction_commit(transaction, xctx);
    afw_adaptor_transaction_release(transaction, xctx);

    /* return metrics */
    afw_object_set_property_as_integer(
        result, &afw_self_s_num_indexed, ctx.num_indexed, xctx);
    afw_object_set_property_as_integer(
        result, &afw_self_s_num_processed, ctx.num_processed, xctx);
...
/*
 * afw_boolean_t afw_adaptor_impl_index_is_property_indexed()
 *
...
/*
 * const afw_object_t * afw_adaptor_impl_index_get_index_definition()
 *
 ...
 *
 */
AFW_DEFINE(void) afw_adaptor_impl_index_object(
    const afw_adaptor_impl_index_t * instance,
    const afw_utf8_t               * object_type_id,
    const afw_object_t             * object,
    const afw_utf8_t               * object_id,
    afw_xctx_t                    * xctx)
{
    ...
    if (instance->indexDefinitions) {
        ...
        while (indexDefinition) {
            afw_adaptor_impl_index_try(instance, index_name, object, 
                object_type_id, object_id, indexDefinition, 
                afw_adaptor_impl_index_mode_add, xctx);
 ...
 *
 */
AFW_DEFINE(void) afw_adaptor_impl_index_unindex_object(
    const afw_adaptor_impl_index_t * instance,
    const afw_utf8_t               * object_type_id,
    const afw_object_t             * object,
    const afw_utf8_t               * object_id,
    afw_xctx_t                    * xctx)
{
    ...
    if (instance->indexDefinitions) {
        ...
        while (indexDefinition) {
            afw_adaptor_impl_index_try(instance, index_name, object, 
                object_type_id, object_id, indexDefinition,
                afw_adaptor_impl_index_mode_delete, xctx);
 ...
 *
 */
AFW_DEFINE(void) afw_adaptor_impl_index_reindex_object(
    const afw_adaptor_impl_index_t * instance,
    const afw_utf8_t               * object_type_id,
    ...
    afw_xctx_t                    * xctx)
{
    ...
    if (instance->indexDefinitions) {
        ...
        while (indexDefinition) {
            ...
            afw_adaptor_impl_index_try(instance, index_name, old_object,
                object_type_id, object_id, indexDefinition,
                afw_adaptor_impl_index_mode_delete, xctx);

            /* now add back the new ones */
            afw_adaptor_impl_index_try(instance, index_name, new_object,
                object_type_id, object_id, indexDefinition, 
                afw_adaptor_impl_index_mode_add, xctx);
...
/*
 * afw_boolean_t afw_adaptor_impl_index_sargable_entry()
 *
    ...
    afw_xctx_t                             * xctx)
{
    ...
    sargable = afw_adaptor_impl_index_is_property_indexed(instance,
    ...
    /* (entry AND on_true) OR (on_false) */
        ...
        on_true = afw_adaptor_impl_index_sargable_entry(
            instance, object_type_id, entry->on_true, xctx);

        on_false = afw_adaptor_impl_index_sargable_entry(
...
/*
 * afw_boolean_t afw_adaptor_impl_index_sargable()
 *
 ...
 *
 */
AFW_DEFINE(afw_boolean_t) afw_adaptor_impl_index_sargable(
    const afw_adaptor_impl_index_t * instance,
    const afw_utf8_t               * object_type_id,
    ...
    afw_xctx_t                    * xctx)
{
    ...
    return afw_adaptor_impl_index_sargable_entry(
        instance, object_type_id, entry, xctx);
}

/*
 * afw_adaptor_impl_index_cursor_list_cardinality()
 *
 ...
 */
afw_boolean_t afw_adaptor_impl_index_cursor_list_cardinality(
    const afw_adaptor_impl_index_t * instance,
    ...
    afw_xctx_t                    * xctx)
{
    ...
    *cardinality = 0;
    for (i = 0; i < cursor_list->nelts; i++) {
        cursor = ((const afw_adaptor_impl_index_cursor_t **)
            cursor_list->elts)[i];
...
/*
 * afw_adaptor_impl_index_cursor_list_join()
 *
 ...
 */
apr_array_header_t * afw_adaptor_impl_index_cursor_list_join(
    const afw_adaptor_impl_index_t * instance,
    ...
    afw_xctx_t                    * xctx)
{
    ...
    for (i = 0; i < this_list->nelts; i++) {
        cursor = ((afw_adaptor_impl_index_cursor_t**)this_list->elts)[i];
        cursor->inner_join = false;
    }

    for (i = 0; i < that_list->nelts; i++) {
        cursor = ((afw_adaptor_impl_index_cursor_t**)that_list->elts)[i];
        cursor->inner_join = false;
    }
        
    rc = afw_adaptor_impl_index_cursor_list_cardinality(
...
/*
 * afw_adaptor_impl_index_cursor_list_merge()
 *
 ...
 *
 */
apr_array_header_t * afw_adaptor_impl_index_cursor_list_merge(
    const afw_adaptor_impl_index_t * instance,
    apr_array_header_t             * this_list,
    apr_array_header_t             * that_list,
    afw_xctx_t                    * xctx)
{
    ...
    const afw_adaptor_impl_index_cursor_t *this_cursor;
    const afw_adaptor_impl_index_cursor_t *that_cursor;
    size_t this_cardinality, that_cardinality;
    afw_boolean_t rc;
    int merged_size;
    int i, j;
    ...
    for (i = 0; i < this_list->nelts; i++) {
        merged_list = apr_array_make(afw_pool_get_apr_pool(xctx->p),
            merged_size, sizeof(const afw_adaptor_impl_index_cursor_t *));

        this_cursor = ((const afw_adaptor_impl_index_cursor_t **)
            this_list->elts)[i];
...
/*
 * void afw_adaptor_impl_index_cursor_list()
 *
 ...
 *
 *      In order to do inner joins, we may rely on the adaptor to
 ...
 *
 */
apr_array_header_t * afw_adaptor_impl_index_cursor_list(
    const afw_adaptor_impl_index_t          * instance,
    const afw_utf8_t                        * object_type_id,
    const afw_query_criteria_filter_entry_t * entry,
    afw_xctx_t                             * xctx)
{  
    afw_adaptor_impl_index_cursor_t *cursor = NULL;
    ...
    cursor_list = apr_array_make(afw_pool_get_apr_pool(xctx->p), 8, 
        sizeof(const afw_adaptor_impl_index_cursor_t*));
    ...
    /* Determine if this property is indexed */
    indexDefinition = afw_adaptor_impl_index_get_index_definition(
    ...
    if (indexDefinition)
    {
        ...
        if (afw_adaptor_impl_index_option_case_insensitive(
            indexDefinition, xctx)) {
            value_string = afw_utf8_to_lower(value_string, xctx->p, xctx);
        }
        unique = afw_adaptor_impl_index_option_unique(
            indexDefinition, xctx);
        ...
        cursor = afw_adaptor_impl_index_open_cursor(instance, object_type_id, 
            ...
            AFW_QUERY_CRITERIA_CONTINUE(entry->on_false)) 
    {
        ...
        next_list = afw_adaptor_impl_index_cursor_list(
            instance, object_type_id, entry->on_true, xctx);
        
        /* compute the optimal disjunction choice */
        cursor_list = afw_adaptor_impl_index_cursor_list_join(
        ...
        cursor_list = afw_adaptor_impl_index_cursor_list_merge(instance,
    ...
    else if (AFW_QUERY_CRITERIA_CONTINUE(entry->on_true)) 
    {
        ...
        next_list = afw_adaptor_impl_index_cursor_list(instance,
...
/*
 * int afw_adaptor_impl_index_compare()
 *
 ...
 *
 */
static int afw_adaptor_impl_index_compare(
    const afw_adaptor_impl_index_t          * instance,
    const afw_query_criteria_filter_entry_t * entry,
    const afw_value_t                       * value,
    afw_xctx_t                             * xctx)
...
/*
 * afw_boolean_t afw_adaptor_impl_index_applies()
 *
 ...
 *
 */
static afw_boolean_t afw_adaptor_impl_index_applies(
    const afw_adaptor_impl_index_t        * instance,
    const afw_adaptor_impl_index_cursor_t * cursor,
    const afw_object_t                    * object,
    afw_xctx_t                           * xctx)
    ...
    if (value) {
        switch (entry->op_id) {
            case afw_query_criteria_filter_op_id_eq:
                if (afw_adaptor_impl_index_compare(instance, 
                        entry, value, xctx) == 0)
                    return true;
                break;
 ...
 */
AFW_DEFINE(void) afw_adaptor_impl_index_query(
    const afw_adaptor_impl_index_t * instance,
    const afw_utf8_t               * object_type_id,
    const afw_query_criteria_t     * criteria,
    ...
    afw_xctx_t                    * xctx)
{
    ...
    const afw_adaptor_impl_index_cursor_t *current_cursor;
    const afw_adaptor_impl_index_cursor_t *next_cursor;
    ...
    cursors = afw_adaptor_impl_index_cursor_list(instance,
    ...
    current_cursor = 
        ((const afw_adaptor_impl_index_cursor_t**)cursors->elts)[0];
    ...
    while (1)
    {
        ...
        object = afw_adaptor_impl_index_cursor_get_next_object(
            current_cursor, p, xctx);

        if (object == NULL) {
            /* No more objects left on this index cursor */
            afw_adaptor_impl_index_cursor_release(current_cursor, xctx);
        ...
        for (i = cursor_index+1; i < cursors->nelts; i++) {
            next_cursor = ((const afw_adaptor_impl_index_cursor_t**)
                cursors->elts)[i];
            /* 
             ...
             */
            if (afw_adaptor_impl_index_applies(instance,
