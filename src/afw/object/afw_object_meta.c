    ...
    afw_xctx_t *xctx)
{
    ...
    meta_self->pub.meta.id = &afw_self_s_a_meta_key;
    meta_self->pub.meta.object_type_uri = &afw_self_s__AdaptiveMeta_;
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_object_set_property(meta, &afw_self_s_parentPaths,
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!property_types) {
        property_types = afw_object_create_embedded(
            meta, &afw_self_s_propertyTypes, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property(meta, &afw_self_s_parentPaths, xctx);
    if (value) {
    ...
    path = afw_object_old_get_property_as_utf8(meta, &afw_self_s_path,
        p, xctx);
    if (path) {
        ...
        afw_object_set_property(meta, &afw_self_s_path, NULL, xctx);
    }
            ...
            if (property_types) {
                ...
                if (property_type) {
                    object_type_id = afw_object_old_get_property_as_string(
                        property_type, &afw_self_s_dataTypeParameter, xctx);
                }
            }
        }
        if (object_type_id) {
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_utf8_equal(adaptor_id, &afw_self_s_a_asterisk) ||
        afw_utf8_equal(object_type_id, &afw_self_s_a_asterisk) ||
        afw_utf8_equal(object_id, &afw_self_s_a_asterisk))
    {
    ...
    afw_xctx_t *xctx)
{
    ...
    meta = afw_object_meta_get_nonempty_delta(instance, xctx);
    errors = afw_object_old_get_property_as_array(meta, &afw_self_s_errors, xctx);
    if (!errors) {
        errors = afw_array_of_create(
            afw_data_type_string, instance->p, xctx);
        afw_object_set_property_as_array(meta,
            &afw_self_s_errors, errors, xctx);
        afw_object_set_property(meta, &afw_self_s_hasErrors, afw_value_true, xctx);
        afw_object_meta_set_property(afw_object_get_entity(instance, xctx),
            &afw_self_s_hasErrors, afw_value_true, xctx);
    }
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!result && afw_utf8_equal(property_name, &afw_self_s_properties)) {
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_utf8_equal(property_name, &afw_self_s_path)) {
