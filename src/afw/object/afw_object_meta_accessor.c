    ...
    for (;;) {
        ...
        case 0:
            ...
            if (self->limited_meta && afw_object_meta_object(self->instance)) {
                result = afw_object_get_property(
                    afw_object_meta_object(self->instance), &afw_self_s_parentPaths,
                    xctx);
                if (result) {
                    if (property_name) {
                        *property_name = &afw_self_s_parentPaths;
                    }
                    goto return_result;
                }
            }
                ...
                AFW_OBJECT_OPTION_IS(self->options, objectId))
            {
                if (self->limited_meta ||
                    !afw_object_has_property(
                        afw_object_meta_object(self->instance),
                        &afw_self_s_objectId, xctx))
                {
                    s = afw_object_meta_get_object_id(self->instance, xctx);
                    if (s) {
                        if (property_name) {
                            *property_name = &afw_self_s_objectId;
                        }
        ...
        case 3:
                ...
                (self->limited_meta && self->instance->meta.embedding_object))
            {
                        ...
                        &afw_self_s_objectType, xctx))
                {
                    ...
                    if (s) {
                        if (property_name) {
                            *property_name = &afw_self_s_objectType;
                        }
                        result = afw_value_create_string(s, p, xctx);
                        goto return_result;
                    }
        ...
        case 4:
                ...
                )
            {
                if (self->limited_meta ||
                    !afw_object_has_property(
                        afw_object_meta_object(self->instance), &afw_self_s_path, xctx))
                {
                    s = afw_object_meta_get_path(self->instance, xctx);
                    if (s) {
                        if (property_name) {
                            *property_name = &afw_self_s_path;
                        }
                        result = afw_value_create_string(s, p, xctx);
                        goto return_result;
                    }
