    ...
    afw_xctx_t *xctx)
{
    ...
    /* If single list, make new list with entries using impl_make_value(). */
        ...
        data_type = afw_array_get_data_type(
            ...
            xctx);
        list = afw_array_create_unmanaged_of(data_type, p, xctx);
        for (iterator = NULL;;) {
    ...
    afw_xctx_t *xctx)
{
    ...
    if (parent_paths) {
        ...
        resolved_parent_paths->internal = afw_array_create_unmanaged_of(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (self->inherited_properties_added) {
        ...
        "Properties already added   - " AFW_UTF8_K_FMT_Q, (path));
#endif
    ...
    path = afw_object_meta_get_path((const afw_object_t *)self, xctx);
    AFW_LOG_FZ(debug, xctx,
        "Starting to add properties - " AFW_UTF8_K_FMT_Q, (path));
#endif
    ...
    if (self->inherited_properties_being_added) {
        path = afw_object_meta_get_path((const afw_object_t *)self, xctx);
        AFW_THROW_ERROR_FZ(general, xctx,
            "parentPaths recursion loop while processing " AFW_UTF8_K_FMT_Q,
            (path));
    }
...
#ifdef __AFW_OBJECT_VIEW_DEBUG__
    AFW_LOG_FZ(debug, xctx,
        "Finished adding properties - " AFW_UTF8_K_FMT_Q, (path));
#endif
}
...
error:
    AFW_THROW_ERROR_FZ(general, xctx, AFW_UTF8_K_FMT_Q
    ...
    afw_xctx_t *xctx)
{
    ...
    if (AFW_OBJECT_OPTION_IS(view->options, composite)) {
        ...
        path = afw_object_meta_get_path((const afw_object_t *)self, xctx);
        AFW_LOG_FZ(debug, xctx,
            "Starting composite view    - " AFW_UTF8_K_FMT_Q,
            (path));
#endif
        ...
        path = afw_object_meta_get_path((const afw_object_t *)self, xctx);
        AFW_LOG_FZ(debug, xctx,
            "Finished composite view    - " AFW_UTF8_K_FMT_Q,
            (path));
#endif
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    view->journal_entry = afw_object_create_unmanaged_new_p(p, xctx);
