...
/*
 * Adaptive Framework Adapter Modify Object
 *
...
AFW_DEFINE(afw_adapter_modify_entry_type_t)
afw_adapter_modify_entry_type(
    const afw_utf8_t *mnemonic)
{
    afw_adapter_modify_entry_type_t i;

    for (i = 0;
        entry_type[i] && !afw_utf8_equal(entry_type[i], mnemonic);
        i++);

    if (i > afw_adapter_modify_entry_type_invalid) {
        i = afw_adapter_modify_entry_type_invalid;
    }
...
afw_adapter_modify_entry_type_mnemonic(
    afw_adapter_modify_entry_type_t type)
{
    return (type < 0 || type >= afw_adapter_modify_entry_type_invalid)
        ? NULL
        : entry_type[type];
}
...
afw_adapter_modify_entry_type_value(
    afw_adapter_modify_entry_type_t type)
{
    return (type < 0 || type >= afw_adapter_modify_entry_type_invalid)
        ? NULL
...
AFW_DEFINE(const afw_adapter_modify_entry_t * const *)
    ...
    const afw_array_t *list, const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    for (;;) {
        ...
        entry->type = afw_adapter_modify_entry_type(s);
        if (entry->type == afw_adapter_modify_entry_type_invalid) {
            goto error;
        }
...
afw_adapter_modify_entries_to_list(
    const afw_adapter_modify_entry_t * const *entries,
    ...
    afw_xctx_t *xctx)
{
    ...
    for (entry = entries; *entry; entry++)
    {
        ...
        /* Add value. */
        case afw_adapter_modify_entry_type_add_value:
        ...
        case afw_adapter_modify_entry_type_remove_property:
        ...
        /* Remove value. */
        case afw_adapter_modify_entry_type_remove_value:
            old_value = afw_object_path_property_name_list_get_property(object,
                first_property_name_entry, xctx);
        ...
        /* Set property. */
        case afw_adapter_modify_entry_type_set_property:
    ...
    for (iterator = NULL;;) {
        ...
        /* If there is a corresponding original value, set if different. */
            ...
            if (afw_value_is_object(modified_value)) {
                ...
                if (object_type_id) {
                    property_object_type = afw_adapter_get_object_type(
                        object_type->adapter_id, object_type_id,
                        journal_entry,
                        wa->xctx);
                }
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    *adapter_id = &parsed_path->adapter_id;
    *object_type_id = &parsed_path->object_type_id;
    *object_id = &parsed_path->entity_object_id;

    object_type = afw_adapter_get_object_type(*adapter_id, *object_type_id,
        journal_entry, xctx);
    if (!object_type) {
        AFW_THROW_ERROR_FZ(general, xctx,
    ...
    afw_xctx_t *xctx)
{
    ...
    /* Modify object. */
    afw_adapter_session_modify_object(session, &impl_request,
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_utf8_equal(object_type_id,
        AFW_OBJECT_S_OBJECT_TYPE_ID_JOURNAL_ENTRY))
    {
        afw_adapter_journal_entry_consume(session, object_id, update_object,
            xctx);
        return;
    }
     */
    ...
    afw_adapter_modify_object(
        ...
        entries, journal_entry, adapter_type_specific, xctx);
}
