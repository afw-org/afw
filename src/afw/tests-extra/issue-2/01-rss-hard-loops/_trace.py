...
Pool line (minimal):
  >debug pool release 3 in_use 100 total 200 rss 5000 KB refs 1 parent 0 (afw_pool.c:281)

Eval / lock (flow only; pair with pool_bytes_in_use() samples for memory):
...
POOL_RE = re.compile(
    r"^>debug pool (?P<op>.*) (?P<pool>-?\d+) in_use (?P<in_use>\d+) "
    r"total (?P<total>\d+) rss (?P<rss>\d+) KB refs (?P<refs>-?\d+) "
...
def format_summary(s):
    ...
    if s["first_pool"] and s["last_pool"]:
        ...
        lines.append(
            "total in_use: first=%s last=%s  delta=%s"
            % (f["total"], last["total"], last["total"] - f["total"])
        )
        lines.append(
            "rss KB: first=%s last=%s  delta=%s"
    ...
    if pools:
        lines.append("last in_use/total/rss per pool (%s pools):" % len(pools))
        ranked = sorted(
            pools.items(), key=lambda kv: kv[1]["in_use"], reverse=True)
        show = ranked[:12]
        for pool, rec in show:
            lines.append(
                "  pool %s  parent=%s  in_use=%s  total=%s  rss=%s KB  "
                "refs=%s  last=%s (%s)"
                % (pool, rec["parent"], rec["in_use"], rec["total"],
                   rec["rss"], rec["refs"], rec["op"], rec["src"])
            )
...
def main(argv):
    ...
    if args.path == "-":
        s = summarize(sys.stdin, top=args.top)
    else:
