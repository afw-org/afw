...
/**
 * @file afw_request_handler_adaptor.c
 * @brief Implementation of afw_request_handler interface for adaptor
 *
 * This request handler is used to interface with an adaptor.  If
...
{
    afw_request_handler_adaptor_internal_self_t *self;
    const afw_object_t *options_object;

    self = afw_pool_calloc_type(p, afw_request_handler_adaptor_internal_self_t, xctx);
    ...
    afw_xctx_t *xctx)
{
     ...
     *  the adaptive object. {Table} is the adaptor id. {id} is the object id.
     *
     *  FIXME fix comment
     * 
    ...
    AFW_TRY {
        ...
        if (afw_utf8_equal_utf8_z(request->method, AFW_REQUEST_Q_METHOD_GET))
        {
            ...
            else if (parsed_path->entity_object_id.len == 0)
            {
                ...
                /* Call adaptor to query for objects. */
                afw_adaptor_retrieve_objects(
                    &parsed_path->adaptor_id, &parsed_path->object_type_id,
            ...
            /* If get_object(), which is a GET where path does not end with '/'. */
                ...
                /* If no property name in path, obj is value. */
                    ...
                    value = afw_value_create_object_unmanaged(obj, xctx->p, xctx);
                }
            ...
            AFW_REQUEST_Q_METHOD_PUT))
        {
            ...
            afw_adaptor_replace_object(&parsed_path->adaptor_id,
            ...
            AFW_REQUEST_Q_METHOD_POST))
        {
             ...
             *
             * POST /<adaptor_id>/<object_type>/<object_id>
             *
            ...
            if (afw_value_is_array(value)) {
                ...
                /* An error if just adaptor_id other than afw. */
                ...
                /* If Add ... */
                    ...
                    afw_adaptor_add_object(&parsed_path->adaptor_id,
    ...
    afw_adaptor_session_commit_and_release_cache(false, xctx);
