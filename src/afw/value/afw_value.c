    ...
    if (afw_value_is_array(value)) {
        ...
        *c++ = ')';
        ...
        result = afw_utf8_concat(p, xctx,
            ...
            &afw_self_s_a_open_parenthesis, &impl_s_a_quote,
            afw_value_as_utf8(value, p, xctx),
            &impl_s_a_quote, &afw_self_s_a_close_parenthesis,
            NULL);
    }
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!substring_data_type ||
        !afw_utf8_equal(&substring_data_type->cType, &afw_self_s_afw_utf8_t))
    {
        AFW_THROW_ERROR_Z(general,
