...
AFW_DEFINE(afw_value_unmanaged_t *)
afw_value_unmanaged_allocate(
    const afw_data_type_t *data_type,
    const afw_pool_t *p,
    afw_xctx_t *xctx)
{
    afw_value_unmanaged_t *result;

    if (!data_type->evaluated_value_inf) {
        AFW_THROW_ERROR_Z(general,
            "afw_value_unmanaged_allocate() called with data type special "
            "data type that does not have evaluated_value_inf",
            xctx);
    }

    result = afw_pool_calloc(p,
        offsetof(afw_value_unmanaged_t, internal) + data_type->c_type_size,
        xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!data_type) {
        AFW_THROW_ERROR_Z(general,
            "afw_value_unmanaged_create() called with NULL data type",
            xctx);
    }
    result = afw_pool_calloc(p,
        offsetof(afw_value_unmanaged_t, internal) + data_type->c_type_size,
