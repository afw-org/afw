    ...
    const afw_error_t **environment_create_error)
{
    ...
    env->pub.multithreaded_pool_lock =
        afw_lock_create_environment_lock(
            &afw_self_s_a_lock_multithreaded_pool, p, xctx);

    /* Create environment lock. */
    env->pub.environment_lock =
        afw_lock_create_and_register(
            &afw_self_s_a_lock_environment,
            &afw_self_s_a_lock_environment_brief,
            &afw_self_s_a_lock_environment_description,
            true, xctx);

    /* Create adaptor id anchors lock. */
    env->pub.adaptor_id_anchor_lock =
        afw_lock_create_and_register(
            &afw_self_s_a_lock_adaptor_id_anchor,
            &afw_self_s_a_lock_adaptor_id_anchor_brief,
            &afw_self_s_a_lock_adaptor_id_anchor_description,
            true, xctx);
    ...
    env->pub.authorization_handler_id_anchor_rw_lock =
        afw_lock_create_rw_and_register(
            &afw_self_s_a_lock_authorization_handler_id_anchor,
            &afw_self_s_a_lock_authorization_handler_id_anchor_brief,
            &afw_self_s_a_lock_authorization_handler_id_anchor_description,
            xctx);

    /* Create active log list lock. */
    env->pub.active_log_list_lock =
        afw_lock_create_and_register(
            &afw_self_s_a_lock_log_list,
            &afw_self_s_a_lock_log_list_brief,
            &afw_self_s_a_lock_log_list_description,
            true, xctx);

    /* Create flags lock. */
    env->pub.flags_lock  =
        afw_lock_create_and_register(
            &afw_self_s_a_lock_flags,
            &afw_self_s_a_lock_flags_brief,
            &afw_self_s_a_lock_flags_description,
            true, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        if (!type && load_extension) {
            ctx.type = &afw_self_s_registry_type;
    ...
    afw_xctx_t *xctx)
{
    ...
    registers_value = afw_object_get_property(object, &afw_self_s_registers, xctx);
    if (!registers_value) {
        return false;
    }
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        if (extension_id) {
            ...
            if (!properties)
            {
                ...
                afw_object_set_property_as_string(properties,
                    &afw_self_s_extensionId, extension_id, xctx);
            }

            if (!module_path) {
                manifest = afw_runtime_get_object(&afw_self_s__AdaptiveManifest_,
                    extension_id, xctx);
                if (manifest) {
            ...
            afw_object_set_property_as_string(properties,
                &afw_self_s_modulePath, module_path, xctx); */

            /* Prepare properties. Supply type=extension if needed. */
            if (!afw_object_has_property(properties, &afw_self_s_type, xctx)) {
                afw_object_set_property_as_string(properties,
                    &afw_self_s_type, &afw_self_s_extension, xctx);
            }
        ...
        /* If just path_z specified. */
            ...
            extension_id_for_message = &afw_self_s_unknown;
         ...
         */
        if (extension_id_for_message == &afw_self_s_unknown) {
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        afw_environment_register_flag(detail_flag_id, brief, description,
            &afw_self_s_a_flag_trace_adaptor_detail, xctx);
        ...
        afw_environment_register_flag(flag_id, brief, description,
            &afw_self_s_a_flag_trace_adaptor, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        afw_environment_register_flag(detail_flag_id, brief, description,
            &afw_self_s_a_flag_trace_authorizationHandler_detail, xctx);
        ...
        afw_environment_register_flag(flag_id, brief, description,
            &afw_self_s_a_flag_trace_authorizationHandler, xctx);
