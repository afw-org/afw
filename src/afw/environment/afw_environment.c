    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        if (!type) {
            AFW_THROW_ERROR_FZ(general, xctx,
                "Invalid environment registry type " AFW_UTF8_K_FMT_Q,
                (type_id));
        }
    }
    AFW_LOCK_END;
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        if (type) {
            AFW_THROW_ERROR_FZ(general, xctx,
                "registry_type " AFW_UTF8_K_FMT_Q " is already assigned",
                (registry_type_id));
        }
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        if (old_value && !type->allow_reregister) {
            AFW_THROW_ERROR_FZ(general, xctx,
                AFW_UTF8_K_FMT_Q " " AFW_UTF8_K_FMT_Q " is already registered",
    ...
    afw_xctx_t *xctx)
{
    ...
    if (properties) {
        ...
        if (extensionId) {
            if (extension_id && !afw_utf8_equal(extension_id, extensionId)) {
                AFW_THROW_ERROR_FZ(general, xctx,
                    "extension_id parameter " AFW_UTF8_K_FMT_Q
                    " does not match properties.extension_id " AFW_UTF8_K_FMT_Q,
                    (extension_id),
                    (extensionId));
            }
        }
        ...
        if (modulePath) {
            if (module_path) {
                if (!afw_utf8_equal(module_path, modulePath)) {
                    AFW_THROW_ERROR_FZ(general, xctx,
                        "module_path parameter " AFW_UTF8_K_FMT_Q
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        if (rv != APR_SUCCESS) {
            path_z = afw_utf8_z_printf(p, xctx,
                AFW_CONFIG_INSTALL_FULL_LIBDIR "/" AFW_UTF8_K_FMT,
                (module_path));
            rv = apr_dso_load(&(dso_handle), (const char *)path_z,
                afw_pool_get_apr_pool(p));
            if (rv != APR_SUCCESS) {
                ...
                if (!afw_utf8_ends_with(module_path, dso_suffix)) {
                    path_z = afw_utf8_z_printf(p, xctx,
                        AFW_CONFIG_INSTALL_FULL_LIBDIR "/" AFW_UTF8_K_FMT AFW_UTF8_K_FMT,
                        (module_path),
                        (dso_suffix));
                    rv = apr_dso_load(&(dso_handle), (const char *)path_z,
                        afw_pool_get_apr_pool(p));
        ...
        if (rv != APR_SUCCESS) {
            ...
            AFW_THROW_ERROR_RV_FZ(general, apr, rv, xctx,
                "Error loading extension extension_id=" AFW_UTF8_K_FMT_Q
                " modulePath='%s': %s",
        ...
        if (rv != APR_SUCCESS) {
            ...
            AFW_THROW_ERROR_RV_FZ(general, apr, rv, xctx,
                ...
                " in extension_id=" AFW_UTF8_K_FMT_Q " modulePath='%s'",
        ...
        AFW_LOG_FZ(info, xctx,
            "Extension " AFW_UTF8_K_FMT_Q " loaded.",
            (extension_id));
    ...
    afw_xctx_t *xctx)
{
    ...
    /* Make sure data type is not already registered. */
        if (afw_environment_get_data_type(data_type_id, xctx)) {
            AFW_THROW_ERROR_FZ(general, xctx,
                "Data type " AFW_UTF8_K_FMT_Q " is already registered",
                (data_type_id));
        }
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
            ...
            !function->data_type)
        {
            ...
            if (!function->data_type) {
                AFW_THROW_ERROR_FZ(general, xctx,
                "dataType " AFW_UTF8_K_FMT_Q " doesn't exist",
                (&function->dataType->internal));
            }
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        brief = afw_utf8_printf(p, xctx,
            "Trace adapter type " AFW_UTF8_K_FMT_Q,
            (adapter_type));
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_LOCK_BEGIN(xctx->env->environment_lock) {
        ...
        detail_flag_id = afw_utf8_printf(p, xctx,
            "trace:authorizationHandlerType:" AFW_UTF8_K_FMT ":detail",
            (authorization_handler_type));

        brief = afw_utf8_printf(p, xctx,
            "Debug trace authorizationHandler type " AFW_UTF8_K_FMT_Q,
            (authorization_handler_type));

        description = afw_utf8_printf(p, xctx,
            "This produces a basic plus additional detail trace for "
            "all authorizationHandlers of type " AFW_UTF8_K_FMT_Q ".",
            (authorization_handler_type));
        ...
        flag_id = afw_utf8_printf(p, xctx,
            "trace:authorizationHandlerType:" AFW_UTF8_K_FMT,
            (authorization_handler_type));

        brief = afw_utf8_printf(p, xctx,
            "Trace authorizationHandler type " AFW_UTF8_K_FMT,
            (authorization_handler_type));

        description = afw_utf8_printf(p, xctx,
            "This produces a basic trace of all authorizationHandlers "
            "of type " AFW_UTF8_K_FMT_Q ".",
            (authorization_handler_type));
