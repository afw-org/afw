    ...
    afw_xctx_t *xctx)
{
    ...
    afw_object_set_property_as_string(conf,
        &afw_self_s_sourceLocation, source_location, xctx);

    /* Get type property. */
    type = afw_object_old_get_property_as_utf8(conf, &afw_self_s_type, p, xctx);
    if (!type) {
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!source_location) {
        source_location = &afw_self_s_Configuration;
    }
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!source_location) {
        source_location = &afw_self_s_a_empty_string;
    }
    ...
    if (source_location == &afw_self_s_a_empty_string) {
    ...
    result = properties;
    if (!afw_utf8_equal(type, &afw_self_s_adaptor) ||
        !afw_utf8_equal(id, &afw_self_s_afw))
    {
