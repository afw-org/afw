    ...
    afw_xctx_t *xctx)
{
    ...
    if (parsed->original_uri) {
        afw_object_set_property_as_string(result,
            &afw_self_s_originalURI, parsed->original_uri, xctx);
    }

    if (parsed->normalized_uri.len > 0) {
        afw_object_set_property_as_string(result,
            &afw_self_s_normalizedURI, &parsed->normalized_uri, xctx);
    }

    if (parsed->scheme.len > 0) {
        afw_object_set_property_as_string(result,
            &afw_self_s_scheme, &parsed->scheme, xctx);
    }

    if (parsed->original_hier_part.len > 0) {
        afw_object_set_property_as_string(result,
            &afw_self_s_originalHierPart, &parsed->original_hier_part, xctx);
    }

    if (parsed->original_authority.len > 0) {
        afw_object_set_property_as_string(result,
            &afw_self_s_originalAuthority, &parsed->original_authority, xctx);
    }

    if (parsed->authority) {
        afw_object_set_property_as_string(result,
            &afw_self_s_authority, parsed->authority, xctx);
    }

    if (parsed->original_userinfo.len > 0) {
        afw_object_set_property_as_string(result,
            &afw_self_s_originalUserinfo, &parsed->original_userinfo, xctx);
    }

    if (parsed->userinfo) {
        afw_object_set_property_as_string(result,
            &afw_self_s_userinfo, parsed->userinfo, xctx);
    }

    if (parsed->original_host.len > 0) {
        afw_object_set_property_as_string(result,
            &afw_self_s_originalHost, &parsed->original_host, xctx);
    }

    if (parsed->host) {
        afw_object_set_property_as_string(result,
            &afw_self_s_host, parsed->host, xctx);
    }

    if (parsed->port.len > 0) {
        afw_object_set_property_as_string(result,
            &afw_self_s_port, &parsed->port, xctx);
    }
    ...
    if (parsed->original_path.len > 0) {
        afw_object_set_property_as_string(result, &afw_self_s_path,
        ...
        afw_object_set_property_as_array(result,
            &afw_self_s_pathTokens, list, xctx);
    }

    if (parsed->original_query.len > 0) {
        afw_object_set_property_as_string(result,
            &afw_self_s_originalQuery, &parsed->original_query, xctx);
    }

    if (parsed->original_query.len > 0) {
        afw_object_set_property_as_string(result, &afw_self_s_query,
        ...
        afw_object_set_property_as_array(result,
            &afw_self_s_queryTokens, list, xctx);
    }

    if (parsed->original_fragment.len > 0) {
        afw_object_set_property_as_string(result,
            &afw_self_s_originalFragment, &parsed->original_fragment, xctx);
    }

    if (parsed->original_fragment.len > 0) {
        afw_object_set_property_as_string(result, &afw_self_s_fragment,
        ...
        afw_object_set_property_as_array(result,
            &afw_self_s_fragmentTokens, list, xctx);
    }
    ...
    if (parsed->path_parsed) {
        ...
        afw_object_set_property_as_object(result,
            &afw_self_s_valuePath, object, xctx);
    }
