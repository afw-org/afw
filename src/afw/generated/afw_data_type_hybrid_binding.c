...
/**
 * @file afw_data_type_hybrid_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_hybrid_direct

/* Define inf variables for data_type and is_evaluated_of_data_type. */
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    (const void *)&afw_data_type_hybrid_direct, \
    (const void *)&afw_data_type_hybrid_direct

/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for unmanaged hybrid values. For this one */
/* optional_release is NULL and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "hybrid"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_evaluated_hybrid_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for managed hybrid values. For this one */
/* optional_release releases value and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "managed_hybrid"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_hybrid_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for permanent hybrid values. For this one */
/* optional_release is NULL and get_reference returns instance asis. */
#define AFW_IMPLEMENTATION_ID "permanent_hybrid"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_hybrid_inf
#define impl_afw_value_optional_release NULL
...
impl_data_type_hybrid_id_value = {
    &afw_value_evaluated_string_inf,
    AFW_UTF8_LITERAL("hybrid")
};

AFW_DEFINE_CONST_DATA(afw_value_t *)
afw_data_type_hybrid_id_value =
{
    (const afw_value_t *)&impl_data_type_hybrid_id_value
};

static const afw_utf8_t
impl_data_type_path_hybrid =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/hybrid");

/* Data type hybrid object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_hybrid = {
    {
...
afw_data_type_hybrid_direct = {
    ...
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_hybrid,
    (const afw_value_t *)&impl_value_empty_array_of_hybrid,
...
/* Data type struct for hybrid. */
AFW_DEFINE_CONST_DATA(afw_data_type_t *)
afw_data_type_hybrid =
    &afw_data_type_hybrid_direct;

/* Set property function for data type hybrid values. */
AFW_DEFINE(void)
afw_object_set_property_as_hybrid(
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_hybrid(internal, object->p, xctx);
    afw_object_set_property(object, property_name, v, xctx);
}

/* Typesafe cast of data type hybrid. */
AFW_DEFINE(const afw_utf8_t *)
afw_value_as_hybrid(const afw_value_t *value, afw_xctx_t *xctx)
{
    value = afw_value_evaluate(value, xctx->p, xctx);
    if (!AFW_VALUE_IS_DATA_TYPE(value, hybrid))
    {
        const afw_utf8_t *data_type_id;

        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'hybrid' but "
    ...
    return &(((const afw_value_hybrid_t *)value)->internal);
}

/* Allocate function for unmanaged data type hybrid values. */
AFW_DEFINE(afw_value_hybrid_t *)
afw_value_allocate_hybrid(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_hybrid_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_hybrid_t),
        xctx);
    result->inf = &afw_value_evaluated_hybrid_inf;
    return result;
}

/* Create function for unmanaged data type hybrid value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_hybrid(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_hybrid_t *v;

    v = afw_value_allocate_hybrid(p, xctx);
    if (internal) {
...
/* Allocate function for managed data type hybrid values. */
AFW_DEFINE(afw_value_hybrid_t *)
afw_value_allocate_managed_hybrid(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_hybrid_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_hybrid_t),
        xctx);
    result->inf = &afw_value_managed_hybrid_inf;
    return result;
}

/* Create function for data type hybrid value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_managed_hybrid(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_hybrid_t *v;

    v = afw_value_allocate_managed_hybrid(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return (const afw_value_t *)v;
...
/* Allocate function for permanent data type hybrid values. */
AFW_DEFINE(afw_value_hybrid_t *)
afw_value_allocate_permanent_hybrid(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_hybrid_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_hybrid_t),
        xctx);
    result->inf = &afw_value_permanent_hybrid_inf;
    return result;
}

/* Create function for data type hybrid value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_permanent_hybrid(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_hybrid_t *v;

    v = afw_value_allocate_permanent_hybrid(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return (const afw_value_t *)v;
...
/* Convert data type hybrid string to afw_utf8_t *. */
AFW_DEFINE(void)
afw_data_type_hybrid_to_internal(afw_utf8_t *to_internal,
...
/*  Convert data type hybrid internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
afw_data_type_hybrid_to_utf8(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Get property function for data type hybrid values. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_property_as_hybrid_source(
    const afw_object_t *object,
    const afw_utf8_t *property_name,
...
/* Get next property function for data type hybrid values. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_next_property_as_hybrid_source(
    const afw_object_t *object,
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_hybrid;
    info->optimized_value = instance;
}


/* Get next value from array of hybrid. */
AFW_DEFINE(const afw_utf8_t *)
afw_array_of_hybrid_get_next_source(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (data_type != afw_data_type_hybrid) {
        const afw_utf8_t *data_type_id;
...
/* Add value from array of hybrid */
AFW_DEFINE(void)
afw_array_of_hybrid_add(
    const afw_array_t *instance,
...
/* Remove value from array of hybrid */
AFW_DEFINE(void)
afw_array_of_hybrid_remove(
    const afw_array_t *instance,
    const afw_utf8_t *value,
    afw_xctx_t *xctx)
{
