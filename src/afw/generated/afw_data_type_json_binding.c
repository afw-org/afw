...
/**
 * @file afw_data_type_json_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_json_direct
...
/* unmanaged json: get_reference/release throw; */
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_json_direct, \
    true
/* managed json: optional_release drops RC; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice json: own RC; holds containing; */
/* last release frees slice header via the stored p. */
#define AFW_IMPLEMENTATION_ID "managed_slice_json"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_slice_json_inf
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_json_direct, \
    false
/* permanent json: optional_release NULL; */
/* get_reference / get_assignable_value as-is. */
#define AFW_IMPLEMENTATION_ID "permanent_json"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_json_inf
...
{
    (const afw_value_t *)&impl_data_type_json_id_value
};

static const afw_utf8_t
impl_data_type_path_json =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/json");

static const afw_value_object_t
impl_data_type_object_json__value;

/* Data type json object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_json = {
    {
        &afw_runtime_inf__AdaptiveDataType_,
        NULL,
        (const afw_value_t *)&impl_data_type_object_json__value,
        {
            NULL,
            NULL,
            &afw_self_s_json,
    ...
    (void *)&afw_data_type_json_direct
};
...
/* Permanent empty array of json. */
const afw_array_from_values_self_t
impl_empty_array_of_json;

/* Permanent empty array value of json. */
...
afw_data_type_json_direct = {
    ...
    (const afw_object_t *)&impl_data_type_object_json,
    AFW_UTF8_LITERAL("json"),
    AFW_UTF8_LITERAL("JSON source"),
    AFW_UTF8_LITERAL("Data type 'json' holds the source for a JSON value."),
    AFW_UTF8_LITERAL("SourceParameter"),
    afw_data_type_number_json,
...
/* Data type struct for json. */
AFW_DEFINE_CONST_DATA(afw_data_type_t *)
afw_data_type_json =
    &afw_data_type_json_direct;

/* Set property from json value. */
AFW_DEFINE(void)
...
/* Set property from json internal. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_json_create_managed(internal, object->p, xctx);
    }
    else {
        v = afw_value_json_create(internal, object->p, xctx);
    }
    afw_object_set_property(object, property_name, v, xctx);
}

/* Typesafe cast to evaluated json value. */
AFW_DEFINE(const afw_value_json_t *)
afw_value_as_json(const afw_value_t *value, afw_xctx_t *xctx)
{
    value = afw_value_evaluate(value, xctx->p, xctx);
    if (!AFW_VALUE_IS_DATA_TYPE(value, json))
    {
        const afw_utf8_t *data_type_id;

        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'json' but "
    ...
    return (const afw_value_json_t *)value;
}

/* Typesafe peel of data type json internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_value_as_json_internal(const afw_value_t *value, afw_xctx_t *xctx)
{
    return &afw_value_as_json(value, xctx)->internal;
}

/* Allocate function for data type json values. */
AFW_DEFINE(afw_value_json_t *)
afw_value_json_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
...
afw_value_json_create_managed_slice(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_json_managed_t *containing;
    const afw_utf8_t *base;
    afw_value_json_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_json_inf) {
        const afw_value_json_managed_slice_t *slice =
            (const afw_value_json_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_json_inf) {
        containing =
        ...
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice json",
            xctx);
    }
    ...
    p = p->managed_p;
    v = afw_pool_calloc(p, sizeof(afw_value_json_managed_slice_t), xctx);
...
/* Deep clone evaluated json unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_json_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_json_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_json, p, xctx);
    afw_data_type_clone_internal(afw_data_type_json,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_json_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated json managed in p->managed_p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_json_managed(
    const afw_value_t *value,
    const afw_pool_t *p,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_json_create_managed(
        &((const afw_value_json_t *)value)->internal,
        p, xctx);
}
...
/*  Convert data type json internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
afw_data_type_json_to_utf8(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_json_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as json value. */
AFW_DEFINE(const afw_value_json_t *)
...
/* Get next property as json internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_json_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_json_t *self =
        (const afw_value_json_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_json_managed_slice_t *self =
        (afw_value_json_managed_slice_t *)instance;
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_json;
    info->optimized_value = instance;
}


/* Get next json value from array of json. */
AFW_DEFINE(const afw_value_json_t *)
...
/* Get next json internal from array of json. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_json_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a json value to array of json. */
AFW_DEFINE(void)
...
/* Add a json internal to array of json. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_json_create_managed(value, instance->p, xctx);
    }
    else {
        v = afw_value_json_create(value, instance->p, xctx);
    }
    afw_array_push_value(instance, v, xctx);
}

/* Remove a json value from array of json. */
AFW_DEFINE(void)
afw_array_of_json_remove(
    ...
    afw_xctx_t *xctx)
{
    afw_array_of_json_remove_internal(instance, &value->internal, xctx);
}

/* Remove a json internal from array of json. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_json_create(value, xctx->p, xctx);
