    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_json_create_managed(internal, xctx);
    }
    else {
...
/* Clone evaluated json managed in xctx->p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_json_managed(
    const afw_value_t *value,
    afw_xctx_t *xctx)
...
/* Get property function for data type json internal. */
AFW_DEFINE(const afw_utf8_t *)
...
/* Get next property function for data type json internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_json_create_managed(value, xctx);
    }
    else {
