...
/* unmanaged null: get_reference/release throw; */
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_null_direct, \
    true
/* managed null: optional_release drops RC; */
...
impl_data_type_object_null__value;
...
impl_data_type_object_null = {
    {
        ...
        NULL,
        (const afw_value_t *)&impl_data_type_object_null__value,
        {
            NULL,
            NULL,
            &afw_self_s_null,
...
/* Permanent empty array of null. */
const afw_array_from_values_self_t
impl_empty_array_of_null;

/* Permanent empty array value of null. */
...
afw_value_null_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Create function for managed data type null value. */
...
/* Clone evaluated null managed in p->managed_p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_null_managed(
    const afw_value_t *value,
    const afw_pool_t *p,
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_null_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as null value. */
AFW_DEFINE(const afw_value_null_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_null_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return value->internal;
