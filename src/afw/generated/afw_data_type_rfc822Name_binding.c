...
/**
 * @file afw_data_type_rfc822Name_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_rfc822Name_direct
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    (const void *)&afw_data_type_rfc822Name_direct, \
    (const void *)&afw_data_type_rfc822Name_direct, \
    false
...
/* unmanaged rfc822Name: get_reference/release throw; */
/* get_assignable_value creates a managed holdable in p->managed_p. */
#define AFW_IMPLEMENTATION_ID "rfc822Name"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_unmanaged_rfc822Name_inf
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_rfc822Name_direct, \
    true
/* managed rfc822Name: optional_release drops RC; */
/* scalar last-release free_memorys via the stored p. */
/* get_reference / get_assignable_value bump. */
#define AFW_IMPLEMENTATION_ID "managed_rfc822Name"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_rfc822Name_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice rfc822Name: own RC; holds containing; */
/* last release frees slice header via the stored p. */
#define AFW_IMPLEMENTATION_ID "managed_slice_rfc822Name"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_slice_rfc822Name_inf
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_rfc822Name_direct, \
    false
/* permanent rfc822Name: optional_release NULL; */
/* get_reference / get_assignable_value as-is. */
#define AFW_IMPLEMENTATION_ID "permanent_rfc822Name"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_rfc822Name_inf
...
impl_data_type_rfc822Name_id_value = {
...
{
    (const afw_value_t *)&impl_data_type_rfc822Name_id_value
};

static const afw_utf8_t
impl_data_type_path_rfc822Name =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/rfc822Name");

static const afw_value_object_t
impl_data_type_object_rfc822Name__value;

/* Data type rfc822Name object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_rfc822Name = {
    {
        &afw_runtime_inf__AdaptiveDataType_,
        NULL,
        (const afw_value_t *)&impl_data_type_object_rfc822Name__value,
        {
            NULL,
            NULL,
    ...
    (void *)&afw_data_type_rfc822Name_direct
};
...
impl_value_empty_array_of_rfc822Name;

/* Data type rfc822Name instance. */
...
afw_data_type_rfc822Name_direct = {
    ...
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_rfc822Name,
...
/* Data type struct for rfc822Name. */
AFW_DEFINE_CONST_DATA(afw_data_type_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_rfc822Name_create_managed(internal, object->p, xctx);
    }
    else {
...
afw_value_rfc822Name_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_rfc822Name_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_rfc822Name_t),
        xctx);
    result->inf = &afw_value_unmanaged_rfc822Name_inf;
    return result;
}

/* Create function for managed data type rfc822Name value. */
...
afw_value_rfc822Name_create_managed(
    ...
    afw_xctx_t *xctx)
{
    afw_value_rfc822Name_managed_t *v;
    afw_size_t len;

    p = p->managed_p;
    len = (internal) ? internal->len : 0;
    v = afw_pool_calloc(p,
        sizeof(afw_value_rfc822Name_managed_t) + len, xctx);
    v->inf = &afw_value_managed_rfc822Name_inf;
    v->internal.len = len;
    v->internal.s = (const afw_utf8_octet_t *)v +
        sizeof(afw_value_rfc822Name_managed_t);
...
/* Create function for managed data type rfc822Name slice value. */
AFW_DEFINE(const afw_value_t *)
afw_value_rfc822Name_create_managed_slice(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_rfc822Name_managed_t *containing;
    const afw_utf8_t *base;
    afw_value_rfc822Name_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_rfc822Name_inf) {
        const afw_value_rfc822Name_managed_slice_t *slice =
            (const afw_value_rfc822Name_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_rfc822Name_inf) {
        containing =
        ...
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice rfc822Name",
            xctx);
    }
    ...
    p = p->managed_p;
    v = afw_pool_calloc(p, sizeof(afw_value_rfc822Name_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_rfc822Name_inf;
...
/* Create function for data type rfc822Name value. */
AFW_DEFINE(const afw_value_t *)
afw_value_rfc822Name_create(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_rfc822Name_t *v;

    v = afw_pool_calloc(p, sizeof(afw_value_rfc822Name_t),
        xctx);
    v->inf = &afw_value_unmanaged_rfc822Name_inf;
    if (internal) {
...
/* Deep clone evaluated rfc822Name unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_rfc822Name_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_rfc822Name_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_rfc822Name, p, xctx);
    afw_data_type_clone_internal(afw_data_type_rfc822Name,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_rfc822Name_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated rfc822Name managed in p->managed_p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_rfc822Name_managed(
    const afw_value_t *value,
    const afw_pool_t *p,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_rfc822Name_create_managed(
        &((const afw_value_rfc822Name_t *)value)->internal,
        p, xctx);
}

/* Convert data type rfc822Name string to afw_utf8_t *. */
AFW_DEFINE(void)
...
/*  Convert data type rfc822Name internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
...
impl_afw_value_managed_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_rfc822Name_managed_t *self =
        (afw_value_rfc822Name_managed_t *)instance;
    ...
    if (self->reference_count == 0) {
        afw_pool_free_memory(self->p, self,
            sizeof(afw_value_rfc822Name_managed_t) + self->internal.len, xctx);
    }
}
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_rfc822Name_t *self =
        (const afw_value_rfc822Name_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_rfc822Name_managed_slice_t *self =
        (afw_value_rfc822Name_managed_slice_t *)instance;
    ...
    if (self->reference_count == 0) {
        ...
        afw_pool_free_memory(self->p, self,
            sizeof(afw_value_rfc822Name_managed_slice_t), xctx);
    }
}
...
impl_afw_value_get_data_type(
    ...
    afw_xctx_t *xctx)
{
    return afw_data_type_rfc822Name;
}
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_rfc822Name;
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_rfc822Name_create_managed(value, instance->p, xctx);
    }
    else {
