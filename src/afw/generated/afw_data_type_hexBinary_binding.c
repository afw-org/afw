...
/**
 * @file afw_data_type_hexBinary_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_hexBinary_direct
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    (const void *)&afw_data_type_hexBinary_direct, \
    (const void *)&afw_data_type_hexBinary_direct, \
    false
...
/* unmanaged hexBinary: get_reference/release throw; */
/* get_assignable_value creates a managed holdable in p->managed_p. */
#define AFW_IMPLEMENTATION_ID "hexBinary"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_unmanaged_hexBinary_inf
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_hexBinary_direct, \
    true
/* managed hexBinary: optional_release drops RC; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice hexBinary: own RC; holds containing; */
/* last release frees slice header via the stored p. */
#define AFW_IMPLEMENTATION_ID "managed_slice_hexBinary"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_slice_hexBinary_inf
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_hexBinary_direct, \
    false
/* permanent hexBinary: optional_release NULL; */
/* get_reference / get_assignable_value as-is. */
#define AFW_IMPLEMENTATION_ID "permanent_hexBinary"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_hexBinary_inf
...
impl_data_type_hexBinary_id_value = {
...
{
    (const afw_value_t *)&impl_data_type_hexBinary_id_value
};

static const afw_utf8_t
impl_data_type_path_hexBinary =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/hexBinary");

static const afw_value_object_t
impl_data_type_object_hexBinary__value;

/* Data type hexBinary object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_hexBinary = {
    {
        &afw_runtime_inf__AdaptiveDataType_,
        NULL,
        (const afw_value_t *)&impl_data_type_object_hexBinary__value,
        {
            NULL,
            NULL,
    ...
    (void *)&afw_data_type_hexBinary_direct
};

static const afw_value_object_t
impl_data_type_object_hexBinary__value = {
    {&afw_value_permanent_object_inf},
    (const afw_object_t *)&impl_data_type_object_hexBinary
};
...
impl_value_empty_array_of_hexBinary;

/* Data type hexBinary instance. */
AFW_DEFINE_CONST_DATA(afw_data_type_t)
afw_data_type_hexBinary_direct = {
    &afw_data_type_hexBinary_inf,
    (const afw_object_t *)&impl_data_type_object_hexBinary,
    AFW_UTF8_LITERAL("hexBinary"),
    AFW_UTF8_LITERAL("A binary value whose string value is character hex encoded"),
    AFW_UTF8_LITERAL("Data type 'hexBinary' holds binary data. Its string value is character hex encoded."),
    AFW_UTF8_LITERAL("MediaType"),
    afw_data_type_number_hexBinary,
    AFW_UTF8_LITERAL("1.3.6.1.4.1.1466.115.121.1.5"),
    AFW_UTF8_LITERAL("string"),
    AFW_UTF8_LITERAL(""),
    AFW_UTF8_LITERAL("afw_memory_t"),
    sizeof(afw_memory_t),
    (const afw_array_t *)&impl_empty_array_of_hexBinary,
...
/* Data type struct for hexBinary. */
AFW_DEFINE_CONST_DATA(afw_data_type_t *)
afw_data_type_hexBinary =
    &afw_data_type_hexBinary_direct;

/* Set property from hexBinary value. */
AFW_DEFINE(void)
...
/* Set property from hexBinary internal. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_hexBinary_create_managed(internal, object->p, xctx);
    }
    else {
        v = afw_value_hexBinary_create(internal, object->p, xctx);
    }
    ...
    if (!AFW_VALUE_IS_DATA_TYPE(value, hexBinary))
    {
        ...
        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'hexBinary' but "
...
AFW_DEFINE(afw_value_hexBinary_t *)
afw_value_hexBinary_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_hexBinary_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_hexBinary_t),
        xctx);
    result->inf = &afw_value_unmanaged_hexBinary_inf;
    return result;
}
...
afw_value_hexBinary_create_managed_slice(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_hexBinary_managed_t *containing;
    const afw_memory_t *base;
    afw_value_hexBinary_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_hexBinary_inf) {
        const afw_value_hexBinary_managed_slice_t *slice =
            (const afw_value_hexBinary_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_hexBinary_inf) {
        containing =
        ...
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice hexBinary",
            xctx);
    }
    ...
    p = p->managed_p;
    v = afw_pool_calloc(p, sizeof(afw_value_hexBinary_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_hexBinary_inf;
...
/* Create function for data type hexBinary value. */
AFW_DEFINE(const afw_value_t *)
...
/* Deep clone evaluated hexBinary unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_hexBinary_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_hexBinary_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_hexBinary, p, xctx);
    afw_data_type_clone_internal(afw_data_type_hexBinary,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_hexBinary_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated hexBinary managed in p->managed_p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_hexBinary_managed(
    const afw_value_t *value,
    const afw_pool_t *p,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_hexBinary_create_managed(
        &((const afw_value_hexBinary_t *)value)->internal,
        p, xctx);
}

/* Convert data type hexBinary string to afw_memory_t *. */
AFW_DEFINE(void)
...
/*  Convert data type hexBinary internal representation to utf-8. */
...
/* Get property as hexBinary internal. */
AFW_DEFINE(const afw_memory_t *)
...
/* Get next property as hexBinary internal. */
AFW_DEFINE(const afw_memory_t *)
...
impl_afw_value_managed_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_hexBinary_managed_t *self =
        (afw_value_hexBinary_managed_t *)instance;
    ...
    if (self->reference_count == 0) {
        afw_pool_free_memory(self->p, self,
            sizeof(afw_value_hexBinary_managed_t) + self->internal.size, xctx);
    }
}
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_hexBinary_t *self =
        (const afw_value_hexBinary_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_hexBinary_managed_slice_t *self =
        (afw_value_hexBinary_managed_slice_t *)instance;
    ...
    if (self->reference_count == 0) {
        ...
        afw_pool_free_memory(self->p, self,
            sizeof(afw_value_hexBinary_managed_slice_t), xctx);
    }
}
...
impl_afw_value_get_data_type(
    ...
    afw_xctx_t *xctx)
{
    return afw_data_type_hexBinary;
}
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_hexBinary;
    info->optimized_value = instance;
}


/* Get next hexBinary value from array of hexBinary. */
AFW_DEFINE(const afw_value_hexBinary_t *)
...
/* Get next hexBinary internal from array of hexBinary. */
AFW_DEFINE(const afw_memory_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_hexBinary_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a hexBinary value to array of hexBinary. */
AFW_DEFINE(void)
...
/* Add a hexBinary internal to array of hexBinary. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_hexBinary_create_managed(value, instance->p, xctx);
    }
    else {
        v = afw_value_hexBinary_create(value, instance->p, xctx);
    }
    afw_array_push_value(instance, v, xctx);
}

/* Remove a hexBinary value from array of hexBinary. */
AFW_DEFINE(void)
