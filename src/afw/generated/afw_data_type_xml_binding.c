...
/**
 * @file afw_data_type_xml_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_xml_direct

/* Define inf variables for data_type and is_evaluated_of_data_type. */
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    (const void *)&afw_data_type_xml_direct, \
    (const void *)&afw_data_type_xml_direct

/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for unmanaged xml values. For this one */
/* optional_release is NULL and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "xml"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_evaluated_xml_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for managed xml values. For this one */
/* optional_release releases value and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "managed_xml"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_xml_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for permanent xml values. For this one */
/* optional_release is NULL and get_reference returns instance asis. */
#define AFW_IMPLEMENTATION_ID "permanent_xml"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_xml_inf
#define impl_afw_value_optional_release NULL
...
{
    (const afw_value_t *)&impl_data_type_xml_id_value
};

static const afw_utf8_t
impl_data_type_path_xml =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/xml");

/* Data type xml object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_xml = {
    {
    ...
    (void *)&afw_data_type_xml_direct
};

/* Value for empty array of xml. */
AFW_DEFINE_INTERNAL_CONST_DATA(afw_array_wrapper_for_array_self_t)
impl_empty_array_of_xml;

/* Value for empty array of xml. */
AFW_DEFINE_INTERNAL_CONST_DATA(afw_value_array_t)
impl_value_empty_array_of_xml;

/* Data type xml instance. */
AFW_DEFINE_INTERNAL_CONST_DATA(afw_data_type_t)
afw_data_type_xml_direct = {
    &afw_data_type_xml_inf,
    (const afw_object_t *)&impl_data_type_object_xml,
    AFW_UTF8_LITERAL("xml"),
    AFW_UTF8_LITERAL("XML source"),
    AFW_UTF8_LITERAL("Data type 'xml' holds the source for an XML value."),
    AFW_UTF8_LITERAL("SourceParameter"),
    afw_data_type_number_xml,
    AFW_UTF8_LITERAL("1.3.6.1.4.1.1466.115.121.1.15{64512}"),
    AFW_UTF8_LITERAL("string"),
    AFW_UTF8_LITERAL(""),
    AFW_UTF8_LITERAL("afw_utf8_t"),
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_xml,
    (const afw_value_t *)&impl_value_empty_array_of_xml,
    &afw_value_evaluated_xml_inf,
    afw_compile_type_xml,
    false,
    true,
    false,
    true,
...
/* Data type struct for xml. */
AFW_DEFINE_CONST_DATA(afw_data_type_t *)
afw_data_type_xml =
    &afw_data_type_xml_direct;

/* Set property function for data type xml values. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_xml(internal, object->p, xctx);
    afw_object_set_property(object, property_name, v, xctx);
}

/* Typesafe cast of data type xml. */
AFW_DEFINE(const afw_utf8_t *)
afw_value_as_xml(const afw_value_t *value, afw_xctx_t *xctx)
{
    value = afw_value_evaluate(value, xctx->p, xctx);
    if (!AFW_VALUE_IS_DATA_TYPE(value, xml))
    {
        const afw_utf8_t *data_type_id;

        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'xml' but "
    ...
    return &(((const afw_value_xml_t *)value)->internal);
}

/* Allocate function for unmanaged data type xml values. */
AFW_DEFINE(afw_value_xml_t *)
afw_value_allocate_xml(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_xml_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_xml_t),
        xctx);
    result->inf = &afw_value_evaluated_xml_inf;
    return result;
}

/* Create function for unmanaged data type xml value. */
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_xml(p, xctx);
    if (internal) {
...
/* Allocate function for managed data type xml values. */
AFW_DEFINE(afw_value_xml_t *)
afw_value_allocate_managed_xml(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_xml_t),
        xctx);
    result->inf = &afw_value_managed_xml_inf;
    return result;
}

/* Create function for data type xml value. */
AFW_DEFINE(const afw_value_t *)
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_managed_xml(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return (const afw_value_t *)v;
...
/* Allocate function for permanent data type xml values. */
AFW_DEFINE(afw_value_xml_t *)
afw_value_allocate_permanent_xml(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_xml_t),
        xctx);
    result->inf = &afw_value_permanent_xml_inf;
    return result;
}
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_permanent_xml(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return (const afw_value_t *)v;
...
/* Convert data type xml string to afw_utf8_t *. */
AFW_DEFINE(void)
...
/*  Convert data type xml internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
...
/* Get property function for data type xml values. */
AFW_DEFINE(const afw_utf8_t *)
...
/* Get next property function for data type xml values. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_xml;
    info->optimized_value = instance;
}


/* Get next value from array of xml. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (data_type != afw_data_type_xml) {
        const afw_utf8_t *data_type_id;
...
/* Remove value from array of xml */
AFW_DEFINE(void)
