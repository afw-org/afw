    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_unmanaged_unevaluated(internal, object->p, xctx);
...
/* Allocate function for managed data type unevaluated value. */
AFW_DEFINE(const afw_value_t *)
afw_value_allocate_managed_unevaluated(
    const afw_value_t * **internal,
    afw_xctx_t *xctx)
{
    afw_value_unevaluated_managed_t *result;

    result = afw_xctx_malloc(sizeof(afw_value_unevaluated_managed_t), xctx);
...
afw_value_allocate_unmanaged_unevaluated(const afw_pool_t *p, afw_xctx_t *xctx)
{
