...
/**
 * @file afw_data_type_ipAddress_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_ipAddress_direct
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for managed ipAddress values. For this one */
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for managed_slice ipAddress values. */
/* optional_release / clone_or_reference update containing refcount. */
#define AFW_IMPLEMENTATION_ID "managed_slice_ipAddress"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_slice_ipAddress_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for permanent ipAddress values. For this one */
...
impl_data_type_ipAddress_id_value = {
...
{
    (const afw_value_t *)&impl_data_type_ipAddress_id_value
};
...
afw_data_type_ipAddress_direct = {
    ...
    (const afw_object_t *)&impl_data_type_object_ipAddress,
    AFW_UTF8_LITERAL("ipAddress"),
    AFW_UTF8_LITERAL("An IP address"),
    AFW_UTF8_LITERAL("Data type 'ipAddress' holds an Internet Protocol address (IP address)."),
    AFW_UTF8_LITERAL(""),
    afw_data_type_number_ipAddress,
...
/* Data type struct for ipAddress. */
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_unmanaged_ipAddress(internal, object->p, xctx);
...
afw_value_allocate_unmanaged_ipAddress(const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Create function for managed data type ipAddress value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_managed_ipAddress(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
    afw_value_ipAddress_managed_t *v;
    afw_size_t len;

    len = (internal) ? internal->len : 0;
    v = afw_xctx_calloc(
        sizeof(afw_value_ipAddress_managed_t) + len, xctx);
    v->inf = &afw_value_managed_ipAddress_inf;
    v->internal.len = len;
...
/* Create function for managed data type ipAddress slice value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_managed_ipAddress_slice(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_ipAddress_managed_t *containing;
    const afw_utf8_t *base;
    afw_value_ipAddress_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_ipAddress_inf) {
        const afw_value_ipAddress_managed_slice_t *slice =
            (const afw_value_ipAddress_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_ipAddress_inf) {
        containing =
        ...
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice ipAddress",
            xctx);
    }
    ...
    v = afw_xctx_calloc(sizeof(afw_value_ipAddress_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_ipAddress_inf;
    v->internal.s = base->s + offset;
    v->internal.len = len;
    v->containing_value = containing;
    ((afw_value_ipAddress_managed_t *)containing)->reference_count++;
    return &v->pub;
}
...
AFW_DEFINE(void)
afw_data_type_ipAddress_to_internal(afw_utf8_t *to_internal,
...
/*  Convert data type ipAddress internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
afw_data_type_ipAddress_to_utf8(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
...
impl_afw_value_managed_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_ipAddress_managed_t *self =
        (afw_value_ipAddress_managed_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_ipAddress_managed_slice_t *self =
        (afw_value_ipAddress_managed_slice_t *)instance;
    afw_value_ipAddress_managed_t *containing =
        (afw_value_ipAddress_managed_t *)self->containing_value;
...
impl_afw_value_managed_slice_get_reference(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_ipAddress_managed_slice_t *self =
        (const afw_value_ipAddress_managed_slice_t *)instance;
    afw_value_ipAddress_managed_t *containing =
        (afw_value_ipAddress_managed_t *)self->containing_value;
    afw_value_ipAddress_managed_slice_t *v;
...
impl_afw_value_get_data_type(
    ...
    afw_xctx_t *xctx)
{
    return afw_data_type_ipAddress;
}
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_ipAddress;
