...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice ipAddress: own RC; holds containing; */
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_ipAddress_create_managed(internal, object->p, xctx);
    }
    else {
...
afw_value_ipAddress_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    afw_xctx_t *xctx)
{
    ...
    p = p->managed_p;
    v = afw_pool_calloc(p, sizeof(afw_value_ipAddress_managed_slice_t), xctx);
...
/* Clone evaluated ipAddress managed in p->managed_p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_ipAddress_managed(
    const afw_value_t *value,
    const afw_pool_t *p,
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_ipAddress_create_managed(value, instance->p, xctx);
    }
    else {
