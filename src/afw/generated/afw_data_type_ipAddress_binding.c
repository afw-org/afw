...
/* Allocate function for managed data type ipAddress value. */
...
afw_value_allocate_managed_ipAddress(
    ...
    afw_xctx_t *xctx)
{
    afw_value_ipAddress_managed_t *result;

    result = afw_xctx_malloc(sizeof(afw_value_ipAddress_managed_t) + len, xctx);
    result->inf = &afw_value_managed_ipAddress_inf;
    result->internal.len = len;
    result->internal.s = (const afw_utf8_octet_t *)result + sizeof(afw_value_ipAddress_managed_t);
