...
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_ipAddress_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for referenced ipAddress values. For this one */
/* optional_release releases value and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "referenced_ipAddress"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_referenced_ipAddress_inf
...
afw_value_allocate_ipAddress(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_ipAddress_t),
        xctx);
    result->inf = &afw_value_ipAddress_inf;
    return result;
}

/* Create function for referenced data type ipAddress value. */
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_pool_calloc(p, sizeof(afw_value_ipAddress_t),
        xctx);
    v->inf = &afw_value_referenced_ipAddress_inf;
    if (internal) {
...
/* Create function for referenced data type ipAddress slice value. */
AFW_DEFINE(const afw_value_t *)
