 ...
 * `const afw_value_ipAddress_t *` (e.g. to read `.internal`).
 * Same as `AFW_VALUE_IS_DATA_TYPE(A_VALUE, ipAddress)`.
 * Not the same as "will evaluate to ipAddress" — see
...
afw_value_allocate_unmanaged_ipAddress(
    const afw_pool_t *p,
    afw_xctx_t *xctx);
...
afw_value_create_managed_ipAddress(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx);
 ...
 */
AFW_DECLARE(const afw_value_t *)
afw_value_create_managed_ipAddress_slice(
 ...
 */
AFW_DECLARE(const afw_value_t *)
afw_value_create_unmanaged_ipAddress(const afw_utf8_t * internal,
