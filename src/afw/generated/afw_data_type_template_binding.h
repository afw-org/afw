...
#ifndef __AFW_DATA_TYPE_TEMPLATE_BINDING_H__
#define __AFW_DATA_TYPE_TEMPLATE_BINDING_H__
...
/**
 * @defgroup afw_c_api_data_type_template template
...
/** @brief Quoted data type id for template */
#define AFW_DATA_TYPE_ID_Q_template "template"

/** @brief Unquoted data type id for template */
#define AFW_DATA_TYPE_ID_U_template template

/** @brief String for data type id template */
#define AFW_DATA_TYPE_ID_S_template afw_s_template

/** @brief Quoted c type for data type id template */
#define AFW_DATA_TYPE_CTYPE_Q_template "afw_utf8_t"

/** @brief Adaptive string value for "template". */
AFW_DECLARE_CONST_DATA(afw_value_t *)
afw_data_type_template_id_value;

/** @brief Data type struct for template. */
AFW_DECLARE_CONST_DATA(afw_data_type_t *)
afw_data_type_template;

/**
 * @brief Unmanaged evaluated value inf for data type template.
 *
...
/**
 * @brief Managed evaluated value inf for data type template.
 *
 ...
 */
AFW_DECLARE_CONST_DATA(afw_value_inf_t)
afw_value_managed_template_inf;
...
/**
 * @brief Permanent (life of afw environment) value inf for data type template.
 *
 ...
 */
AFW_DECLARE_CONST_DATA(afw_value_inf_t)
afw_value_permanent_template_inf;

/**
 * @brief Macro to determine if data type is template.
 * @param _A_DATA_TYPE to test.
 * @return boolean result.
 */
#define afw_data_type_is_template(_A_DATA_TYPE) \
( \
    (_A_DATA_TYPE) && \
    (_A_DATA_TYPE) == afw_data_type_template \
)

/**
 * @brief True if _A_VALUE is an evaluated template value.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * For evaluated values only. When true, it is safe to cast _A_VALUE to
 * `const afw_value_template_t *`.
 * If you want to know if the value will be template when fully
 * evaluated (not necessarily cast-safe yet), use
 * `AFW_VALUE_EVALUATES_TO_DATA_TYPE(_A_VALUE, template, xctx)` instead.
 */
#define afw_value_is_template(_A_VALUE) \
( \
    (_A_VALUE) && \
    (_A_VALUE)->inf->is_evaluated_of_data_type == afw_data_type_template\
)

/**
 * @brief True if _A_VALUE is an evaluated array of template.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 ...
 */
#define afw_value_is_array_of_template(_A_VALUE) \
( \
    afw_value_is_array(_A_VALUE) && \
    afw_array_get_data_type( \
        ((const afw_value_array_t *)(_A_VALUE))->internal, \
        xctx) == afw_data_type_template \
)

/**
 * @brief Convert data type template string to afw_utf8_t *.
 ...
 */
AFW_DECLARE(void)
afw_data_type_template_to_internal(
...
/**
 * @brief Convert data type template internal representation to utf-8.
...
afw_data_type_template_to_utf8(
    const afw_utf8_t * internal,
    const afw_pool_t *p,
    afw_xctx_t *xctx);

/** @brief struct for data type template values. */
struct afw_value_template_s {
...
/** @brief struct for managed data type template values.
 *
 * This is the same as afw_value_template_s with the addition of a
 * reference count. This is intended for internal use only.
 */
struct afw_value_template_managed_s {
    /** @brief  Value inf union with afw_value_t pub to reduce casting needed. */
    union {
        const afw_value_inf_t *inf;
        afw_value_t pub;
...
/** @brief struct for managed slice data type template values.
 *
 * A view into a containing managed value. Holds that value.
 */
struct afw_value_template_managed_slice_s {
    /** @brief  Value inf union with afw_value_t pub to reduce casting needed. */
    union {
        const afw_value_inf_t *inf;
        afw_value_t pub;
...
/**
 * @brief Allocate function for data type template value.
 * @param p to use for returned value.
 * @param xctx of caller.
 * @return Allocated afw_value_template_t with unmanaged inf set.
 *
 * Unmanaged: lifetime is pool p; no value refcount.
 * Caller fills internal after allocate.
 */
AFW_DECLARE(afw_value_template_t *)
afw_value_template_allocate(
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_allocate_unmanaged_template afw_value_template_allocate

/**
 * @brief Create function for managed data type template value.
 * @param internal.
...
afw_value_template_create_managed(
    ...
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_template afw_value_template_create_managed

/**
 * @brief Deep clone an evaluated template value unmanaged into p.
 * @param value evaluated template.
 * @param p dest pool.
 * @param xctx of caller.
...
/**
 * @brief Clone an evaluated template value managed in p->managed_p.
 * @param value evaluated template.
 * @param p dest pool (uses p->managed_p).
 * @param xctx of caller.
...
/**
 * @brief Create a managed slice of a managed data type template value.
...
/**
 * @brief Create function for unmanaged data type template value.
 * @param internal.
 * @param p to use for returned value.
 * @param xctx of caller.
 * @return Created const afw_value_t *.
 ...
 */
#define afw_array_of_template_get_next( \
...
/**
 * @brief Public data type template struct instance.
 *
 * Prefer afw_data_type_template when a pointer is enough and you are not
...
/**
 * @brief Public data type template inf.
 *
...
#endif /* __AFW_DATA_TYPE_TEMPLATE_BINDING_H__ */
