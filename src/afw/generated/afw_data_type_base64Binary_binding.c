...
/* This is the inf for base64Binary values. For this one */
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for managed_slice base64Binary values. */
...
/* Create function for managed data type base64Binary slice value. */
AFW_DEFINE(const afw_value_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_value_base64Binary_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_base64Binary_inf) {
        const afw_value_base64Binary_managed_slice_t *slice =
            (const afw_value_base64Binary_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_base64Binary_inf) {
        containing =
        ...
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice base64Binary",
            xctx);
    }
    ...
    v = afw_xctx_calloc(sizeof(afw_value_base64Binary_managed_slice_t), xctx);
    ...
    ((afw_value_base64Binary_managed_t *)containing)->reference_count++;
    return &v->pub;
}
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_base64Binary_managed_slice_t *self =
        (afw_value_base64Binary_managed_slice_t *)instance;
    afw_value_base64Binary_managed_t *containing =
        (afw_value_base64Binary_managed_t *)self->containing_value;
...
impl_afw_value_managed_slice_get_reference(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_base64Binary_managed_slice_t *self =
        (const afw_value_base64Binary_managed_slice_t *)instance;
    afw_value_base64Binary_managed_t *containing =
        (afw_value_base64Binary_managed_t *)self->containing_value;
    afw_value_base64Binary_managed_slice_t *v;
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_base64Binary;
