...
/* This is the inf for unmanaged dateTime values. For this one */
/* optional_release is NULL and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "dateTime"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_evaluated_dateTime_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for managed dateTime values. For this one */
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for permanent dateTime values. For this one */
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_dateTime_unmanaged(internal, object->p, xctx);
    afw_object_set_property(object, property_name, v, xctx);
}

/* Typesafe cast of data type dateTime. */
...
afw_value_as_dateTime(const afw_value_t *value, afw_xctx_t *xctx)
{
    ...
    return &(((const afw_value_dateTime_t *)value)->internal);
}

/* Allocate function for unmanaged data type dateTime values. */
...
afw_value_allocate_dateTime(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_dateTime_t),
        xctx);
    result->inf = &afw_value_evaluated_dateTime_inf;
    return result;
}
...
afw_value_allocate_managed_dateTime(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_dateTime_t),
        xctx);
    result->inf = &afw_value_managed_dateTime_inf;
    return result;
}
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_managed_dateTime(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_dateTime_t));
    }
    return &v->pub;
...
afw_value_allocate_permanent_dateTime(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_dateTime_t),
        xctx);
    result->inf = &afw_value_permanent_dateTime_inf;
    return result;
}
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_permanent_dateTime(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_dateTime_t));
    }
    return &v->pub;
...
/* Get property function for data type dateTime values. */
AFW_DEFINE(const afw_dateTime_t *)
...
/* Get next property function for data type dateTime values. */
AFW_DEFINE(const afw_dateTime_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (data_type != afw_data_type_dateTime) {
        const afw_utf8_t *data_type_id;
