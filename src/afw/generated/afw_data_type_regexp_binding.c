...
/* This is the inf for regexp values. For this one */
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for managed regexp values. For this one */
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for managed_slice regexp values. */
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for permanent regexp values. For this one */
...
/* Value for empty array of regexp. */
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_unmanaged_regexp(internal, object->p, xctx);
...
afw_value_allocate_unmanaged_regexp(const afw_pool_t *p, afw_xctx_t *xctx)
{
...
afw_value_create_managed_regexp(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
...
AFW_DEFINE(const afw_value_t *)
afw_value_create_managed_regexp_slice(
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_xctx_calloc(sizeof(afw_value_regexp_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_regexp_inf;
    v->internal.s = base->s + offset;
    v->internal.len = len;
    v->containing_value = containing;
    ((afw_value_regexp_managed_t *)containing)->reference_count++;
    return &v->pub;
}
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_value_regexp_managed_t *containing =
        (afw_value_regexp_managed_t *)self->containing_value;
...
impl_afw_value_managed_slice_get_reference(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_regexp_managed_slice_t *self =
        (const afw_value_regexp_managed_slice_t *)instance;
    afw_value_regexp_managed_t *containing =
        (afw_value_regexp_managed_t *)self->containing_value;
    afw_value_regexp_managed_slice_t *v;
