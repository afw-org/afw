...
/**
 * @file afw_data_type_regexp_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_regexp_direct
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    (const void *)&afw_data_type_regexp_direct, \
    (const void *)&afw_data_type_regexp_direct, \
    false
...
/* unmanaged regexp: get_reference/release throw; */
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_regexp_direct, \
    true
/* managed regexp: optional_release drops RC; */
/* scalar last-release free_memorys via xctx->p. */
/* get_reference / get_assignable_value bump. */
#define AFW_IMPLEMENTATION_ID "managed_regexp"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_regexp_inf
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_regexp_direct, \
    false
/* permanent regexp: optional_release NULL; */
/* get_reference / get_assignable_value as-is. */
#define AFW_IMPLEMENTATION_ID "permanent_regexp"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_regexp_inf
...
impl_data_type_regexp_id_value = {
...
{
    (const afw_value_t *)&impl_data_type_regexp_id_value
};

static const afw_utf8_t
impl_data_type_path_regexp =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/regexp");

static const afw_value_object_t
impl_data_type_object_regexp__value;

/* Data type regexp object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_regexp = {
    {
    ...
    (void *)&afw_data_type_regexp_direct
};

static const afw_value_object_t
impl_data_type_object_regexp__value = {
    {&afw_value_permanent_object_inf},
    (const afw_object_t *)&impl_data_type_object_regexp
};

/* Permanent empty array of regexp. */
const afw_array_from_values_self_t
impl_empty_array_of_regexp;

/* Permanent empty array value of regexp. */
const afw_value_array_t
impl_value_empty_array_of_regexp;

/* Data type regexp instance. */
AFW_DEFINE_CONST_DATA(afw_data_type_t)
afw_data_type_regexp_direct = {
    &afw_data_type_regexp_inf,
    (const afw_object_t *)&impl_data_type_object_regexp,
    AFW_UTF8_LITERAL("regexp"),
    AFW_UTF8_LITERAL("An regular expression"),
    AFW_UTF8_LITERAL("Data type 'regexp' holds the source for a regular expression conforming to XML Schemas Part 2 Datatype Appendix F."),
    AFW_UTF8_LITERAL("SourceParameter"),
    afw_data_type_number_regexp,
    AFW_UTF8_LITERAL("1.3.6.1.4.1.1466.115.121.1.15{64512}"),
    AFW_UTF8_LITERAL("string"),
    AFW_UTF8_LITERAL("regex"),
    AFW_UTF8_LITERAL("afw_utf8_t"),
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_regexp,
    (const afw_value_t *)&impl_value_empty_array_of_regexp,
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_regexp_create_managed(internal, xctx);
    }
    else {
        v = afw_value_regexp_create(internal, object->p, xctx);
    }
    afw_object_set_property(object, property_name, v, xctx);
}

/* Typesafe cast to evaluated regexp value. */
AFW_DEFINE(const afw_value_regexp_t *)
afw_value_as_regexp(const afw_value_t *value, afw_xctx_t *xctx)
{
    value = afw_value_evaluate(value, xctx->p, xctx);
    if (!AFW_VALUE_IS_DATA_TYPE(value, regexp))
    {
        const afw_utf8_t *data_type_id;

        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'regexp' but "
    ...
    return (const afw_value_regexp_t *)value;
}

/* Typesafe peel of data type regexp internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_value_as_regexp_internal(const afw_value_t *value, afw_xctx_t *xctx)
{
    return &afw_value_as_regexp(value, xctx)->internal;
}

/* Allocate function for data type regexp values. */
AFW_DEFINE(afw_value_regexp_t *)
afw_value_regexp_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_regexp_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_regexp_t),
        xctx);
    result->inf = &afw_value_unmanaged_regexp_inf;
    return result;
}

/* Create function for managed data type regexp value. */
AFW_DEFINE(const afw_value_t *)
afw_value_regexp_create_managed(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
    afw_value_regexp_managed_t *v;
    afw_size_t len;

    len = (internal) ? internal->len : 0;
    v = afw_pool_calloc(xctx->p->managed_p,
        sizeof(afw_value_regexp_managed_t) + len, xctx);
    v->inf = &afw_value_managed_regexp_inf;
    v->internal.len = len;
    v->internal.s = (const afw_utf8_octet_t *)v +
        sizeof(afw_value_regexp_managed_t);
...
/* Create function for managed data type regexp slice value. */
AFW_DEFINE(const afw_value_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_value_regexp_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_regexp_inf) {
        const afw_value_regexp_managed_slice_t *slice =
            (const afw_value_regexp_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_regexp_inf) {
        containing =
        ...
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice regexp",
            xctx);
    }
    ...
    v = afw_pool_calloc(xctx->p->managed_p, sizeof(afw_value_regexp_managed_slice_t), xctx);
...
/* Create function for data type regexp value. */
AFW_DEFINE(const afw_value_t *)
afw_value_regexp_create(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_regexp_t *v;

    v = afw_pool_calloc(p, sizeof(afw_value_regexp_t),
        xctx);
    v->inf = &afw_value_unmanaged_regexp_inf;
    if (internal) {
...
/* Deep clone evaluated regexp unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_regexp_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_regexp_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_regexp, p, xctx);
    afw_data_type_clone_internal(afw_data_type_regexp,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_regexp_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated regexp managed in xctx->p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_regexp_managed(
    const afw_value_t *value,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_regexp_create_managed(
        &((const afw_value_regexp_t *)value)->internal,
        xctx);
}

/* Convert data type regexp string to afw_utf8_t *. */
AFW_DEFINE(void)
afw_data_type_regexp_to_internal(afw_utf8_t *to_internal,
...
/*  Convert data type regexp internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
afw_data_type_regexp_to_utf8(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Get property as regexp internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_property_as_regexp_internal_source(
    const afw_object_t *object,
    const afw_value_t *property_name,
    const afw_utf8_z_t *source_z,
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_regexp_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as regexp value. */
AFW_DEFINE(const afw_value_regexp_t *)
...
/* Get next property as regexp internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_next_property_as_regexp_internal_source(
    const afw_object_t *object,
    const afw_iterator_old_t * *iterator,
    const afw_value_t * *property_name,
    const afw_utf8_z_t *source_z,
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_regexp_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
impl_afw_value_managed_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_regexp_managed_t *self =
        (afw_value_regexp_managed_t *)instance;
    ...
    if (self->reference_count == 0) {
        afw_pool_free_memory(xctx->p->managed_p, self,
            sizeof(afw_value_regexp_managed_t) + self->internal.len, xctx);
    }
}
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_regexp_t *self =
        (const afw_value_regexp_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_regexp_managed_slice_t *self =
        (afw_value_regexp_managed_slice_t *)instance;
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_regexp;
    info->optimized_value = instance;
}


/* Get next regexp value from array of regexp. */
AFW_DEFINE(const afw_value_regexp_t *)
...
/* Get next regexp internal from array of regexp. */
AFW_DEFINE(const afw_utf8_t *)
afw_array_of_regexp_get_next_internal_source(
    const afw_array_t *instance,
    const afw_iterator_old_t * *iterator,
    const afw_utf8_z_t *source_z,
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_regexp_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a regexp value to array of regexp. */
AFW_DEFINE(void)
...
/* Add a regexp internal to array of regexp. */
AFW_DEFINE(void)
afw_array_of_regexp_add_internal(
    const afw_array_t *instance,
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_regexp_create_managed(value, xctx);
    }
    else {
        v = afw_value_regexp_create(value, instance->p, xctx);
    }
    afw_array_push_value(instance, v, xctx);
}

/* Remove a regexp value from array of regexp. */
AFW_DEFINE(void)
afw_array_of_regexp_remove(
    ...
    afw_xctx_t *xctx)
{
    afw_array_of_regexp_remove_internal(instance, &value->internal, xctx);
}

/* Remove a regexp internal from array of regexp. */
AFW_DEFINE(void)
afw_array_of_regexp_remove_internal(
    const afw_array_t *instance,
    const afw_utf8_t *value,
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_regexp_create(value, xctx->p, xctx);
