...
/* This is the inf for unmanaged regexp values. For this one */
/* optional_release is NULL and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "regexp"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_evaluated_regexp_inf
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_regexp_unmanaged(internal, object->p, xctx);
...
/* Allocate function for unmanaged data type regexp values. */
...
afw_value_allocate_regexp(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_regexp_t),
        xctx);
    result->inf = &afw_value_evaluated_regexp_inf;
    return result;
}

/* Create function for unmanaged data type regexp value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_regexp_unmanaged(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_regexp_t *v;

    v = afw_value_allocate_regexp(p, xctx);
    if (internal) {
...
/* Allocate function for managed data type regexp values. */
AFW_DEFINE(afw_value_regexp_t *)
afw_value_allocate_managed_regexp(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_regexp_t *result;
...
AFW_DEFINE(const afw_value_t *)
afw_value_create_managed_regexp(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_regexp_t *v;

    v = afw_value_allocate_managed_regexp(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return &v->pub;
...
/* Allocate function for permanent data type regexp values. */
AFW_DEFINE(afw_value_regexp_t *)
afw_value_allocate_permanent_regexp(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_regexp_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_regexp_t),
        xctx);
    result->inf = &afw_value_permanent_regexp_inf;
    return result;
}

/* Create function for data type regexp value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_permanent_regexp(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_regexp_t *v;

    v = afw_value_allocate_permanent_regexp(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return &v->pub;
