...
##
# @file xpathExpression.py
# @ingroup afw_bindings_python
# @brief This file contains the Python bindings for the xpathExpression category.
#
...
def compile_xpathExpression(session, source, listing=None):
    ...
    Args:
        ...
        listing (object): If specified, a human compiler listing is produced
            ...
            stringify() for pure JSON of evaluated data. This parameter can be
            an integer between 0 and 10 or a string that is used for
            indentation. If 0 is specified, no whitespace is added to the
            resulting string. If 1 through 10 is specified, that number of
            spaces is used.
...
def xpathExpression(session, value):
    ...
    value (does not evaluate). Use polymorphic compile when a compiled form is
    required.
