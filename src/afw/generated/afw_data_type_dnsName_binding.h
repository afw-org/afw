 ...
 * `const afw_value_dnsName_t *` (e.g. to read `.internal`).
 * Same as `AFW_VALUE_IS_DATA_TYPE(A_VALUE, dnsName)`.
 * Not the same as "will evaluate to dnsName" — see
...
afw_value_allocate_unmanaged_dnsName(
    const afw_pool_t *p,
    afw_xctx_t *xctx);
...
afw_value_create_managed_dnsName(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx);
 ...
 */
AFW_DECLARE(const afw_value_t *)
afw_value_create_managed_dnsName_slice(
 ...
 */
AFW_DECLARE(const afw_value_t *)
afw_value_create_unmanaged_dnsName(const afw_utf8_t * internal,
...
/**
 * @brief extern for data type dnsName struct.
 *
 * This should only be managed in the linkage unit the extern is
 * defined in.  Use afw_data_type_dnsName when not referencing in
 * a static.
 */
...
/**
 * @brief extern for data type dnsName inf.
 *
 * This should only be managed in the linkage unit the extern is
 * defined in.
 *
