...
/* unmanaged function: get_reference/release throw; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed function: optional_release drops RC; */
...
impl_data_type_object_function__value;
...
impl_data_type_object_function = {
    {
        ...
        NULL,
        (const afw_value_t *)&impl_data_type_object_function__value,
        {
            NULL,
            NULL,
            &afw_self_s_function,
...
/* Set property from function internal via setter. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_object_setter_set_property_internal(setter,
        property_name, afw_data_type_function,
        &internal, xctx);
}
...
afw_value_function_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_function_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_function_t),
        xctx);
    result->inf = &afw_value_unmanaged_function_inf;
    return result;
}
...
afw_value_function_create_managed(
    ...
    afw_xctx_t *xctx)
{
    afw_value_function_managed_t *v;
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_pool_calloc(p, sizeof(afw_value_function_t),
        xctx);
    v->inf = &afw_value_unmanaged_function_inf;
    v->internal = internal;
    return &v->pub;
}

/* Deep clone evaluated function unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_function_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_function_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_function, p, xctx);
    afw_data_type_clone_internal(afw_data_type_function,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_function_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated function managed in xctx->p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_function_managed(
    const afw_value_t *value,
    afw_xctx_t *xctx)
...
/* Get property function for data type function internal. */
AFW_DEFINE(const afw_value_t *)
...
/* Get next property function for data type function internal. */
AFW_DEFINE(const afw_value_t *)
