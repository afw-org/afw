...
/**
 * @brief Managed slice value inf for data type objectId.
 *
 ...
 */
AFW_DECLARE_CONST_DATA(afw_value_inf_t)
afw_value_managed_slice_objectId_inf;

/**
 * @brief Permanent (life of afw environment) value inf for data type objectId.
 *
 ...
 */
#define afw_data_type_is_objectId(_A_DATA_TYPE) \
( \
    (_A_DATA_TYPE) && \
    (_A_DATA_TYPE) == afw_data_type_objectId \
)

/**
 * @brief True if _A_VALUE is an evaluated objectId value.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * For evaluated values only. When true, it is safe to cast _A_VALUE to
 * `const afw_value_objectId_t *`.
 * If you want to know if the value will be objectId when fully
 * evaluated (not necessarily cast-safe yet), use
 * `AFW_VALUE_EVALUATES_TO_DATA_TYPE(_A_VALUE, objectId, xctx)` instead.
 */
#define afw_value_is_objectId(_A_VALUE) \
( \
    (_A_VALUE) && \
    (_A_VALUE)->inf->is_evaluated_of_data_type == afw_data_type_objectId\
)

/**
 * @brief True if _A_VALUE is an evaluated array of objectId.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * When true, _A_VALUE is an evaluated array (`const afw_value_array_t *`)
 * whose element data type is objectId.
 */
#define afw_value_is_array_of_objectId(_A_VALUE) \
( \
 ...
 */
AFW_DECLARE(void)
afw_data_type_objectId_to_internal(
...
afw_data_type_objectId_to_utf8(
...
/** @brief struct for managed data type objectId values.
 *
 * This is the same as afw_value_objectId_s with the addition of a
 * reference count. This is intended for internal use only.
 */
struct afw_value_objectId_managed_s {
    /** @brief  Value inf union with afw_value_t pub to reduce casting needed. */
    union {
        const afw_value_inf_t *inf;
        afw_value_t pub;
...
/** @brief struct for managed slice data type objectId values.
 *
 * A view into a containing managed value. Holds that value.
 */
struct afw_value_objectId_managed_slice_s {
    /** @brief  Value inf union with afw_value_t pub to reduce casting needed. */
    union {
        const afw_value_inf_t *inf;
        afw_value_t pub;
...
/**
 * @brief Typesafe cast to evaluated objectId value.
 * @param value (const afw_value_t *). Evaluated if needed.
 * @return (const afw_value_objectId_t *)
 *
 * Throws if missing or wrong type. Use ->internal for the C
 * payload, or afw_value_as_objectId_internal().
 */
AFW_DECLARE(const afw_value_objectId_t *)
afw_value_as_objectId(
    const afw_value_t *value,
    afw_xctx_t *xctx);

/**
 * @brief Typesafe peel of data type objectId internal.
 * @param value (const afw_value_t *).
 * @return (const afw_utf8_t *)
 */
AFW_DECLARE(const afw_utf8_t *)
afw_value_as_objectId_internal(
    const afw_value_t *value,
    afw_xctx_t *xctx);

/**
 * @brief Allocate function for data type objectId value.
 * @param p to use for returned value.
 * @param xctx of caller.
 * @return Allocated afw_value_objectId_t with unmanaged inf set.
 *
 * Unmanaged: lifetime is pool p; no value refcount.
 * Caller fills internal after allocate.
 */
AFW_DECLARE(afw_value_objectId_t *)
afw_value_objectId_allocate(
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_allocate_unmanaged_objectId afw_value_objectId_allocate
...
afw_value_objectId_create_managed(
    ...
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_objectId afw_value_objectId_create_managed

/**
 * @brief Deep clone an evaluated objectId value unmanaged into p.
 * @param value evaluated objectId.
 * @param p dest pool.
 * @param xctx of caller.
...
/**
 * @brief Clone an evaluated objectId value managed in p->managed_p.
 * @param value evaluated objectId.
 * @param p dest pool (uses p->managed_p).
 * @param xctx of caller.
...
/**
 * @brief Create a managed slice of a managed data type objectId value.
...
afw_value_objectId_create_managed_slice(
    ...
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_objectId_slice afw_value_objectId_create_managed_slice
...
#define afw_value_create_unmanaged_objectId afw_value_objectId_create

/**
 * @brief Get property as objectId value.
 * @param object of property to get.
 * @param property_name of property to get.
 * @param xctx of caller.
 * @return (const afw_value_objectId_t *) or NULL if missing.
 *
 * Does not evaluate. Throws if present but not objectId.
 */
...
/**
 * @brief Get property as objectId internal.
 * @param object of property to get.
 * @param property_name of property to get.
 * @param xctx of caller.
 * @return const afw_utf8_t *.
 */
#define afw_object_get_property_as_objectId_internal( \
    _object, _property_name, _xctx) \
...
/**
 * @brief Get next property as objectId internal.
 * @param object of property to get.
 * @param iterator pointer. Set to NULL before first call.
 * @param property_name is place to return pointer to property name.
 * @param xctx of caller.
 * @return const afw_utf8_t *.
 */
#define afw_object_get_next_property_as_objectId_internal( \
    _object, _iterator, _property_name, _xctx) \
...
/**
 * @brief Set property as objectId value.
 * @param object of property to set.
 * @param property_name of property to set.
 * @param value to set.
 * @param xctx of caller.
 *
 * Compile-time type check for const afw_value_objectId_t *.
 */
AFW_DECLARE(void)
...
/**
 * @brief Set property as objectId internal.
 * @param object of property to set.
 * @param property_name of property to set.
...
/**
 * @brief Get next objectId value from array of objectId.
 * @param instance of array.
 * @param iterator.
 * @param xctx of caller.
 * @return (const afw_value_objectId_t *) or NULL.
 *
...
/**
 * @brief Get next objectId internal from array of objectId.
 * @param instance of array.
 * @param iterator.
 * @param xctx of caller.
 * @return (const afw_utf8_t *) or NULL.
 */
#define afw_array_of_objectId_get_next_internal( \
    _array, _iterator, _xctx) \
...
/**
 * @brief Add a objectId value to array of objectId.
 * @param instance of array.
 * @param value to add.
 * @param xctx of caller.
 */
...
/**
 * @brief Add a objectId internal to array of objectId.
 * @param instance of array.
 * @param value to add.
 * @param xctx of caller.
 */
...
/**
 * @brief Remove a objectId value from array of objectId.
 * @param instance of array.
...
/**
 * @brief Remove a objectId internal from array of objectId.
 * @param instance of array.
 * @param value to remove.
 * @param xctx of caller.
 */
