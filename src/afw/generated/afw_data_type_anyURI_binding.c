...
/**
 * @file afw_data_type_anyURI_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_anyURI_direct
...
/* unmanaged anyURI: get_reference/release throw; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed anyURI: optional_release drops RC; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice anyURI: own RC; holds containing; */
...
impl_data_type_anyURI_id_value = {
    {&afw_value_permanent_string_inf},
    AFW_UTF8_LITERAL("anyURI")
};

AFW_DEFINE_CONST_DATA(afw_value_t *)
afw_data_type_anyURI_id_value =
{
    (const afw_value_t *)&impl_data_type_anyURI_id_value
};

static const afw_utf8_t
impl_data_type_path_anyURI =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/anyURI");

static const afw_value_object_t
impl_data_type_object_anyURI__value;

/* Data type anyURI object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_anyURI = {
    {
    ...
    (void *)&afw_data_type_anyURI_direct
};
...
afw_data_type_anyURI_direct = {
    ...
    (const afw_object_t *)&impl_data_type_object_anyURI,
    AFW_UTF8_LITERAL("anyURI"),
    AFW_UTF8_LITERAL("A Uniform Resource Identifier Reference (URI)"),
    AFW_UTF8_LITERAL("Data type 'anyURI' holds a Uniform Resource Identifier Reference (URI). The URI can be absolute or relative, and may have an optional fragment identifier."),
    AFW_UTF8_LITERAL(""),
    afw_data_type_number_anyURI,
    AFW_UTF8_LITERAL("1.3.6.1.4.1.1466.115.121.1.15{64512}"),
    AFW_UTF8_LITERAL("string"),
    AFW_UTF8_LITERAL("uri-reference"),
    ...
    afw_value_clone_anyURI_unmanaged,
    afw_value_clone_anyURI_managed,
...
/* Set property from anyURI value. */
AFW_DEFINE(void)
afw_object_set_property_as_anyURI(
    const afw_object_t *object,
    const afw_value_t *property_name,
    const afw_value_anyURI_t *value,
    afw_xctx_t *xctx)
{
...
/* Set property from anyURI internal. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_anyURI_create_managed(internal, xctx);
    }
    else {
        v = afw_value_anyURI_create(internal, object->p, xctx);
    }
    afw_object_set_property(object, property_name, v, xctx);
}

/* Typesafe cast to evaluated anyURI value. */
AFW_DEFINE(const afw_value_anyURI_t *)
afw_value_as_anyURI(const afw_value_t *value, afw_xctx_t *xctx)
{
    value = afw_value_evaluate(value, xctx->p, xctx);
    if (!AFW_VALUE_IS_DATA_TYPE(value, anyURI))
    {
        const afw_utf8_t *data_type_id;

        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'anyURI' but "
    ...
    return (const afw_value_anyURI_t *)value;
}

/* Typesafe peel of data type anyURI internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_value_as_anyURI_internal(const afw_value_t *value, afw_xctx_t *xctx)
{
    return &afw_value_as_anyURI(value, xctx)->internal;
}

/* Allocate function for data type anyURI values. */
AFW_DEFINE(afw_value_anyURI_t *)
afw_value_anyURI_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_anyURI_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_anyURI_t),
        xctx);
    result->inf = &afw_value_unmanaged_anyURI_inf;
    return result;
}

/* Create function for managed data type anyURI value. */
AFW_DEFINE(const afw_value_t *)
afw_value_anyURI_create_managed(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
    afw_value_anyURI_managed_t *v;
    afw_size_t len;

    len = (internal) ? internal->len : 0;
    v = afw_pool_calloc(xctx->p,
        sizeof(afw_value_anyURI_managed_t) + len, xctx);
    v->inf = &afw_value_managed_anyURI_inf;
    v->internal.len = len;
    v->internal.s = (const afw_utf8_octet_t *)v +
        sizeof(afw_value_anyURI_managed_t);
...
/* Create function for managed data type anyURI slice value. */
AFW_DEFINE(const afw_value_t *)
afw_value_anyURI_create_managed_slice(
    const afw_value_t *containing_value,
    afw_size_t offset,
    afw_size_t len,
    afw_xctx_t *xctx)
{
    const afw_value_anyURI_managed_t *containing;
    const afw_utf8_t *base;
    afw_value_anyURI_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_anyURI_inf) {
        const afw_value_anyURI_managed_slice_t *slice =
            (const afw_value_anyURI_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_anyURI_inf) {
        containing =
            (const afw_value_anyURI_managed_t *)containing_value;
        base = &containing->internal;
    }
    else {
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice anyURI",
            xctx);
    }
    ...
    v = afw_pool_calloc(xctx->p, sizeof(afw_value_anyURI_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_anyURI_inf;
...
/* Create function for data type anyURI value. */
AFW_DEFINE(const afw_value_t *)
afw_value_anyURI_create(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_anyURI_t *v;

    v = afw_pool_calloc(p, sizeof(afw_value_anyURI_t),
        xctx);
    v->inf = &afw_value_unmanaged_anyURI_inf;
    if (internal) {
...
/* Deep clone evaluated anyURI unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_anyURI_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_anyURI_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_anyURI, p, xctx);
    afw_data_type_clone_internal(afw_data_type_anyURI,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_anyURI_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated anyURI managed in xctx->p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_anyURI_managed(
    const afw_value_t *value,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_anyURI_create_managed(
        &((const afw_value_anyURI_t *)value)->internal,
        xctx);
}

/* Convert data type anyURI string to afw_utf8_t *. */
AFW_DEFINE(void)
afw_data_type_anyURI_to_internal(afw_utf8_t *to_internal,
...
/*  Convert data type anyURI internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
afw_data_type_anyURI_to_utf8(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Get property as anyURI value. */
AFW_DEFINE(const afw_value_anyURI_t *)
afw_object_get_property_as_anyURI_source(
    const afw_object_t *object,
    const afw_value_t *property_name,
...
/* Get property function for data type anyURI internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_property_as_anyURI_internal_source(
    const afw_object_t *object,
    const afw_value_t *property_name,
    const afw_utf8_z_t *source_z,
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_anyURI_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as anyURI value. */
AFW_DEFINE(const afw_value_anyURI_t *)
afw_object_get_next_property_as_anyURI_source(
    const afw_object_t *object,
...
/* Get next property function for data type anyURI internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_next_property_as_anyURI_internal_source(
    const afw_object_t *object,
    const afw_iterator_old_t * *iterator,
    const afw_value_t * *property_name,
    const afw_utf8_z_t *source_z,
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_anyURI_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
    ...
    afw_xctx_t *xctx)
{
    ...
    if (self->reference_count == 0) {
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_anyURI_managed_t) + self->internal.len, xctx);
    }
}
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_anyURI_t *self =
        (const afw_value_anyURI_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_anyURI_managed_slice_t *self =
        (afw_value_anyURI_managed_slice_t *)instance;
    ...
    if (self->reference_count == 0) {
        ...
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_anyURI_managed_slice_t), xctx);
    }
}
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_anyURI;
    info->optimized_value = instance;
}


/* Get next anyURI value from array of anyURI. */
AFW_DEFINE(const afw_value_anyURI_t *)
afw_array_of_anyURI_get_next_source(
...
/* Get next anyURI internal from array of anyURI. */
AFW_DEFINE(const afw_utf8_t *)
afw_array_of_anyURI_get_next_internal_source(
    const afw_array_t *instance,
    const afw_iterator_old_t * *iterator,
    const afw_utf8_z_t *source_z,
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_anyURI_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a anyURI value to array of anyURI. */
AFW_DEFINE(void)
afw_array_of_anyURI_add(
    const afw_array_t *instance,
    const afw_value_anyURI_t *value,
    afw_xctx_t *xctx)
{
...
/* Add a anyURI internal to array of anyURI. */
AFW_DEFINE(void)
afw_array_of_anyURI_add_internal(
    const afw_array_t *instance,
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_anyURI_create_managed(value, xctx);
    }
    else {
        v = afw_value_anyURI_create(value, instance->p, xctx);
    }
    afw_array_push_value(instance, v, xctx);
}

/* Remove a anyURI value from array of anyURI. */
AFW_DEFINE(void)
afw_array_of_anyURI_remove(
    const afw_array_t *instance,
    const afw_value_anyURI_t *value,
    afw_xctx_t *xctx)
{
    afw_array_of_anyURI_remove_internal(instance, &value->internal, xctx);
}

/* Remove a anyURI internal from array of anyURI. */
AFW_DEFINE(void)
afw_array_of_anyURI_remove_internal(
    const afw_array_t *instance,
    const afw_utf8_t *value,
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_anyURI_create(value, xctx->p, xctx);
