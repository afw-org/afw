...
/**
 * @file afw_data_type_objectId_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_objectId_direct

/* Define inf variables for data_type and is_evaluated_of_data_type. */
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    (const void *)&afw_data_type_objectId_direct, \
    (const void *)&afw_data_type_objectId_direct
...
/* unmanaged objectId: get_reference/release throw; */
/* get_assignable_value creates a managed holdable in xctx->p. */
#define AFW_IMPLEMENTATION_ID "objectId"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_unmanaged_objectId_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* managed objectId: optional_release drops RC; */
/* get_reference / get_assignable_value bump. */
#define AFW_IMPLEMENTATION_ID "managed_objectId"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_objectId_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice objectId: own RC; holds containing; */
/* last release frees slice header via xctx->p. */
#define AFW_IMPLEMENTATION_ID "managed_slice_objectId"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_slice_objectId_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* permanent objectId: optional_release NULL; */
/* get_reference / get_assignable_value as-is. */
#define AFW_IMPLEMENTATION_ID "permanent_objectId"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_objectId_inf
...
impl_data_type_objectId_id_value = {
...
{
    (const afw_value_t *)&impl_data_type_objectId_id_value
};
...
/* Permanent empty array of objectId. */
const afw_array_from_values_self_t
impl_empty_array_of_objectId;

/* Permanent empty array value of objectId. */
const afw_value_array_t
impl_value_empty_array_of_objectId;

/* Data type objectId instance. */
...
afw_data_type_objectId_direct = {
    ...
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_objectId,
    (const afw_value_t *)&impl_value_empty_array_of_objectId,
    &afw_value_unmanaged_objectId_inf,
    afw_value_clone_objectId_unmanaged,
    afw_value_clone_objectId_managed,
...
/* Data type struct for objectId. */
AFW_DEFINE_CONST_DATA(afw_data_type_t *)
afw_data_type_objectId =
    &afw_data_type_objectId_direct;

/* Set property from objectId value. */
AFW_DEFINE(void)
afw_object_set_property_as_objectId(
    const afw_object_t *object,
    const afw_value_t *property_name,
    const afw_value_objectId_t *value,
    afw_xctx_t *xctx)
{
...
/* Set property from objectId internal. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_objectId_create_managed(internal, xctx);
    }
    else {
        v = afw_value_objectId_create(internal, object->p, xctx);
    }
    afw_object_set_property(object, property_name, v, xctx);
}

/* Typesafe cast to evaluated objectId value. */
AFW_DEFINE(const afw_value_objectId_t *)
afw_value_as_objectId(const afw_value_t *value, afw_xctx_t *xctx)
{
    ...
    if (!AFW_VALUE_IS_DATA_TYPE(value, objectId))
    {
        ...
        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'objectId' but "
    ...
    return (const afw_value_objectId_t *)value;
}

/* Typesafe peel of data type objectId internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_value_as_objectId_internal(const afw_value_t *value, afw_xctx_t *xctx)
{
    return &afw_value_as_objectId(value, xctx)->internal;
}

/* Allocate function for data type objectId values. */
AFW_DEFINE(afw_value_objectId_t *)
afw_value_objectId_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_objectId_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_objectId_t),
        xctx);
    result->inf = &afw_value_unmanaged_objectId_inf;
    return result;
}

/* Create function for managed data type objectId value. */
AFW_DEFINE(const afw_value_t *)
afw_value_objectId_create_managed(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
    afw_value_objectId_managed_t *v;
    afw_size_t len;

    len = (internal) ? internal->len : 0;
    v = afw_pool_calloc(xctx->p,
        sizeof(afw_value_objectId_managed_t) + len, xctx);
    v->inf = &afw_value_managed_objectId_inf;
    v->internal.len = len;
    v->internal.s = (const afw_utf8_octet_t *)v +
        sizeof(afw_value_objectId_managed_t);
...
AFW_DEFINE(const afw_value_t *)
afw_value_objectId_create_managed_slice(
    const afw_value_t *containing_value,
    afw_size_t offset,
    afw_size_t len,
    afw_xctx_t *xctx)
{
    const afw_value_objectId_managed_t *containing;
    const afw_utf8_t *base;
    afw_value_objectId_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_objectId_inf) {
        const afw_value_objectId_managed_slice_t *slice =
            (const afw_value_objectId_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_objectId_inf) {
        containing =
            (const afw_value_objectId_managed_t *)containing_value;
        base = &containing->internal;
    }
    else {
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice objectId",
            xctx);
    }
    ...
    v = afw_pool_calloc(xctx->p, sizeof(afw_value_objectId_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_objectId_inf;
...
/* Create function for data type objectId value. */
AFW_DEFINE(const afw_value_t *)
afw_value_objectId_create(const afw_utf8_t * internal,
...
/* Deep clone evaluated objectId unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_objectId_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_objectId_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_objectId, p, xctx);
    afw_data_type_clone_internal(afw_data_type_objectId,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_objectId_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated objectId managed in xctx->p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_objectId_managed(
    const afw_value_t *value,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_objectId_create_managed(
        &((const afw_value_objectId_t *)value)->internal,
        xctx);
}

/* Convert data type objectId string to afw_utf8_t *. */
AFW_DEFINE(void)
...
/*  Convert data type objectId internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
...
/* Get property as objectId value. */
AFW_DEFINE(const afw_value_objectId_t *)
...
/* Get property function for data type objectId internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_property_as_objectId_internal_source(
    const afw_object_t *object,
    const afw_value_t *property_name,
    const afw_utf8_z_t *source_z,
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_objectId_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as objectId value. */
AFW_DEFINE(const afw_value_objectId_t *)
...
/* Get next property function for data type objectId internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_next_property_as_objectId_internal_source(
    const afw_object_t *object,
    const afw_iterator_old_t * *iterator,
    const afw_value_t * *property_name,
    const afw_utf8_z_t *source_z,
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_objectId_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
impl_afw_value_managed_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_objectId_managed_t *self =
        (afw_value_objectId_managed_t *)instance;
    ...
    if (self->reference_count == 0) {
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_objectId_managed_t) + self->internal.len, xctx);
    }
}
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_objectId_t *self =
        (const afw_value_objectId_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_objectId_managed_slice_t *self =
        (afw_value_objectId_managed_slice_t *)instance;
    ...
    if (self->reference_count == 0) {
        ...
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_objectId_managed_slice_t), xctx);
    }
}
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_objectId;
    info->optimized_value = instance;
}


/* Get next objectId value from array of objectId. */
AFW_DEFINE(const afw_value_objectId_t *)
...
/* Get next objectId internal from array of objectId. */
AFW_DEFINE(const afw_utf8_t *)
afw_array_of_objectId_get_next_internal_source(
    const afw_array_t *instance,
    const afw_iterator_old_t * *iterator,
    const afw_utf8_z_t *source_z,
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_objectId_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a objectId value to array of objectId. */
AFW_DEFINE(void)
...
/* Add a objectId internal to array of objectId. */
AFW_DEFINE(void)
afw_array_of_objectId_add_internal(
    const afw_array_t *instance,
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_objectId_create_managed(value, xctx);
    }
    else {
        v = afw_value_objectId_create(value, instance->p, xctx);
    }
    afw_array_push_value(instance, v, xctx);
}

/* Remove a objectId value from array of objectId. */
AFW_DEFINE(void)
afw_array_of_objectId_remove(
    ...
    afw_xctx_t *xctx)
{
    afw_array_of_objectId_remove_internal(instance, &value->internal, xctx);
}

/* Remove a objectId internal from array of objectId. */
AFW_DEFINE(void)
afw_array_of_objectId_remove_internal(
    const afw_array_t *instance,
    const afw_utf8_t *value,
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_objectId_create(value, xctx->p, xctx);
