...
/**
 * @file afw_data_type_template_binding.c
...
/* unmanaged template: get_reference/release throw; */
/* get_assignable_value creates a managed holdable in xctx->p. */
#define AFW_IMPLEMENTATION_ID "template"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_unmanaged_template_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* managed template: optional_release drops RC; */
/* get_reference / get_assignable_value bump. */
#define AFW_IMPLEMENTATION_ID "managed_template"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_template_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice template: own RC; holds containing; */
/* last release frees slice header via xctx->p. */
#define AFW_IMPLEMENTATION_ID "managed_slice_template"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_slice_template_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* permanent template: optional_release NULL; */
/* get_reference / get_assignable_value as-is. */
#define AFW_IMPLEMENTATION_ID "permanent_template"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_template_inf
...
impl_data_type_template_id_value = {
    {&afw_value_permanent_string_inf},
    AFW_UTF8_LITERAL("template")
};

AFW_DEFINE_CONST_DATA(afw_value_t *)
afw_data_type_template_id_value =
{
    (const afw_value_t *)&impl_data_type_template_id_value
};

static const afw_utf8_t
impl_data_type_path_template =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/template");

static const afw_value_object_t
impl_data_type_object_template__value;

/* Data type template object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_template = {
    {
        &afw_runtime_inf__AdaptiveDataType_,
        NULL,
        (const afw_value_t *)&impl_data_type_object_template__value,
        {
            NULL,
            NULL,
            &afw_self_s_template,
    ...
    (void *)&afw_data_type_template_direct
};

static const afw_value_object_t
impl_data_type_object_template__value = {
    {&afw_value_permanent_object_inf},
    (const afw_object_t *)&impl_data_type_object_template
};

/* Permanent empty array of template. */
const afw_array_from_values_self_t
impl_empty_array_of_template;

/* Permanent empty array value of template. */
const afw_value_array_t
impl_value_empty_array_of_template;

/* Data type template instance. */
AFW_DEFINE_CONST_DATA(afw_data_type_t)
afw_data_type_template_direct = {
    &afw_data_type_template_inf,
    (const afw_object_t *)&impl_data_type_object_template,
    AFW_UTF8_LITERAL("template"),
    AFW_UTF8_LITERAL("An adaptive template"),
    AFW_UTF8_LITERAL("Data type 'template' holds the source for an adaptive template."),
    AFW_UTF8_LITERAL("SourceParameter"),
    afw_data_type_number_template,
    AFW_UTF8_LITERAL("1.3.6.1.4.1.1466.115.121.1.15{64512}"),
    AFW_UTF8_LITERAL("string"),
    AFW_UTF8_LITERAL(""),
    AFW_UTF8_LITERAL("afw_utf8_t"),
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_template,
    (const afw_value_t *)&impl_value_empty_array_of_template,
    &afw_value_unmanaged_template_inf,
    afw_value_clone_template_unmanaged,
    afw_value_clone_template_managed,
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_template_create_managed(internal, xctx);
    }
    else {
    ...
    if (!AFW_VALUE_IS_DATA_TYPE(value, template))
    {
        ...
        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'template' but "
...
AFW_DEFINE(afw_value_template_t *)
afw_value_template_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_template_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_template_t),
        xctx);
    result->inf = &afw_value_unmanaged_template_inf;
    return result;
}

/* Create function for managed data type template value. */
AFW_DEFINE(const afw_value_t *)
afw_value_template_create_managed(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
...
/* Create function for managed data type template slice value. */
AFW_DEFINE(const afw_value_t *)
afw_value_template_create_managed_slice(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_template_managed_t *containing;
    const afw_utf8_t *base;
    afw_value_template_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_template_inf) {
        const afw_value_template_managed_slice_t *slice =
            (const afw_value_template_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_template_inf) {
        containing =
            (const afw_value_template_managed_t *)containing_value;
        base = &containing->internal;
    }
    else {
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice template",
            xctx);
    }
    ...
    v = afw_pool_calloc(xctx->p, sizeof(afw_value_template_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_template_inf;
...
/* Create function for data type template value. */
AFW_DEFINE(const afw_value_t *)
afw_value_template_create(const afw_utf8_t * internal,
...
/* Deep clone evaluated template unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_template_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_template_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_template, p, xctx);
    afw_data_type_clone_internal(afw_data_type_template,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_template_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated template managed in xctx->p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_template_managed(
    const afw_value_t *value,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_template_create_managed(
        &((const afw_value_template_t *)value)->internal,
        xctx);
}

/* Convert data type template string to afw_utf8_t *. */
AFW_DEFINE(void)
afw_data_type_template_to_internal(afw_utf8_t *to_internal,
...
/*  Convert data type template internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
afw_data_type_template_to_utf8(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Get property as template value. */
AFW_DEFINE(const afw_value_template_t *)
...
/* Get property function for data type template internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_template_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as template value. */
AFW_DEFINE(const afw_value_template_t *)
...
/* Get next property function for data type template internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_template_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
impl_afw_value_managed_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_template_managed_t *self =
        (afw_value_template_managed_t *)instance;
    ...
    if (self->reference_count == 0) {
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_template_managed_t) + self->internal.len, xctx);
    }
}
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_template_t *self =
        (const afw_value_template_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_template_managed_slice_t *self =
        (afw_value_template_managed_slice_t *)instance;
    ...
    if (self->reference_count == 0) {
        ...
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_template_managed_slice_t), xctx);
    }
}
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_template;
    info->optimized_value = instance;
}


/* Get next template value from array of template. */
AFW_DEFINE(const afw_value_template_t *)
...
/* Get next template internal from array of template. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_template_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a template value to array of template. */
AFW_DEFINE(void)
...
/* Add a template internal to array of template. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_template_create_managed(value, xctx);
    }
    else {
