...
/* unmanaged template: optional_release NULL; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed template: optional_release frees header at RC 0; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice template: RC on containing managed value; */
...
/* Value for empty array of template. */
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_unmanaged_template(internal, object->p, xctx);
...
afw_value_allocate_unmanaged_template(const afw_pool_t *p, afw_xctx_t *xctx)
{
...
AFW_DEFINE(const afw_value_t *)
afw_value_create_managed_template_slice(
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_xctx_calloc(sizeof(afw_value_template_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_template_inf;
    v->internal.s = base->s + offset;
    v->internal.len = len;
    v->containing_value = containing;
    ((afw_value_template_managed_t *)containing)->reference_count++;
    return &v->pub;
}

/* Create function for data type template value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_unmanaged_template(const afw_utf8_t * internal,
    ...
    afw_xctx_t *xctx)
{
    ...
    (void)p;
    boxed = afw_value_create_managed_template(
        &self->internal, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_value_template_managed_t *containing =
        (afw_value_template_managed_t *)self->containing_value;
...
impl_afw_value_managed_slice_get_reference(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_template_managed_slice_t *self =
        (const afw_value_template_managed_slice_t *)instance;
    afw_value_template_managed_t *containing =
        (afw_value_template_managed_t *)self->containing_value;
    afw_value_template_managed_slice_t *v;
