...
/* unmanaged string: optional_release NULL; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed string: optional_release frees header at RC 0; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice string: RC on containing managed value; */
...
/* Value for empty array of string. */
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_unmanaged_string(internal, object->p, xctx);
...
afw_value_allocate_unmanaged_string(const afw_pool_t *p, afw_xctx_t *xctx)
{
...
afw_value_create_managed_string(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
...
AFW_DEFINE(const afw_value_t *)
afw_value_create_managed_string_slice(
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_xctx_calloc(sizeof(afw_value_string_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_string_inf;
    v->internal.s = base->s + offset;
    v->internal.len = len;
    v->containing_value = containing;
    ((afw_value_string_managed_t *)containing)->reference_count++;
    return &v->pub;
}

/* Create function for data type string value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_unmanaged_string(const afw_utf8_t * internal,
    ...
    afw_xctx_t *xctx)
{
    ...
    (void)p;
    boxed = afw_value_create_managed_string(
        &self->internal, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_value_string_managed_t *containing =
        (afw_value_string_managed_t *)self->containing_value;
