...
/* This is the inf for unmanaged string values. For this one */
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_string_unmanaged(internal, object->p, xctx);
...
/* Allocate function for unmanaged data type string values. */
...
/* Allocate function for managed data type string values. */
AFW_DEFINE(afw_value_string_t *)
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_managed_string(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return &v->pub;
...
/* Allocate function for permanent data type string values. */
AFW_DEFINE(afw_value_string_t *)
afw_value_allocate_permanent_string(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_string_t),
        xctx);
    result->inf = &afw_value_permanent_string_inf;
    return result;
}
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_permanent_string(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return &v->pub;
