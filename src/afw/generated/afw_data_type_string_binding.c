...
/**
 * @file afw_data_type_string_binding.c
...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_string_direct

/* Define inf variables for data_type and is_evaluated_of_data_type. */
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    (const void *)&afw_data_type_string_direct, \
    (const void *)&afw_data_type_string_direct
...
#define AFW_IMPLEMENTATION_ID "string"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_unmanaged_string_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* managed string: optional_release drops RC; */
/* get_reference / get_assignable_value bump. */
#define AFW_IMPLEMENTATION_ID "managed_string"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_string_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice string: own RC; holds containing; */
/* last release frees slice header via xctx->p. */
#define AFW_IMPLEMENTATION_ID "managed_slice_string"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_slice_string_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* permanent string: optional_release NULL; */
/* get_reference / get_assignable_value as-is. */
#define AFW_IMPLEMENTATION_ID "permanent_string"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_string_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* compile_literal string: optional_release NULL; */
/* get_reference / get_assignable_value as-is; clone copies. */
#define AFW_IMPLEMENTATION_ID "compile_literal_string"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_compile_literal_string_inf
#define impl_afw_value_optional_release NULL
#define impl_afw_value_get_reference impl_afw_value_permanent_get_reference
#define impl_afw_value_get_assignable_value impl_afw_value_get_assignable_via_reference
#define AFW_VALUE_INF_ONLY 1
...
impl_data_type_string_id_value = {
...
{
    (const afw_value_t *)&impl_data_type_string_id_value
};

static const afw_utf8_t
impl_data_type_path_string =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/string");

static const afw_value_object_t
impl_data_type_object_string__value;

/* Data type string object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_string = {
    {
        &afw_runtime_inf__AdaptiveDataType_,
        NULL,
        (const afw_value_t *)&impl_data_type_object_string__value,
        {
            NULL,
            NULL,
            &afw_self_s_string,
    ...
    (void *)&afw_data_type_string_direct
};
...
/* Permanent empty array of string. */
const afw_array_from_values_self_t
impl_empty_array_of_string;

/* Permanent empty array value of string. */
const afw_value_array_t
impl_value_empty_array_of_string;

/* Data type string instance. */
...
afw_data_type_string_direct = {
    ...
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_string,
    (const afw_value_t *)&impl_value_empty_array_of_string,
    &afw_value_unmanaged_string_inf,
    afw_value_clone_string_unmanaged,
    afw_value_clone_string_managed,
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_string_create_managed(internal, xctx);
    }
    else {
        v = afw_value_string_create(internal, object->p, xctx);
    }
    ...
    if (!AFW_VALUE_IS_DATA_TYPE(value, string))
    {
        ...
        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'string' but "
...
AFW_DEFINE(afw_value_string_t *)
afw_value_string_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_string_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_string_t),
        xctx);
    result->inf = &afw_value_unmanaged_string_inf;
    return result;
}

/* Create function for managed data type string value. */
AFW_DEFINE(const afw_value_t *)
afw_value_string_create_managed(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
    afw_value_string_managed_t *v;
    afw_size_t len;

    len = (internal) ? internal->len : 0;
    v = afw_pool_calloc(xctx->p,
        sizeof(afw_value_string_managed_t) + len, xctx);
    v->inf = &afw_value_managed_string_inf;
    v->internal.len = len;
    v->internal.s = (const afw_utf8_octet_t *)v +
        sizeof(afw_value_string_managed_t);
...
/* Create function for managed data type string slice value. */
AFW_DEFINE(const afw_value_t *)
afw_value_string_create_managed_slice(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_string_managed_t *containing;
    const afw_utf8_t *base;
    afw_value_string_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_string_inf) {
        const afw_value_string_managed_slice_t *slice =
            (const afw_value_string_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_string_inf) {
        containing =
            (const afw_value_string_managed_t *)containing_value;
        base = &containing->internal;
    }
    else {
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice string",
            xctx);
    }
    ...
    v = afw_pool_calloc(xctx->p, sizeof(afw_value_string_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_string_inf;
...
/* Create function for data type string value. */
AFW_DEFINE(const afw_value_t *)
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_pool_calloc(p, sizeof(afw_value_string_t),
        xctx);
    v->inf = &afw_value_unmanaged_string_inf;
    if (internal) {
...
/* Compiler-only: compile-unit string literal in p. */
AFW_DEFINE(const afw_value_t *)
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_pool_calloc(p, sizeof(afw_value_string_t),
        xctx);
    v->inf = &afw_value_compile_literal_string_inf;
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return &v->pub;
...
/* Deep clone evaluated string unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_string_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_string_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_string, p, xctx);
    afw_data_type_clone_internal(afw_data_type_string,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_string_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated string managed in xctx->p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_string_managed(
    const afw_value_t *value,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_string_create_managed(
        &((const afw_value_string_t *)value)->internal,
        xctx);
}

/* Convert data type string string to afw_utf8_t *. */
AFW_DEFINE(void)
...
/*  Convert data type string internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
...
/* Get property function for data type string internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_string_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as string value. */
AFW_DEFINE(const afw_value_string_t *)
...
/* Get next property function for data type string internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_string_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
impl_afw_value_managed_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_string_managed_t *self =
        (afw_value_string_managed_t *)instance;
    ...
    if (self->reference_count == 0) {
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_string_managed_t) + self->internal.len, xctx);
    }
}
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_string_t *self =
        (const afw_value_string_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_string_managed_slice_t *self =
        (afw_value_string_managed_slice_t *)instance;
    ...
    if (self->reference_count == 0) {
        ...
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_string_managed_slice_t), xctx);
    }
}
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_string;
    info->optimized_value = instance;
}


/* Get next string value from array of string. */
AFW_DEFINE(const afw_value_string_t *)
...
/* Get next string internal from array of string. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_string_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a string value to array of string. */
AFW_DEFINE(void)
...
/* Add a string internal to array of string. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_string_create_managed(value, xctx);
    }
    else {
        v = afw_value_string_create(value, instance->p, xctx);
    }
    afw_array_push_value(instance, v, xctx);
}

/* Remove a string value from array of string. */
AFW_DEFINE(void)
