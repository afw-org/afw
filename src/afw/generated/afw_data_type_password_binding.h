 ...
 * @brief Generated header for adaptive data type `password`.
 */
...
/**
 * @brief Managed slice value inf for data type password.
 *
 ...
 */
AFW_DECLARE_CONST_DATA(afw_value_inf_t)
afw_value_managed_slice_password_inf;
 ...
 */
#define afw_data_type_is_password(_A_DATA_TYPE) \
( \
    (_A_DATA_TYPE) && \
    (_A_DATA_TYPE) == afw_data_type_password \
)

/**
 * @brief True if _A_VALUE is an evaluated password value.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * For evaluated values only. When true, it is safe to cast _A_VALUE to
 * `const afw_value_password_t *`.
 * If you want to know if the value will be password when fully
 * evaluated (not necessarily cast-safe yet), use
 * `AFW_VALUE_EVALUATES_TO_DATA_TYPE(_A_VALUE, password, xctx)` instead.
 */
#define afw_value_is_password(_A_VALUE) \
( \
    (_A_VALUE) && \
    (_A_VALUE)->inf->is_evaluated_of_data_type == afw_data_type_password\
)

/**
 * @brief True if _A_VALUE is an evaluated array of password.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * When true, _A_VALUE is an evaluated array (`const afw_value_array_t *`)
 * whose element data type is password.
 */
#define afw_value_is_array_of_password(_A_VALUE) \
( \
...
/** @brief struct for managed slice data type password values.
 *
 * A view into a containing managed value. Holds that value.
 */
struct afw_value_password_managed_slice_s {
    /** @brief  Value inf union with afw_value_t pub to reduce casting needed. */
    union {
        const afw_value_inf_t *inf;
        afw_value_t pub;
...
/**
 * @brief Typesafe cast to evaluated password value.
 * @param value (const afw_value_t *). Evaluated if needed.
 * @return (const afw_value_password_t *)
 *
 * Throws if missing or wrong type. Use ->internal for the C
 * payload, or afw_value_as_password_internal().
 */
AFW_DECLARE(const afw_value_password_t *)
afw_value_as_password(
    const afw_value_t *value,
    afw_xctx_t *xctx);

/**
 * @brief Typesafe peel of data type password internal.
 * @param value (const afw_value_t *).
 * @return (const afw_utf8_t *)
 */
AFW_DECLARE(const afw_utf8_t *)
afw_value_as_password_internal(
    const afw_value_t *value,
    afw_xctx_t *xctx);
 ...
 * @param xctx of caller.
 * @return Allocated afw_value_password_t with unmanaged inf set.
 *
 * Unmanaged: lifetime is pool p; no value refcount.
 * Caller fills internal after allocate.
 */
AFW_DECLARE(afw_value_password_t *)
afw_value_password_allocate(
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_allocate_unmanaged_password afw_value_password_allocate
...
afw_value_password_create_managed(
    const afw_utf8_t * internal,
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_password afw_value_password_create_managed

/**
 * @brief Deep clone an evaluated password value unmanaged into p.
 * @param value evaluated password.
 * @param p dest pool.
 * @param xctx of caller.
 ...
 */
AFW_DECLARE(const afw_value_t *)
afw_value_clone_password_unmanaged(
    const afw_value_t *value,
    const afw_pool_t *p,
    afw_xctx_t *xctx);

/**
 * @brief Clone an evaluated password value managed in p->managed_p.
 * @param value evaluated password.
 * @param p dest pool (uses p->managed_p).
 * @param xctx of caller.
 ...
 */
AFW_DECLARE(const afw_value_t *)
afw_value_clone_password_managed(
    const afw_value_t *value,
    const afw_pool_t *p,
    afw_xctx_t *xctx);

/**
 * @brief Create a managed slice of a managed data type password value.
 ...
 */
AFW_DECLARE(const afw_value_t *)
afw_value_password_create_managed_slice(
    const afw_value_t *containing_value,
    afw_size_t offset,
    afw_size_t len,
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_password_slice afw_value_password_create_managed_slice
 ...
 */
AFW_DECLARE(const afw_value_t *)
afw_value_password_create(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx);
#define afw_value_create_unmanaged_password afw_value_password_create

/**
 * @brief Get property as password value.
 * @param object of property to get.
 * @param property_name of property to get.
 * @param xctx of caller.
 * @return (const afw_value_password_t *) or NULL if missing.
 *
 * Does not evaluate. Throws if present but not password.
 */
...
/**
 * @brief Get property as password internal.
 * @param object of property to get.
 * @param property_name of property to get.
 * @param xctx of caller.
 * @return const afw_utf8_t *.
 */
#define afw_object_get_property_as_password_internal( \
    _object, _property_name, _xctx) \
...
/**
 * @brief Get next property as password internal.
 * @param object of property to get.
 * @param iterator pointer. Set to NULL before first call.
 * @param property_name is place to return pointer to property name.
 * @param xctx of caller.
 * @return const afw_utf8_t *.
 */
#define afw_object_get_next_property_as_password_internal( \
    _object, _iterator, _property_name, _xctx) \
...
/**
 * @brief Set property as password value.
 * @param object of property to set.
 * @param property_name of property to set.
 * @param value to set.
 * @param xctx of caller.
 *
 * Compile-time type check for const afw_value_password_t *.
 */
AFW_DECLARE(void)
...
/**
 * @brief Set property as password internal.
 * @param object of property to set.
 * @param property_name of property to set.
...
/**
 * @brief Get next password value from array of password.
 * @param instance of array.
 * @param iterator.
 * @param xctx of caller.
 * @return (const afw_value_password_t *) or NULL.
 *
...
/**
 * @brief Get next password internal from array of password.
 * @param instance of array.
 * @param iterator.
 * @param xctx of caller.
 * @return (const afw_utf8_t *) or NULL.
 */
#define afw_array_of_password_get_next_internal( \
    _array, _iterator, _xctx) \
    afw_array_of_password_get_next_internal_source( \
    _array, _iterator, AFW__FILE_LINE__, _xctx)
...
/**
 * @brief Add a password value to array of password.
 * @param instance of array.
 * @param value to add.
 * @param xctx of caller.
 */
...
/**
 * @brief Add a password internal to array of password.
 * @param instance of array.
 * @param value to add.
 * @param xctx of caller.
 */
AFW_DECLARE(void)
afw_array_of_password_add_internal(
    const afw_array_t *instance,
    const afw_utf8_t *value,
    afw_xctx_t *xctx);

/**
 * @brief Remove a password value from array of password.
 * @param instance of array.
...
/**
 * @brief Remove a password internal from array of password.
 * @param instance of array.
 * @param value to remove.
 * @param xctx of caller.
 */
AFW_DECLARE(void)
afw_array_of_password_remove_internal(
    const afw_array_t *instance,
    const afw_utf8_t *value,
    afw_xctx_t *xctx);

/**
 * @brief Public data type password struct instance.
 *
 * Prefer afw_data_type_password when a pointer is enough and you are not
...
/**
 * @brief Public data type password inf.
 *
