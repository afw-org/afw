...
/* unmanaged date: get_reference/release throw; */
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_date_direct, \
    true
/* managed date: optional_release drops RC; */
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_date_direct, \
    false
/* permanent date: optional_release NULL; */
...
impl_data_type_object_date__value;
...
impl_data_type_object_date = {
    {
        ...
        NULL,
        (const afw_value_t *)&impl_data_type_object_date__value,
        {
            NULL,
            NULL,
            &afw_self_s_date,
...
/* Permanent empty array of date. */
const afw_array_from_values_self_t
impl_empty_array_of_date;

/* Permanent empty array value of date. */
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_date_create_managed(internal, object->p, xctx);
    }
    else {
...
AFW_DEFINE(const afw_value_date_t *)
...
/* Typesafe peel of data type date internal. */
AFW_DEFINE(const afw_date_t *)
afw_value_as_date_internal(const afw_value_t *value, afw_xctx_t *xctx)
{
    return &afw_value_as_date(value, xctx)->internal;
}

/* Allocate function for data type date values. */
AFW_DEFINE(afw_value_date_t *)
afw_value_date_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
...
afw_value_date_create_managed(
    ...
    afw_xctx_t *xctx)
{
    afw_value_date_managed_t *v;

    p = p->managed_p;
    v = afw_pool_calloc(p,
        sizeof(afw_value_date_managed_t), xctx);
...
/* Deep clone evaluated date unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_date_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_date_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_date, p, xctx);
    afw_data_type_clone_internal(afw_data_type_date,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_date_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated date managed in p->managed_p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_date_managed(
    const afw_value_t *value,
    const afw_pool_t *p,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_date_create_managed(
        &((const afw_value_date_t *)value)->internal,
        p, xctx);
}
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_date_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as date value. */
AFW_DEFINE(const afw_value_date_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_date_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
impl_afw_value_managed_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_date_managed_t *self =
        (afw_value_date_managed_t *)instance;
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_date_t *self =
        (const afw_value_date_t *)instance;
...
/* Get next date value from array of date. */
AFW_DEFINE(const afw_value_date_t *)
...
/* Get next date internal from array of date. */
AFW_DEFINE(const afw_date_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_date_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a date value to array of date. */
AFW_DEFINE(void)
...
/* Add a date internal to array of date. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_date_create_managed(value, instance->p, xctx);
    }
    else {
...
afw_array_of_date_remove(
    ...
    afw_xctx_t *xctx)
{
    afw_array_of_date_remove_internal(instance, &value->internal, xctx);
}

/* Remove a date internal from array of date. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_date_create(value, xctx->p, xctx);
