 ...
 */
#define afw_data_type_is_anyURI(A_DATA_TYPE) \
( \
    (A_DATA_TYPE) && \
    (A_DATA_TYPE) == afw_data_type_anyURI \
)

/**
 * @brief True if A_VALUE is an evaluated anyURI value.
 ...
 * `AFW_VALUE_EVALUATES_TO_DATA_TYPE(A_VALUE, anyURI, xctx)` instead.
 */
#define afw_value_is_anyURI(A_VALUE) \
( \
    (A_VALUE) && \
    (A_VALUE)->inf->is_evaluated_of_data_type == afw_data_type_anyURI\
)

/**
 * @brief True if A_VALUE is an evaluated array of anyURI.
 * @param A_VALUE to test.
 * @return boolean result.
 *
 ...
 */
#define afw_value_is_array_of_anyURI(A_VALUE) \
( \
