...
#ifndef __AFW_DATA_TYPE_ANYURI_BINDING_H__
#define __AFW_DATA_TYPE_ANYURI_BINDING_H__
...
/**
 * @defgroup afw_c_api_data_type_anyURI anyURI
...
/**
 * @file afw_data_type_anyURI_binding.h
 * @brief Generated header for adaptive data type `anyURI`.
 */

AFW_BEGIN_DECLARES

/** @brief Quoted data type id for anyURI */
#define AFW_DATA_TYPE_ID_Q_anyURI "anyURI"

/** @brief Unquoted data type id for anyURI */
#define AFW_DATA_TYPE_ID_U_anyURI anyURI

/** @brief String for data type id anyURI */
#define AFW_DATA_TYPE_ID_S_anyURI afw_s_anyURI

/** @brief Quoted c type for data type id anyURI */
#define AFW_DATA_TYPE_CTYPE_Q_anyURI "afw_utf8_t"

/** @brief Adaptive string value for "anyURI". */
AFW_DECLARE_CONST_DATA(afw_value_t *)
afw_data_type_anyURI_id_value;

/** @brief Data type struct for anyURI. */
...
/**
 * @brief Managed slice value inf for data type anyURI.
 *
...
/**
 * @brief Permanent (life of afw environment) value inf for data type anyURI.
 *
...
/**
 * @brief Macro to determine if data type is anyURI.
 * @param _A_DATA_TYPE to test.
 * @return boolean result.
 */
#define afw_data_type_is_anyURI(_A_DATA_TYPE) \
( \
    (_A_DATA_TYPE) && \
    (_A_DATA_TYPE) == afw_data_type_anyURI \
)

/**
 * @brief True if _A_VALUE is an evaluated anyURI value.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * For evaluated values only. When true, it is safe to cast _A_VALUE to
 * `const afw_value_anyURI_t *`.
 * If you want to know if the value will be anyURI when fully
 * evaluated (not necessarily cast-safe yet), use
 * `AFW_VALUE_EVALUATES_TO_DATA_TYPE(_A_VALUE, anyURI, xctx)` instead.
 */
#define afw_value_is_anyURI(_A_VALUE) \
( \
    (_A_VALUE) && \
    (_A_VALUE)->inf->is_evaluated_of_data_type == afw_data_type_anyURI\
)

/**
 * @brief True if _A_VALUE is an evaluated array of anyURI.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * When true, _A_VALUE is an evaluated array (`const afw_value_array_t *`)
 * whose element data type is anyURI.
 */
#define afw_value_is_array_of_anyURI(_A_VALUE) \
( \
    afw_value_is_array(_A_VALUE) && \
    afw_array_get_data_type( \
        ((const afw_value_array_t *)(_A_VALUE))->internal, \
        xctx) == afw_data_type_anyURI \
)

/**
 * @brief Convert data type anyURI string to afw_utf8_t *.
 ...
 */
AFW_DECLARE(void)
afw_data_type_anyURI_to_internal(
...
/**
 * @brief Convert data type anyURI internal representation to utf-8.
...
afw_data_type_anyURI_to_utf8(
    const afw_utf8_t * internal,
    const afw_pool_t *p,
    afw_xctx_t *xctx);

/** @brief struct for data type anyURI values. */
struct afw_value_anyURI_s {
...
/** @brief struct for managed data type anyURI values.
 *
 * This is the same as afw_value_anyURI_s with the addition of a
 * reference count. This is intended for internal use only.
 */
struct afw_value_anyURI_managed_s {
    /** @brief  Value inf union with afw_value_t pub to reduce casting needed. */
    union {
        const afw_value_inf_t *inf;
        afw_value_t pub;
...
/** @brief struct for managed slice data type anyURI values.
 *
 * A view into a containing managed value. Holds that value.
 */
struct afw_value_anyURI_managed_slice_s {
    /** @brief  Value inf union with afw_value_t pub to reduce casting needed. */
    union {
        const afw_value_inf_t *inf;
        afw_value_t pub;
...
/**
 * @brief Typesafe cast to evaluated anyURI value.
 * @param value (const afw_value_t *). Evaluated if needed.
 * @return (const afw_value_anyURI_t *)
 *
 * Throws if missing or wrong type. Use ->internal for the C
 * payload, or afw_value_as_anyURI_internal().
 */
AFW_DECLARE(const afw_value_anyURI_t *)
afw_value_as_anyURI(
    const afw_value_t *value,
    afw_xctx_t *xctx);

/**
 * @brief Typesafe peel of data type anyURI internal.
 * @param value (const afw_value_t *).
 * @return (const afw_utf8_t *)
 */
AFW_DECLARE(const afw_utf8_t *)
afw_value_as_anyURI_internal(
    const afw_value_t *value,
    afw_xctx_t *xctx);

/**
 * @brief Allocate function for data type anyURI value.
 * @param p to use for returned value.
 * @param xctx of caller.
 * @return Allocated afw_value_anyURI_t with unmanaged inf set.
 *
 * Unmanaged: lifetime is pool p; no value refcount.
 * Caller fills internal after allocate.
 */
AFW_DECLARE(afw_value_anyURI_t *)
afw_value_anyURI_allocate(
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_allocate_unmanaged_anyURI afw_value_anyURI_allocate

/**
 * @brief Create function for managed data type anyURI value.
 * @param internal.
...
afw_value_anyURI_create_managed(
    ...
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_anyURI afw_value_anyURI_create_managed

/**
 * @brief Deep clone an evaluated anyURI value unmanaged into p.
 * @param value evaluated anyURI.
 * @param p dest pool.
 * @param xctx of caller.
...
/**
 * @brief Clone an evaluated anyURI value managed in p->managed_p.
 * @param value evaluated anyURI.
 * @param p dest pool (uses p->managed_p).
 * @param xctx of caller.
...
/**
 * @brief Create a managed slice of a managed data type anyURI value.
...
afw_value_anyURI_create_managed_slice(
    ...
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_anyURI_slice afw_value_anyURI_create_managed_slice

/**
 * @brief Create function for unmanaged data type anyURI value.
 * @param internal.
 * @param p to use for returned value.
 * @param xctx of caller.
 * @return Created const afw_value_t *.
...
#define afw_value_create_unmanaged_anyURI afw_value_anyURI_create

/**
 * @brief Get property as anyURI value.
 * @param object of property to get.
 * @param property_name of property to get.
 * @param xctx of caller.
 * @return (const afw_value_anyURI_t *) or NULL if missing.
 *
 * Does not evaluate. Throws if present but not anyURI.
 */
#define afw_object_get_property_as_anyURI( \
 ...
 * @return const afw_utf8_t *.
 */
#define afw_object_get_property_as_anyURI_internal( \
    _object, _property_name, _xctx) \
...
/**
 * @brief Get next property as anyURI value.
 * @param object of property to get.
 * @param iterator pointer. Set to NULL before first call.
 * @param property_name is place to return pointer to property name.
 * @param xctx of caller.
 * @return (const afw_value_anyURI_t *) or NULL if no more.
 */
#define afw_object_get_next_property_as_anyURI( \
 ...
 * @return const afw_utf8_t *.
 */
#define afw_object_get_next_property_as_anyURI_internal( \
    _object, _iterator, _property_name, _xctx) \
...
/**
 * @brief Set property as anyURI value.
 * @param object of property to set.
 * @param property_name of property to set.
 * @param value to set.
 * @param xctx of caller.
 *
 * Compile-time type check for const afw_value_anyURI_t *.
 */
AFW_DECLARE(void)
...
/**
 * @brief Set property as anyURI internal.
 * @param object of property to set.
 * @param property_name of property to set.
...
/**
 * @brief Get next anyURI internal from array of anyURI.
 * @param instance of array.
 * @param iterator.
 * @param xctx of caller.
 * @return (const afw_utf8_t *) or NULL.
 */
#define afw_array_of_anyURI_get_next_internal( \
    _array, _iterator, _xctx) \
...
/**
 * @brief Add a anyURI value to array of anyURI.
 * @param instance of array.
 * @param value to add.
 * @param xctx of caller.
 */
...
/**
 * @brief Add a anyURI internal to array of anyURI.
 * @param instance of array.
 * @param value to add.
 * @param xctx of caller.
 */
...
/**
 * @brief Remove a anyURI value from array of anyURI.
 * @param instance of array.
...
/**
 * @brief Remove a anyURI internal from array of anyURI.
 * @param instance of array.
 * @param value to remove.
 * @param xctx of caller.
 */
...
/**
 * @brief Public data type anyURI struct instance.
 *
 * Prefer afw_data_type_anyURI when a pointer is enough and you are not
...
/**
 * @brief Public data type anyURI inf.
 *
...
#endif /* __AFW_DATA_TYPE_ANYURI_BINDING_H__ */
