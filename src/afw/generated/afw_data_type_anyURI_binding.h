 ...
 * `const afw_value_anyURI_t *` (e.g. to read `.internal`).
 * Same as `AFW_VALUE_IS_DATA_TYPE(A_VALUE, anyURI)`.
 * Not the same as "will evaluate to anyURI" — see
...
afw_value_allocate_unmanaged_anyURI(
    const afw_pool_t *p,
    afw_xctx_t *xctx);
...
/**
 * @brief extern for data type anyURI struct.
 *
 * This should only be managed in the linkage unit the extern is
 * defined in.  Use afw_data_type_anyURI when not referencing in
 * a static.
 */
...
/**
 * @brief extern for data type anyURI inf.
 *
 * This should only be managed in the linkage unit the extern is
 * defined in.
 *
