...
/**
 * @brief Managed slice value inf for data type string.
 *
 ...
 */
#define afw_data_type_is_string(_A_DATA_TYPE) \
( \
    (_A_DATA_TYPE) && \
    (_A_DATA_TYPE) == afw_data_type_string \
)

/**
 * @brief True if _A_VALUE is an evaluated string value.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * For evaluated values only. When true, it is safe to cast _A_VALUE to
 * `const afw_value_string_t *`.
 * If you want to know if the value will be string when fully
 * evaluated (not necessarily cast-safe yet), use
 * `AFW_VALUE_EVALUATES_TO_DATA_TYPE(_A_VALUE, string, xctx)` instead.
 */
#define afw_value_is_string(_A_VALUE) \
( \
    (_A_VALUE) && \
    (_A_VALUE)->inf->is_evaluated_of_data_type == afw_data_type_string\
)

/**
 * @brief True if _A_VALUE is an evaluated array of string.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 ...
 */
#define afw_value_is_array_of_string(_A_VALUE) \
( \
 ...
 */
AFW_DECLARE(void)
afw_data_type_string_to_internal(
...
/** @brief struct for managed data type string values.
 *
 * This is the same as afw_value_string_s with the addition of a
 * reference count. This is intended for internal use only.
 */
struct afw_value_string_managed_s {
    /** @brief  Value inf union with afw_value_t pub to reduce casting needed. */
    union {
        const afw_value_inf_t *inf;
        afw_value_t pub;
...
/** @brief struct for managed slice data type string values.
 *
 * A view into a containing managed value. Holds that value.
 */
struct afw_value_string_managed_slice_s {
    /** @brief  Value inf union with afw_value_t pub to reduce casting needed. */
    union {
        const afw_value_inf_t *inf;
        afw_value_t pub;
...
/**
 * @brief Allocate function for data type string value.
 * @param p to use for returned value.
 * @param xctx of caller.
 * @return Allocated afw_value_string_t with unmanaged inf set.
 *
 * Unmanaged: lifetime is pool p; no value refcount.
 * Caller fills internal after allocate.
 */
AFW_DECLARE(afw_value_string_t *)
afw_value_string_allocate(
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_allocate_unmanaged_string afw_value_string_allocate
...
afw_value_string_create_managed(
    ...
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_string afw_value_string_create_managed

/**
 * @brief Deep clone an evaluated string value unmanaged into p.
...
/**
 * @brief Clone an evaluated string value managed in p->managed_p.
 * @param value evaluated string.
 * @param p dest pool (uses p->managed_p).
 * @param xctx of caller.
...
/**
 * @brief Create a managed slice of a managed data type string value.
...
afw_value_string_create_managed_slice(
    ...
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_string_slice afw_value_string_create_managed_slice
