...
/**
 * @file afw_data_type_expression_binding.c
...
/* This is the inf for unmanaged expression values. For this one */
/* optional_release is NULL and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "expression"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_evaluated_expression_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for managed expression values. For this one */
/* optional_release releases value and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "managed_expression"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_expression_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for permanent expression values. For this one */
/* optional_release is NULL and get_reference returns instance asis. */
#define AFW_IMPLEMENTATION_ID "permanent_expression"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_expression_inf
#define impl_afw_value_optional_release NULL
...
impl_data_type_expression_id_value = {
    &afw_value_evaluated_string_inf,
    AFW_UTF8_LITERAL("expression")
};

AFW_DEFINE_CONST_DATA(afw_value_t *)
afw_data_type_expression_id_value =
{
    (const afw_value_t *)&impl_data_type_expression_id_value
};

static const afw_utf8_t
impl_data_type_path_expression =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/expression");

/* Data type expression object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_expression = {
    {
    ...
    (void *)&afw_data_type_expression_direct
};

/* Value for empty array of expression. */
AFW_DEFINE_INTERNAL_CONST_DATA(afw_array_wrapper_for_array_self_t)
impl_empty_array_of_expression;

/* Value for empty array of expression. */
AFW_DEFINE_INTERNAL_CONST_DATA(afw_value_array_t)
impl_value_empty_array_of_expression;

/* Data type expression instance. */
AFW_DEFINE_INTERNAL_CONST_DATA(afw_data_type_t)
afw_data_type_expression_direct = {
    &afw_data_type_expression_inf,
    (const afw_object_t *)&impl_data_type_object_expression,
    AFW_UTF8_LITERAL("expression"),
    AFW_UTF8_LITERAL("An adaptive expression"),
    AFW_UTF8_LITERAL("Data type expression holds the source for an adaptive expression."),
    AFW_UTF8_LITERAL("SourceParameter"),
    afw_data_type_number_expression,
    AFW_UTF8_LITERAL("1.3.6.1.4.1.1466.115.121.1.15{64512}"),
    AFW_UTF8_LITERAL("string"),
    AFW_UTF8_LITERAL(""),
    AFW_UTF8_LITERAL("afw_utf8_t"),
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_expression,
    (const afw_value_t *)&impl_value_empty_array_of_expression,
...
/* Set property function for data type expression values. */
AFW_DEFINE(void)
afw_object_set_property_as_expression(
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_expression(internal, object->p, xctx);
    afw_object_set_property(object, property_name, v, xctx);
}

/* Typesafe cast of data type expression. */
AFW_DEFINE(const afw_utf8_t *)
afw_value_as_expression(const afw_value_t *value, afw_xctx_t *xctx)
{
    value = afw_value_evaluate(value, xctx->p, xctx);
    if (!AFW_VALUE_IS_DATA_TYPE(value, expression))
    {
        const afw_utf8_t *data_type_id;

        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'expression' but "
    ...
    return &(((const afw_value_expression_t *)value)->internal);
}

/* Allocate function for unmanaged data type expression values. */
AFW_DEFINE(afw_value_expression_t *)
afw_value_allocate_expression(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_expression_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_expression_t),
        xctx);
    result->inf = &afw_value_evaluated_expression_inf;
    return result;
}

/* Create function for unmanaged data type expression value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_expression(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_expression_t *v;

    v = afw_value_allocate_expression(p, xctx);
    if (internal) {
...
/* Allocate function for managed data type expression values. */
AFW_DEFINE(afw_value_expression_t *)
afw_value_allocate_managed_expression(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_expression_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_expression_t),
        xctx);
    result->inf = &afw_value_managed_expression_inf;
    return result;
}

/* Create function for data type expression value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_managed_expression(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_expression_t *v;

    v = afw_value_allocate_managed_expression(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return (const afw_value_t *)v;
...
/* Allocate function for permanent data type expression values. */
AFW_DEFINE(afw_value_expression_t *)
afw_value_allocate_permanent_expression(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_expression_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_expression_t),
        xctx);
    result->inf = &afw_value_permanent_expression_inf;
    return result;
}

/* Create function for data type expression value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_permanent_expression(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_expression_t *v;

    v = afw_value_allocate_permanent_expression(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return (const afw_value_t *)v;
...
/* Convert data type expression string to afw_utf8_t *. */
AFW_DEFINE(void)
afw_data_type_expression_to_internal(afw_utf8_t *to_internal,
...
/*  Convert data type expression internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
afw_data_type_expression_to_utf8(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Get property function for data type expression values. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_property_as_expression_source(
    const afw_object_t *object,
    const afw_utf8_t *property_name,
...
/* Get next property function for data type expression values. */
AFW_DEFINE(const afw_utf8_t *)
afw_object_get_next_property_as_expression_source(
    const afw_object_t *object,
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_expression;
    info->optimized_value = instance;
}


/* Get next value from array of expression. */
AFW_DEFINE(const afw_utf8_t *)
afw_array_of_expression_get_next_source(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (data_type != afw_data_type_expression) {
        const afw_utf8_t *data_type_id;
...
/* Add value from array of expression */
AFW_DEFINE(void)
afw_array_of_expression_add(
    const afw_array_t *instance,
...
/* Remove value from array of expression */
AFW_DEFINE(void)
afw_array_of_expression_remove(
    const afw_array_t *instance,
    const afw_utf8_t *value,
    afw_xctx_t *xctx)
{
