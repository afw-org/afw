...
/* unmanaged time: get_reference/release throw; */
...
#define AFW_IMPLEMENTATION_INF_VARIABLES \
    ...
    (const void *)&afw_data_type_time_direct, \
    true
/* managed time: optional_release drops RC; */
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_time_create_managed(internal, object->p, xctx);
    }
    else {
...
afw_value_time_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Clone evaluated time managed in p->managed_p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_time_managed(
    const afw_value_t *value,
    const afw_pool_t *p,
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_time_t *self =
        (const afw_value_time_t *)instance;
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_time_create_managed(value, instance->p, xctx);
    }
    else {
