...
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_evaluated_time_inf
...
afw_data_type_time_direct = {
    ...
    sizeof(afw_time_t),
    (const afw_list_t *)&impl_empty_list_of_time,
    (const afw_value_t *)&impl_value_empty_list_of_time,
...
afw_value_allocate_time(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_time_t),
        xctx);
    result->inf = &afw_value_evaluated_time_inf;
    return result;
}
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_time(p, xctx);
    if (internal) {
...
/* Allocate function for permanent data type time values. */
AFW_DEFINE(afw_value_time_t *)
afw_value_allocate_permanent_time(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_time_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_time_t),
        xctx);
    result->inf = &afw_value_permanent_time_inf;
    return result;
}
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_permanent_time(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_time_t));
    }
    return (const afw_value_t *)v;
    ...
    afw_xctx_t *xctx)
{
    ...
    info->optimized_value_data_type = afw_data_type_time;
}


/* Get next value from list of time. */
AFW_DEFINE(const afw_time_t *)
afw_list_of_time_get_next_source(
...
/* Add value from list of time */
AFW_DEFINE(void)
afw_list_of_time_add(
    const afw_list_t *instance,
...
/* Remove value from list of time */
AFW_DEFINE(void)
afw_list_of_time_remove(
    const afw_list_t *instance,
    const afw_time_t *value,
    afw_xctx_t *xctx)
{
