...
impl_data_type_object_time__value;
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_unmanaged_time(internal, object->p, xctx);
...
/* Allocate function for managed data type time value. */
AFW_DEFINE(const afw_value_t *)
afw_value_allocate_managed_time(
    afw_time_t **internal,
    afw_xctx_t *xctx)
{
    afw_value_time_managed_t *result;

    result = afw_xctx_malloc(sizeof(afw_value_time_managed_t), xctx);
...
afw_value_allocate_unmanaged_time(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_time_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_time_t),
        xctx);
    result->inf = &afw_value_unmanaged_time_inf;
    return result;
}

/* Create function for managed data type time value. */
AFW_DEFINE(const afw_value_t *)
afw_value_create_managed_time(
    const afw_time_t * internal,
    afw_xctx_t *xctx)
{
    afw_value_time_managed_t *v;

    v = afw_xctx_calloc(
        sizeof(afw_value_time_managed_t), xctx);
    v->inf = &afw_value_managed_time_inf;
    if (internal) {
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_pool_calloc(p, sizeof(afw_value_time_t),
        xctx);
    v->inf = &afw_value_unmanaged_time_inf;
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_time_t));
    }
    return &v->pub;
