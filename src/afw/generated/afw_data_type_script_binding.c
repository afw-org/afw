    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_script_create_managed(internal, object->p, xctx);
    }
    else {
    ...
    afw_xctx_t *xctx)
{
    ...
    p = p->managed_p;
    v = afw_pool_calloc(p, sizeof(afw_value_script_managed_slice_t), xctx);
...
/* Clone evaluated script managed in p->managed_p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_script_managed(
    const afw_value_t *value,
    const afw_pool_t *p,
...
/* Get property as script internal. */
AFW_DEFINE(const afw_utf8_t *)
...
/* Get next property as script internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_script_create_managed(value, instance->p, xctx);
    }
    else {
