...
#define AFW_IMPLEMENTATION_SPECIFIC (const void *)&afw_data_type_script_direct
...
/* unmanaged script: get_reference/release throw; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed script: optional_release drops RC; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice script: own RC; holds containing; */
...
impl_data_type_script_id_value = {
...
{
    (const afw_value_t *)&impl_data_type_script_id_value
};

static const afw_utf8_t
impl_data_type_path_script =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/script");

static const afw_value_object_t
impl_data_type_object_script__value;

/* Data type script object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_script = {
    {
        &afw_runtime_inf__AdaptiveDataType_,
        NULL,
        (const afw_value_t *)&impl_data_type_object_script__value,
        {
            NULL,
            NULL,
            &afw_self_s_script,
    ...
    (void *)&afw_data_type_script_direct
};
...
/* Permanent empty array of script. */
const afw_array_from_values_self_t
impl_empty_array_of_script;

/* Permanent empty array value of script. */
...
afw_data_type_script_direct = {
    ...
    (const afw_object_t *)&impl_data_type_object_script,
    AFW_UTF8_LITERAL("script"),
    AFW_UTF8_LITERAL("An adaptive script"),
    AFW_UTF8_LITERAL("Data type 'script' holds the source for an adaptive script."),
    AFW_UTF8_LITERAL("SourceParameter"),
    afw_data_type_number_script,
...
/* Data type struct for script. */
AFW_DEFINE_CONST_DATA(afw_data_type_t *)
afw_data_type_script =
    &afw_data_type_script_direct;

/* Set property from script value. */
AFW_DEFINE(void)
...
/* Set property from script internal. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_script_create_managed(internal, xctx);
    }
    else {
        v = afw_value_script_create(internal, object->p, xctx);
    }
...
afw_value_as_script(const afw_value_t *value, afw_xctx_t *xctx)
{
    ...
    if (!AFW_VALUE_IS_DATA_TYPE(value, script))
    {
        ...
        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'script' but "
    ...
    return (const afw_value_script_t *)value;
}
...
AFW_DEFINE(afw_value_script_t *)
afw_value_script_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_script_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_script_t),
        xctx);
    result->inf = &afw_value_unmanaged_script_inf;
    return result;
}

/* Create function for managed data type script value. */
AFW_DEFINE(const afw_value_t *)
afw_value_script_create_managed(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
    afw_value_script_managed_t *v;
    afw_size_t len;

    len = (internal) ? internal->len : 0;
    v = afw_pool_calloc(xctx->p,
        sizeof(afw_value_script_managed_t) + len, xctx);
    v->inf = &afw_value_managed_script_inf;
    v->internal.len = len;
    v->internal.s = (const afw_utf8_octet_t *)v +
        sizeof(afw_value_script_managed_t);
...
/* Create function for managed data type script slice value. */
AFW_DEFINE(const afw_value_t *)
afw_value_script_create_managed_slice(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_script_managed_t *containing;
    const afw_utf8_t *base;
    afw_value_script_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_script_inf) {
        const afw_value_script_managed_slice_t *slice =
            (const afw_value_script_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_script_inf) {
        containing =
        ...
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice script",
            xctx);
    }
    ...
    v = afw_pool_calloc(xctx->p, sizeof(afw_value_script_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_script_inf;
...
/* Create function for data type script value. */
AFW_DEFINE(const afw_value_t *)
...
/* Deep clone evaluated script unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_script_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_script_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_script, p, xctx);
    afw_data_type_clone_internal(afw_data_type_script,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_script_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated script managed in xctx->p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_script_managed(
    const afw_value_t *value,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_script_create_managed(
        &((const afw_value_script_t *)value)->internal,
        xctx);
}

/* Convert data type script string to afw_utf8_t *. */
AFW_DEFINE(void)
afw_data_type_script_to_internal(afw_utf8_t *to_internal,
...
/*  Convert data type script internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
afw_data_type_script_to_utf8(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Get property function for data type script internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_script_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as script value. */
AFW_DEFINE(const afw_value_script_t *)
...
/* Get next property function for data type script internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_script_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
    ...
    afw_xctx_t *xctx)
{
    ...
    if (self->reference_count == 0) {
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_script_managed_t) + self->internal.len, xctx);
    }
}
...
impl_afw_value_get_assignable_value(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_script_t *self =
        (const afw_value_script_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_script_managed_slice_t *self =
        (afw_value_script_managed_slice_t *)instance;
    ...
    if (self->reference_count == 0) {
        ...
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_script_managed_slice_t), xctx);
    }
}
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_script;
    info->optimized_value = instance;
}


/* Get next script value from array of script. */
AFW_DEFINE(const afw_value_script_t *)
...
/* Get next script internal from array of script. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_script_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a script value to array of script. */
AFW_DEFINE(void)
...
/* Add a script internal to array of script. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_script_create_managed(value, xctx);
    }
    else {
        v = afw_value_script_create(value, instance->p, xctx);
    }
    afw_array_push_value(instance, v, xctx);
}

/* Remove a script value from array of script. */
AFW_DEFINE(void)
