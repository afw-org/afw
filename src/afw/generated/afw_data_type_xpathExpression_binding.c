...
/* Declares and rti/inf defines for interface afw_value */
/* managed xpathExpression: optional_release frees header at RC 0; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice xpathExpression: RC on containing managed value; */
...
/* Value for empty array of xpathExpression. */
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_unmanaged_xpathExpression(internal, object->p, xctx);
...
afw_value_allocate_unmanaged_xpathExpression(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_xctx_calloc(sizeof(afw_value_xpathExpression_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_xpathExpression_inf;
    v->internal.s = base->s + offset;
    v->internal.len = len;
    v->containing_value = containing;
    ((afw_value_xpathExpression_managed_t *)containing)->reference_count++;
    return &v->pub;
}
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_value_xpathExpression_managed_t *containing =
        (afw_value_xpathExpression_managed_t *)self->containing_value;
...
impl_afw_value_managed_slice_get_reference(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_xpathExpression_managed_slice_t *self =
        (const afw_value_xpathExpression_managed_slice_t *)instance;
    afw_value_xpathExpression_managed_t *containing =
        (afw_value_xpathExpression_managed_t *)self->containing_value;
    afw_value_xpathExpression_managed_slice_t *v;
