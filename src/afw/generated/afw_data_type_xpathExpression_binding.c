...
/* unmanaged xpathExpression: get_reference/release throw; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed xpathExpression: optional_release drops RC; */
...
/* Declares and rti/inf defines for interface afw_value */
/* managed_slice xpathExpression: own RC; holds containing; */
/* last release frees slice header via xctx->p. */
#define AFW_IMPLEMENTATION_ID "managed_slice_xpathExpression"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_managed_slice_xpathExpression_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* permanent xpathExpression: optional_release NULL; */
...
impl_data_type_object_xpathExpression__value;
...
impl_data_type_object_xpathExpression = {
    {
        ...
        NULL,
        (const afw_value_t *)&impl_data_type_object_xpathExpression__value,
        {
            NULL,
            NULL,
...
/* Permanent empty array of xpathExpression. */
const afw_array_from_values_self_t
impl_empty_array_of_xpathExpression;

/* Permanent empty array value of xpathExpression. */
const afw_value_array_t
impl_value_empty_array_of_xpathExpression;
...
afw_data_type_xpathExpression_direct = {
    ...
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_xpathExpression,
    (const afw_value_t *)&impl_value_empty_array_of_xpathExpression,
...
/* Set property from xpathExpression value. */
AFW_DEFINE(void)
...
/* Set property from xpathExpression internal. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_object_is_memory_managed(object) ||
        ...
        v = afw_value_xpathExpression_create_managed(internal, xctx);
    }
    else {
        v = afw_value_xpathExpression_create(internal, object->p, xctx);
    }
    afw_object_set_property(object, property_name, v, xctx);
}

/* Typesafe cast to evaluated xpathExpression value. */
AFW_DEFINE(const afw_value_xpathExpression_t *)
afw_value_as_xpathExpression(const afw_value_t *value, afw_xctx_t *xctx)
{
    ...
    return (const afw_value_xpathExpression_t *)value;
}

/* Typesafe peel of data type xpathExpression internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_value_as_xpathExpression_internal(const afw_value_t *value, afw_xctx_t *xctx)
{
    return &afw_value_as_xpathExpression(value, xctx)->internal;
}

/* Allocate function for data type xpathExpression values. */
AFW_DEFINE(afw_value_xpathExpression_t *)
afw_value_xpathExpression_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_xpathExpression_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_xpathExpression_t),
        xctx);
    result->inf = &afw_value_unmanaged_xpathExpression_inf;
    return result;
}

/* Create function for managed data type xpathExpression value. */
AFW_DEFINE(const afw_value_t *)
afw_value_xpathExpression_create_managed(
    const afw_utf8_t * internal,
    afw_xctx_t *xctx)
{
    afw_value_xpathExpression_managed_t *v;
    afw_size_t len;

    len = (internal) ? internal->len : 0;
    v = afw_pool_calloc(xctx->p,
        sizeof(afw_value_xpathExpression_managed_t) + len, xctx);
    v->inf = &afw_value_managed_xpathExpression_inf;
    v->internal.len = len;
...
/* Create function for managed data type xpathExpression slice value. */
AFW_DEFINE(const afw_value_t *)
afw_value_xpathExpression_create_managed_slice(
    ...
    afw_xctx_t *xctx)
{
    const afw_value_xpathExpression_managed_t *containing;
    const afw_utf8_t *base;
    afw_value_xpathExpression_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_xpathExpression_inf) {
        const afw_value_xpathExpression_managed_slice_t *slice =
            (const afw_value_xpathExpression_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_xpathExpression_inf) {
        containing =
        ...
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice xpathExpression",
            xctx);
    }
    ...
    v = afw_pool_calloc(xctx->p, sizeof(afw_value_xpathExpression_managed_slice_t), xctx);
    v->inf = &afw_value_managed_slice_xpathExpression_inf;
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_pool_calloc(p, sizeof(afw_value_xpathExpression_t),
        xctx);
    v->inf = &afw_value_unmanaged_xpathExpression_inf;
    if (internal) {
...
/* Deep clone evaluated xpathExpression unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_xpathExpression_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (value->inf == &afw_value_permanent_xpathExpression_inf) {
        return value;
    }
    cloned = afw_value_common_allocate(
        afw_data_type_xpathExpression, p, xctx);
    afw_data_type_clone_internal(afw_data_type_xpathExpression,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_xpathExpression_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}

/* Clone evaluated xpathExpression managed in xctx->p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_xpathExpression_managed(
    const afw_value_t *value,
    afw_xctx_t *xctx)
{
    ...
    return afw_value_xpathExpression_create_managed(
        &((const afw_value_xpathExpression_t *)value)->internal,
        xctx);
}
...
/* Get property as xpathExpression value. */
AFW_DEFINE(const afw_value_xpathExpression_t *)
...
/* Get property function for data type xpathExpression internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_xpathExpression_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as xpathExpression value. */
AFW_DEFINE(const afw_value_xpathExpression_t *)
...
/* Get next property function for data type xpathExpression internal. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_xpathExpression_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
    ...
    afw_xctx_t *xctx)
{
    ...
    if (self->reference_count == 0) {
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_xpathExpression_managed_t) + self->internal.len, xctx);
    }
}
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_xpathExpression_managed_slice_t *self =
        (afw_value_xpathExpression_managed_slice_t *)instance;
    ...
    if (self->reference_count == 0) {
        ...
        afw_pool_free_memory(xctx->p, self,
            sizeof(afw_value_xpathExpression_managed_slice_t), xctx);
    }
}
...
/* Get next xpathExpression value from array of xpathExpression. */
AFW_DEFINE(const afw_value_xpathExpression_t *)
...
/* Get next xpathExpression internal from array of xpathExpression. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_xpathExpression_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a xpathExpression value to array of xpathExpression. */
AFW_DEFINE(void)
...
/* Add a xpathExpression internal to array of xpathExpression. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_array_is_memory_managed(instance) ||
        ...
        v = afw_value_xpathExpression_create_managed(value, xctx);
    }
    else {
        v = afw_value_xpathExpression_create(value, instance->p, xctx);
    }
    afw_array_push_value(instance, v, xctx);
}

/* Remove a xpathExpression value from array of xpathExpression. */
AFW_DEFINE(void)
afw_array_of_xpathExpression_remove(
    ...
    afw_xctx_t *xctx)
{
    afw_array_of_xpathExpression_remove_internal(instance, &value->internal, xctx);
}

/* Remove a xpathExpression internal from array of xpathExpression. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_xpathExpression_create(value, xctx->p, xctx);
