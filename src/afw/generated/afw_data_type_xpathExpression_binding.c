...
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_xpathExpression_inf
...
/* Declares and rti/inf defines for interface afw_value */
/* This is the inf for referenced xpathExpression values. For this one */
/* optional_release releases value and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "referenced_xpathExpression"
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_referenced_xpathExpression_inf
...
afw_value_allocate_xpathExpression(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_xpathExpression_t),
        xctx);
    result->inf = &afw_value_xpathExpression_inf;
    return result;
}

/* Create function for referenced data type xpathExpression value. */
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_pool_calloc(p, sizeof(afw_value_xpathExpression_t),
        xctx);
    v->inf = &afw_value_referenced_xpathExpression_inf;
    if (internal) {
...
/* Create function for referenced data type xpathExpression slice value. */
AFW_DEFINE(const afw_value_t *)
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_pool_calloc(p, sizeof(afw_value_xpathExpression_t),
        xctx);
    v->inf = &afw_value_xpathExpression_inf;
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return &v->pub;
