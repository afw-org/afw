...
class compiler_internal
{
     ...
     *                   'throw'. Must be a name allowed on script throw
     *                   (request-facing HTTP names such as not_found, denied,
     *                   gone, too_many_requests). Sets the id property of the
     *                   catch object and the HTTP status if the error is not
     *                   caught.
     *
     * @return void
     */
    public function throw(, $message, $data = null, $id = null)
    {
        $request = $this->$session->request();
