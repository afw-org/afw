...
/**
 * @file afw_data_type_x500Name_binding.c
...
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_permanent_x500Name_inf
...
impl_data_type_x500Name_id_value = {
...
{
    (const afw_value_t *)&impl_data_type_x500Name_id_value
};

static const afw_utf8_t
impl_data_type_path_x500Name =
    AFW_UTF8_LITERAL("/afw/_AdaptiveDataType_/x500Name");

static const afw_value_object_t
impl_data_type_object_x500Name__value;

/* Data type x500Name object. */
static const afw_runtime_object_indirect_t
impl_data_type_object_x500Name = {
    {
        &afw_runtime_inf__AdaptiveDataType_,
        NULL,
        (const afw_value_t *)&impl_data_type_object_x500Name__value,
        {
            NULL,
            NULL,
            &afw_self_s_x500Name,
    ...
    (void *)&afw_data_type_x500Name_direct
};
...
/* Permanent empty array of x500Name. */
const afw_array_from_values_self_t
impl_empty_array_of_x500Name;

/* Permanent empty array value of x500Name. */
const afw_value_array_t
impl_value_empty_array_of_x500Name;

/* Data type x500Name instance. */
...
afw_data_type_x500Name_direct = {
    ...
    sizeof(afw_utf8_t),
    (const afw_array_t *)&impl_empty_array_of_x500Name,
...
/* Data type struct for x500Name. */
AFW_DEFINE_CONST_DATA(afw_data_type_t *)
afw_data_type_x500Name =
    &afw_data_type_x500Name_direct;

/* Set property from x500Name value. */
AFW_DEFINE(void)
...
/* Set property from x500Name internal. */
AFW_DEFINE(void)
...
/* Typesafe cast to evaluated x500Name value. */
AFW_DEFINE(const afw_value_x500Name_t *)
afw_value_as_x500Name(const afw_value_t *value, afw_xctx_t *xctx)
{
    value = afw_value_evaluate(value, xctx->p, xctx);
    if (!AFW_VALUE_IS_DATA_TYPE(value, x500Name))
    {
        const afw_utf8_t *data_type_id;

        if (!value) {
            AFW_THROW_ERROR_Z(general,
                "Typesafe error: expecting 'x500Name' but "
...
/* Typesafe peel of data type x500Name internal. */
AFW_DEFINE(const afw_utf8_t *)
afw_value_as_x500Name_internal(const afw_value_t *value, afw_xctx_t *xctx)
{
    return &afw_value_as_x500Name(value, xctx)->internal;
}

/* Allocate function for data type x500Name values. */
AFW_DEFINE(afw_value_x500Name_t *)
afw_value_x500Name_allocate(const afw_pool_t *p, afw_xctx_t *xctx)
{
    afw_value_x500Name_t *result;

    result = afw_pool_calloc(p, sizeof(afw_value_x500Name_t),
        xctx);
    result->inf = &afw_value_unmanaged_x500Name_inf;
    return result;
}

/* Create function for managed data type x500Name value. */
    ...
    afw_xctx_t *xctx)
{
    ...
    v->internal.s = (const afw_utf8_octet_t *)v +
        sizeof(afw_value_x500Name_managed_t);
...
/* Create function for managed data type x500Name slice value. */
AFW_DEFINE(const afw_value_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    afw_value_x500Name_managed_slice_t *v;

    if (!containing_value) {
        AFW_THROW_ERROR_Z(general,
            "containing_value required for managed slice", xctx);
    }
    if (containing_value->inf == &afw_value_managed_slice_x500Name_inf) {
        const afw_value_x500Name_managed_slice_t *slice =
            (const afw_value_x500Name_managed_slice_t *)containing_value;
        containing = slice->containing_value;
        base = &slice->internal;
    }
    else if (containing_value->inf == &afw_value_managed_x500Name_inf) {
        containing =
        ...
        AFW_THROW_ERROR_Z(general,
            "containing_value must be managed or managed_slice x500Name",
            xctx);
    }
    ...
    v = afw_pool_calloc(xctx->p, sizeof(afw_value_x500Name_managed_slice_t), xctx);
...
/* Create function for data type x500Name value. */
AFW_DEFINE(const afw_value_t *)
...
/* Deep clone evaluated x500Name unmanaged into p. */
AFW_DEFINE(const afw_value_t *)
afw_value_clone_x500Name_unmanaged(
    ...
    afw_xctx_t *xctx)
{
    ...
    cloned = afw_value_common_allocate(
        afw_data_type_x500Name, p, xctx);
    afw_data_type_clone_internal(afw_data_type_x500Name,
        (void *)&cloned->internal,
        (const void *)&((const afw_value_x500Name_t *)value)->internal,
        p, xctx);
    return &cloned->pub;
}
...
AFW_DEFINE(void)
afw_data_type_x500Name_to_internal(afw_utf8_t *to_internal,
...
/*  Convert data type x500Name internal representation to utf-8. */
AFW_DEFINE(const afw_utf8_t *)
afw_data_type_x500Name_to_utf8(const afw_utf8_t * internal,
    const afw_pool_t *p, afw_xctx_t *xctx)
{
...
/* Get property as x500Name value. */
AFW_DEFINE(const afw_value_x500Name_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_property_as_x500Name_source(object, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
...
/* Get next property as x500Name value. */
AFW_DEFINE(const afw_value_x500Name_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_object_get_next_property_as_x500Name_source(object, iterator, property_name, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
impl_afw_value_managed_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_x500Name_managed_t *self =
        (afw_value_x500Name_managed_t *)instance;
...
impl_afw_value_managed_slice_optional_release(
    ...
    afw_xctx_t *xctx)
{
    afw_value_x500Name_managed_slice_t *self =
        (afw_value_x500Name_managed_slice_t *)instance;
    ...
    afw_xctx_t *xctx)
{
    ...
    info->evaluated_data_type = afw_data_type_x500Name;
    info->optimized_value = instance;
}


/* Get next x500Name value from array of x500Name. */
AFW_DEFINE(const afw_value_x500Name_t *)
...
/* Get next x500Name internal from array of x500Name. */
AFW_DEFINE(const afw_utf8_t *)
    ...
    afw_xctx_t *xctx)
{
    ...
    value = afw_array_of_x500Name_get_next_source(instance, iterator, source_z, xctx);
    if (!value) {
        return NULL;
    }
    return &value->internal;
...
/* Add a x500Name value to array of x500Name. */
AFW_DEFINE(void)
...
/* Add a x500Name internal to array of x500Name. */
AFW_DEFINE(void)
...
/* Remove a x500Name value from array of x500Name. */
AFW_DEFINE(void)
afw_array_of_x500Name_remove(
    ...
    afw_xctx_t *xctx)
{
    afw_array_of_x500Name_remove_internal(instance, &value->internal, xctx);
}

/* Remove a x500Name internal from array of x500Name. */
AFW_DEFINE(void)
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_x500Name_create(value, xctx->p, xctx);
