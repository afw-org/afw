...
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_evaluated_relaxed_json_inf
...
afw_value_allocate_relaxed_json(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_relaxed_json_t),
        xctx);
    result->inf = &afw_value_evaluated_relaxed_json_inf;
    return result;
}
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_relaxed_json(p, xctx);
    if (internal) {
...
/* Allocate function for managed data type relaxed_json values. */
AFW_DEFINE(afw_value_relaxed_json_t *)
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_managed_relaxed_json(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return &v->pub;
...
/* Allocate function for permanent data type relaxed_json values. */
AFW_DEFINE(afw_value_relaxed_json_t *)
afw_value_allocate_permanent_relaxed_json(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_relaxed_json_t),
        xctx);
    result->inf = &afw_value_permanent_relaxed_json_inf;
    return result;
}
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_permanent_relaxed_json(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return &v->pub;
