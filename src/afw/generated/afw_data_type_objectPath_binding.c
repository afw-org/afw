...
/* This is the inf for unmanaged objectPath values. For this one */
/* optional_release is NULL and get_reference returns new reference. */
#define AFW_IMPLEMENTATION_ID "objectPath"
#define AFW_IMPLEMENTATION_INF_SPECIFIER AFW_DEFINE_CONST_DATA
#define AFW_IMPLEMENTATION_INF_LABEL afw_value_evaluated_objectPath_inf
...
afw_data_type_objectPath_direct = {
    ...
    sizeof(afw_utf8_t),
    (const afw_list_t *)&impl_empty_list_of_objectPath,
    (const afw_value_t *)&impl_value_empty_list_of_objectPath,
    ...
    afw_xctx_t *xctx)
{
    ...
    v = afw_value_create_objectPath(internal, object->p, xctx);
...
/* Allocate function for unmanaged data type objectPath values. */
...
afw_value_allocate_objectPath(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_objectPath_t),
        xctx);
    result->inf = &afw_value_evaluated_objectPath_inf;
    return result;
}

/* Create function for unmanaged data type objectPath value. */
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_objectPath(p, xctx);
    if (internal) {
...
/* Allocate function for managed data type objectPath values. */
AFW_DEFINE(afw_value_objectPath_t *)
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_managed_objectPath(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return (const afw_value_t *)v;
...
/* Allocate function for permanent data type objectPath values. */
AFW_DEFINE(afw_value_objectPath_t *)
afw_value_allocate_permanent_objectPath(const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    result = afw_pool_calloc(p, sizeof(afw_value_objectPath_t),
        xctx);
    result->inf = &afw_value_permanent_objectPath_inf;
    return result;
}
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    v = afw_value_allocate_permanent_objectPath(p, xctx);
    if (internal) {
        memcpy(&v->internal, internal, sizeof(afw_utf8_t));
    }
    return (const afw_value_t *)v;
    ...
    afw_xctx_t *xctx)
{
    ...
    info->optimized_value_data_type = afw_data_type_objectPath;
}


/* Get next value from list of objectPath. */
AFW_DEFINE(const afw_utf8_t *)
...
/* Add value from list of objectPath */
AFW_DEFINE(void)
...
/* Remove value from list of objectPath */
AFW_DEFINE(void)
