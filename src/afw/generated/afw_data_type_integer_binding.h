 ...
 * @brief Generated header for adaptive data type `integer`.
 */
...
/**
 * @brief Compile-unit literal inf for data type integer.
 *
 ...
 * Compiler-only: afw_compile_literal_integer_create().
 */
AFW_DECLARE_CONST_DATA(afw_value_inf_t)
afw_value_compile_literal_integer_inf;
 ...
 */
#define afw_data_type_is_integer(_A_DATA_TYPE) \
( \
    (_A_DATA_TYPE) && \
    (_A_DATA_TYPE) == afw_data_type_integer \
)

/**
 * @brief True if _A_VALUE is an evaluated integer value.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * For evaluated values only. When true, it is safe to cast _A_VALUE to
 * `const afw_value_integer_t *`.
 * If you want to know if the value will be integer when fully
 * evaluated (not necessarily cast-safe yet), use
 * `AFW_VALUE_EVALUATES_TO_DATA_TYPE(_A_VALUE, integer, xctx)` instead.
 */
#define afw_value_is_integer(_A_VALUE) \
( \
    (_A_VALUE) && \
    (_A_VALUE)->inf->is_evaluated_of_data_type == afw_data_type_integer\
)

/**
 * @brief True if _A_VALUE is an evaluated array of integer.
 * @param _A_VALUE to test.
 * @return boolean result.
 *
 * When true, _A_VALUE is an evaluated array (`const afw_value_array_t *`)
 * whose element data type is integer.
 */
#define afw_value_is_array_of_integer(_A_VALUE) \
( \
 ...
 * @param xctx of caller.
 * @return Allocated afw_value_integer_t with unmanaged inf set.
 *
...
afw_value_integer_allocate(
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_allocate_unmanaged_integer afw_value_integer_allocate
...
afw_value_integer_create_managed(
    ...
    const afw_pool_t *p,
    afw_xctx_t *xctx);
#define afw_value_create_managed_integer afw_value_integer_create_managed

/**
 * @brief Deep clone an evaluated integer value unmanaged into p.
 * @param value evaluated integer.
 * @param p dest pool.
 * @param xctx of caller.
...
/**
 * @brief Clone an evaluated integer value managed in p->managed_p.
 * @param value evaluated integer.
 * @param p dest pool (uses p->managed_p).
 * @param xctx of caller.
...
#define afw_value_create_unmanaged_integer afw_value_integer_create

/**
 * @brief Create a compile-unit integer literal in p.
 * @param internal.
...
/**
 * @brief Public data type integer struct instance.
 *
 * Prefer afw_data_type_integer when a pointer is enough and you are not
...
/**
 * @brief Public data type integer inf.
 *
