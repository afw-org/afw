 ...
 *
 *   adapterTypeSpecific - (optional object) This is an optional object
 *       parameter with an objectType determined by the adapterType associated
 *       with the adapterId parameter. If the adapterType supports this
 *       parameter, adapter afw will have an adaptive object type with an id of:
 * 
    ...
    afw_function_execute_t *x)
{
    ...
    /* Optional adapterTypeSpecific. */
    adapterTypeSpecific = NULL;
    if (AFW_FUNCTION_PARAMETER_IS_PRESENT(6)) {
        AFW_FUNCTION_EVALUATE_REQUIRED_DATA_TYPE_PARAMETER(adapterTypeSpecific,
            6, object);
    }
    ...
    /* Add object. */
    afw_adapter_add_object(&adapterId->internal,
        &objectType->internal,
 ...
 *
 *   adapterId - (optional string) This is the adapterId of adapter containing
    ...
    afw_function_execute_t *x)
{
    ...
    /* Get object type, if adapterId and objectType is specified. */
    object_type = NULL;
    if (AFW_FUNCTION_PARAMETER_IS_PRESENT(2)) {
        ...
        if (!object_type) {
            AFW_THROW_ERROR_FZ(general, x->xctx,
                "adapterId " AFW_UTF8_K_FMT_Q
                " objectType " AFW_UTF8_K_FMT_Q
        ...
        AFW_THROW_ERROR_Z(general,
            "objectType parameter is required if adapterId is specified",
            x->xctx);
    }
 ...
 * Parameters:
 *
 *   adapterId - (string) Id of adapter containing object to delete.
 *
    ...
    afw_function_execute_t *x)
{
    ...
    /* Delete object. */
    afw_adapter_delete_object(&adapterId->internal, &objectType->internal,
 ...
 * Parameters:
 *
 *   adapterId - (string) Id of adapter containing object to retrieve.
 *
 ...
 * Parameters:
 *
 *   adapterId - (string) Id of adapter containing object to modify.
 *
    ...
    afw_function_execute_t *x)
{
    ...
    /* Modify object. */
    afw_adapter_modify_object(&adapterId->internal, &objectType->internal,
    ...
    afw_function_execute_t *x)
{
    ...
    afw_adapter_modify_needed_to_reconcile(
        &adapter_id, &object_type_id, &object_id, &entries,
 ...
 * Parameters:
 *
 *   adapterId - (string) Id of adapter containing object to replace.
 *
    ...
    afw_function_execute_t *x)
{
    ...
    /* Replace object. */
    afw_adapter_replace_object(&adapterId->internal, &objectType->internal,
    ...
    afw_function_execute_t *x)
{
    ...
    if (queryCriteria) {
        criteria = afw_adapter_query_criteria_parse_object(
            queryCriteria->internal,
            &adapterId->internal, &objectType->internal,
            journal_entry, x->p, x->xctx);
    }
    ...
    afw_function_execute_t *x)
{
    ...
    /* Optional adapterTypeSpecific. */
    adapter_type_specific = NULL;
    if (adapterTypeSpecific) {
        adapter_type_specific = adapterTypeSpecific->internal;
    }
    ...
    if (contextType) {
        ...
        if (!ctx.response_content_type) {
            AFW_THROW_ERROR_FZ(general, x->xctx,
                "Unknown contentType " AFW_UTF8_K_FMT_Q,
    ...
    afw_function_execute_t *x)
{
    ...
    /* Parse URI to get adapterId and objectType. */
    parsed_uri = afw_uri_parse(&uri->internal, true, NULL, x->p, x->xctx);
    if (!parsed_uri) {
        AFW_THROW_ERROR_Z(general, "Parameter uri is not valid", x->xctx);
    }
    ...
    if (parsed_uri->original_query.len > 0) {
        criteria = afw_adapter_query_criteria_parse_url_encoded_rql_string(
 ...
 * Parameters:
 *
 *   adapterId - (string) Id of adapter containing object to update.
 *
...
/*
 * Adaptive function: adapter_objectCallback_signature
 *
 ...
 * Parameters:
 *
 *   object - (object) This is the object from adapter or undefined if there are
