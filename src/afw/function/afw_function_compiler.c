    ...
    afw_function_execute_t *x)
{
    ...
    afw_object_set_property(result, &afw_self_s_passed, afw_value_true, xctx);
    afw_object_set_property_as_string(result,
        &afw_self_s_id, &id->internal, xctx);
    afw_object_set_property_as_string(result,
        &afw_self_s_description, &description->internal, xctx);
    afw_object_set_property_as_string(result,
        &afw_self_s_expression, &expression->internal, xctx);
    afw_object_set_property_as_string(result,
        &afw_self_s_expected,
    ...
    AFW_TRY{
        ...
        afw_object_set_property_as_string(result, &afw_self_s_result,
            afw_value_as_casted_utf8(evaluated, x->p, xctx),
            xctx);

        if (!afw_value_equal(evaluated, expected, xctx)) {
            afw_object_set_property(result, &afw_self_s_passed, afw_value_false,
                xctx);
        }
    }
        ...
        AFW_CATCH_UNHANDLED{
            ...
            /* Set error property. */
            afw_object_set_property_as_object(result, &afw_self_s_error,
    ...
    afw_function_execute_t *x)
{
    ...
    afw_object_set_property_as_string(result,
        &afw_self_s_description, &description->internal, xctx);
    afw_object_set_property_as_string(result,
        &afw_self_s_template, &template->internal, xctx);
    ...
    afw_function_execute_t *x)
{
    ...
    if (afw_value_is_any_call(AFW_FUNCTION_ARGV(3))) {
        ...
        afw_object_set_property_as_string(result,
            &afw_self_s_value, &value_source, xctx);
    }
