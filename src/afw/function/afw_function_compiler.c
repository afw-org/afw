        ...
        !((const afw_value_boolean_t *)assertion)->internal)
    {
        ...
        if (reason) {
            AFW_THROW_ERROR_FZ(assertion_failed, x->xctx,
                "Assertion failed: " AFW_UTF8_K_FMT,
                (&reason->internal));
        }
        else {
    ...
    const afw_array_t *list)
{
    ...
    out = afw_array_create_unmanaged(ctx->p, ctx->xctx);
    iterator = NULL;
    index = 0;
    for (;;) {
    ...
    afw_function_execute_t *x)
{
    ...
    afw_object_set_property_as_string_internal(result,
        afw_v_description, &description->internal, xctx);
    afw_object_set_property_as_string_internal(result,
    ...
    AFW_TRY{
        ...
        afw_object_set_property_as_string_internal(result, afw_v_result,
            afw_value_convert_to_casted_utf8(evaluated, x->p, xctx),
            xctx);

        if (!afw_value_equal(evaluated, expected, xctx)) {
            afw_object_set_property(result, afw_v_passed, afw_boolean_v_false,
                xctx);
        }
    }
    ...
    afw_function_execute_t *x)
{
    ...
    afw_object_set_property_as_string_internal(result,
        afw_v_description, &description->internal, xctx);
    afw_object_set_property_as_string_internal(result,
        afw_v_template, &template->internal, xctx);
    ...
    if (compile_type_value) {
        ...
        if (afw_utf8_compare(compile_type_string, afw_s_json) == 0)
            ...
            AFW_THROW_ERROR_FZ(bad_request, xctx,
                "Invalid compile type " AFW_UTF8_K_FMT_Q ".",
                (compile_type_string));
        }
    }
