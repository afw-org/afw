...
/**
 * @file afw_list_wrapper_for_array.c
 * @brief Adaptive Framework afw_list interface for array.
 */
...
afw_list_create_wrapper_for_array(
    ...
    afw_xctx_t *xctx)
{
    afw_list_wrapper_for_array_self_t *self;
    const afw_octet_t *ptr;

    self = afw_pool_calloc_type(p, afw_list_wrapper_for_array_self_t, xctx);
    self->inf = &afw_list_wrapper_for_array_inf;
    self->p = p;
    ...
    afw_xctx_t *xctx)
{
    ...
    if (afw_list_get_data_type(list, xctx) == afw_data_type_string) {
        return list;
    }
        ...
        internal = (void *)(((afw_byte_t *)internal) + sizeof(afw_utf8_t)))
    {
        ...
        if (!data_type) {
            AFW_THROW_ERROR_Z(general,
                "data type needed in afw_list_convert_to_list_of_strings()",
                xctx);
        }
        s = afw_data_type_internal_to_utf8(
            data_type, &((const afw_value_evaluated_t *)value)->internal,
            p, xctx);
...
{
    afw_list_wrapper_for_array_self_t * self = 
        (afw_list_wrapper_for_array_self_t *)instance;
    ...
    afw_xctx_t *xctx)
{
    ...
    if (impl_afw_list_get_entry_internal(instance,
        index, &data_type, &internal, xctx))
    {
        result = afw_value_evaluated_create(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (!p) {
        AFW_THROW_ERROR_Z(general,
            "afw_list_get_next_value() requires p",
            xctx);
    }
