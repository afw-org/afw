        ...
        if (*a && (*a)->minArgs->internal == -1) {
            ...
            /* If arg is NULL and not optional, throw error. */
                ...
                AFW_THROW_ERROR_FZ(general, xctx,
                    "Missing parameter " AFW_UTF8_K_FMT_Q,
                    (&(*a)->name->internal));
            }
        }
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    AFW_TRY{
            ...
            if (!value) {
                ...
                if (!function) {
                    AFW_THROW_ERROR_FZ(syntax, xctx,
                        "Unknown function " AFW_UTF8_K_FMT_Q,
                        (functionId));
                }
        ...
        if (!afw_value_is_array(value)) {
            AFW_THROW_ERROR_FZ(syntax, xctx,
                "Property " AFW_UTF8_K_FMT_Q " of actions is missing or invalid",
        ...
        for (iterator = NULL, action_number = 1; ; action_number++) {
            ...
            if (!functionId) {
                AFW_THROW_ERROR_FZ(syntax, xctx,
                    "Property " AFW_UTF8_K_FMT_Q " of action " AFW_INTEGER_FMT
