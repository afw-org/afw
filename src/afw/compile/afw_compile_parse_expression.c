    ...
    afw_compile_parser_t *parser)
{
    ...
    if (!result) {
        ...
        if (afw_compile_token_is(identifier)) {
             ...
             */
            if (afw_compile_token_is_name(&afw_self_s_void))
            {
...
afw_compile_parse_Lambda(afw_compile_parser_t *parser)
{
    ...
    if (afw_compile_token_is_name(&afw_self_s_function)) {
        afw_compile_get_token();
        if (!afw_compile_token_is(open_parenthesis) &&
...
afw_compile_parse_Type(afw_compile_parser_t *parser)
{
    ...
    if (afw_compile_token_is_unqualified_identifier()) {
        ...
        if (enclosed && !afw_compile_token_is(close_parenthesis)) {
            ...
            {
                /* MediaType */
                if (afw_utf8_equal(dataType, &afw_self_s_string) ||
                    afw_utf8_equal(dataType, &afw_self_s_base64Binary) ||
                    afw_utf8_equal(dataType, &afw_self_s_hexBinary))
                {
                ...
                /* ReturnType */
                    ...
                    afw_utf8_equal(dataType, &afw_self_s_script) ||
                    afw_utf8_equal(dataType, &afw_self_s_template))
                {
                    if (!afw_compile_token_is_name(&afw_self_s_void)) {
                        afw_compile_reuse_token();
                ...
                /* FunctionSignature */
                else if (afw_utf8_equal(dataType, &afw_self_s_function))
                {
                    afw_compile_reuse_token();
                ...
                /* ArrayOf */
                else if (afw_utf8_equal(dataType, &afw_self_s_array))
                {
                    ...
                    afw_compile_get_token();
                    if (!afw_compile_token_is_name(&afw_self_s_array)) {
                        list_type->dimension++;
                    }
                    else {
                ...
                /* ObjectType */
                else if (afw_utf8_equal(dataType, &afw_self_s_object) ||
                    afw_utf8_equal(dataType, &afw_self_s_objectId))
                {
                    if (!afw_compile_token_is(utf8_string)) {
                        AFW_COMPILE_THROW_ERROR_Z("Expecting quoted string");
                    }
                ...
                /* Type */
                else if (afw_utf8_equal(dataType, &afw_self_s_unevaluated))
                {
                    afw_compile_reuse_token();
...
afw_compile_parse_NullishCoalescing(afw_compile_parser_t *parser)
{
    ...
    afw_compile_args_add_value(args,
        (const afw_value_t *)&afw_function_definition_nullish_coalescing);
...
afw_compile_parse_LogicalExpression(afw_compile_parser_t *parser)
{
    ...
    args = afw_compile_args_create(parser);
    afw_compile_args_add_value(args,
        (const afw_value_t *)&afw_function_definition_or);
    afw_compile_args_add_value(args, result);
...
afw_compile_parse_LogicalAnd(afw_compile_parser_t *parser)
{
    ...
    args = afw_compile_args_create(parser);
    afw_compile_args_add_value(args,
        (const afw_value_t *)&afw_function_definition_and);
    afw_compile_args_add_value(args, result);
...
afw_compile_parse_Equality(afw_compile_parser_t *parser)
{
    ...
    case afw_compile_token_type_equal_to:
        function = (const afw_value_t *)&afw_function_definition_eq;
        break;

    case afw_compile_token_type_equal_value_and_type:
        function = (const afw_value_t *)&afw_function_definition_eqx;
        break;

    case afw_compile_token_type_not_equal_to:
        function = (const afw_value_t *)&afw_function_definition_ne;
        break;

    case afw_compile_token_type_not_equal_value_and_type:
        function = (const afw_value_t *)&afw_function_definition_nex;
        break;
    ...
    const afw_value_t *value)
{
    ...
    argv = afw_pool_malloc(parser->p, sizeof(afw_value_t *) * 3, parser->xctx);
    argv[0] = (const afw_value_t *)&afw_function_definition_subtract;
    argv[1] = value;
    ...
    for (args = NULL;;) {
        ...
        case afw_compile_token_type_add:
            if (!args) {
                args = afw_compile_args_create(parser);
                afw_compile_args_add_value(args,
                    (const afw_value_t *)&afw_function_definition_add);
                afw_compile_args_add_value(args, result);
            }
    ...
    const afw_value_t *value)
{
    ...
    function = (parser->token->type == afw_compile_token_type_divide)
        ? (const afw_value_t *)&afw_function_definition_divide
        : (const afw_value_t *)&afw_function_definition_mod;
    ...
    for (args = NULL;;) {
        ...
        case afw_compile_token_type_multiply:
            if (!args) {
                args = afw_compile_args_create(parser);
                afw_compile_args_add_value(args,
                    (const afw_value_t *)&afw_function_definition_multiply);
                afw_compile_args_add_value(args, result);
            }
    ...
    for (args = NULL;;) {
        ...
        if (afw_compile_token_is(exponentiation)) {
            if (!args) {
                args = afw_compile_args_create(parser);
                afw_compile_args_add_value(args,
                    (const afw_value_t *)&afw_function_definition_pow);
                afw_compile_args_add_value(args, result);
            }
...
afw_compile_parse_Prefixed(afw_compile_parser_t *parser)
{
    ...
    case afw_compile_token_type_unary_minus:
        argv = afw_pool_malloc(parser->p, sizeof(afw_value_t *) * 2,
            parser->xctx);
        argv[0] = (const afw_value_t *)&afw_function_definition_negative;
    ...
    case afw_compile_token_type_unary_not:
        argv = afw_pool_malloc(parser->p, sizeof(afw_value_t *) * 2,
            parser->xctx);
        argv[0] = (const afw_value_t *)&afw_function_definition_not;
        argv[1] = afw_compile_parse_Value(parser);
        result = afw_value_call_built_in_function_create(
            afw_compile_create_contextual_to_cursor(start_offset),
            1, argv, true, parser->p, parser->xctx);
