    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_TRY{
        ...
        {
            parser->compiled_value->full_source_type = &afw_self_s_json;
    ...
    afw_xctx_t *xctx)
{
        ...
        object, &iterator, &property_name, xctx)))
    {
        ...
        if (!value_data_type ||
            !afw_utf8_equal(&value_data_type->cType, &afw_self_s_afw_utf8_t))
        {
    ...
    afw_xctx_t *xctx)
{
    ...
    if (info.contextual) {
        ...
        else if (info.contextual->source_location)
        {
            ...
            if (info.contextual->value_offset == 0) {
                result = &afw_self_s_a_empty_string;
            }
            else {
                result = afw_utf8_printf(p, xctx,
                    AFW_SIZE_T_FMT,
