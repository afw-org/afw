...
impl_parse_DoWhileStatement(afw_compile_parser_t *parser)
{
    ...
    afw_compile_get_token();
    if (!afw_compile_token_is_name(&afw_self_s_while)) {
...
impl_parse_ForStatement(afw_compile_parser_t *parser)
{
    ...
    if (!afw_compile_token_is(semicolon)) {
        ...
        for (;;) {
            ...
            afw_compile_get_token();
            if (afw_compile_token_is_name(&afw_self_s_of)) {
                if (list) {
...
impl_parse_IfStatement(afw_compile_parser_t *parser)
{
    ...
    afw_compile_get_token();
    if (afw_compile_token_is_unqualified_identifier()) {
        if (afw_utf8_equal(parser->token->identifier_name, &afw_self_s_else))
        {
...
impl_parse_SwitchStatement(afw_compile_parser_t *parser)
{
    ...
    afw_compile_get_token();
    if (afw_compile_token_is_name(&afw_self_s_using)) {
    ...
    for (;;) {
        ...
        if (afw_compile_token_is_name(&afw_self_s_case)) {
            case_expression = afw_compile_parse_Expression(parser);
        }
        else if (afw_compile_token_is_name(&afw_self_s_default)) {
...
impl_parse_TryStatement(afw_compile_parser_t *parser)
{
    ...
    /* Catch */
    afw_compile_get_token();
    if (afw_compile_token_is_name(&afw_self_s_catch)) {
    ...
    /* Finally */
    afw_compile_get_token();
    if (afw_compile_token_is_name(&afw_self_s_finally)) {
        if (argc == 1) {
            argc = 2;
        }
    ...
    afw_boolean_t can_be_single_return_expression)
{
    ...
    for (;;) {
        ...
        if (end_is_close_brace_case_or_default) {
            if (afw_compile_token_is(close_brace) ||
                afw_compile_token_is_name(&afw_self_s_case) ||
                afw_compile_token_is_name(&afw_self_s_default))
            {
                afw_compile_reuse_token();
                break;
            }
    ...
    afw_boolean_t end_is_close_brace)
{
    ...
    if (afw_compile_next_raw_starts_with_z("#!")) {
        ...
        if (!afw_utf8_contains(&line, &afw_self_s_afw) &&
            !afw_utf8_contains(&line, &afw_self_s_maluba) &&     /* Easter egg */
            !afw_utf8_contains(&line, &afw_self_s_JeremyScript)) /* Easter egg */
        {
            AFW_COMPILE_THROW_ERROR_Z(
                "Shebang line must contain 'afw' to be recognized as an "
                "adaptive script");
        }
        if (afw_utf8_contains(&line, &afw_self_s_a_dash_s_test_script) ||
            afw_utf8_contains(&line, &afw_self_s_a_dash_dash_syntax_test_script))
        {
    ...
    afw_size_t *string_length)
{
    ...
    if (afw_utf8_equal(*key, &afw_self_s_expect)) {
        if (*string &&
            afw_utf8_starts_with(*string, &afw_self_s_error) &&
    ...
    afw_compile_parser_t *parser)
{
    ...
    if (afw_compile_next_raw_starts_with_z("#!")) {
        ...
        if ((!afw_utf8_contains(&line, &afw_self_s_afw) &&
            !afw_utf8_contains(&line, &afw_self_s_JeremyScript) && /* Easter egg */
            !afw_utf8_contains(&line, &afw_self_s_maluba))   /* Easter egg */ ||
            (!afw_utf8_contains(&line, &afw_self_s_a_dash_s_test_script) &&
            !afw_utf8_contains(&line, &afw_self_s_a_dash_dash_syntax_test_script)))
        {
            AFW_COMPILE_THROW_ERROR_Z(
    ...
    afw_object_set_property_as_array(test_script_object,
        &afw_self_s_tests, test_list, parser->xctx);
    ...
    for (global_source_type = NULL, test_script_id = NULL;;)
    {
        ...
        if (afw_utf8_equal(key, &afw_self_s_testScript)) {
        ...
        if (!key || afw_utf8_equal(key, &afw_self_s_test)) {
            break;
        }
        if (afw_utf8_equal(key, &afw_self_s_skip)) {
            if (afw_utf8_equal(string, &afw_self_s_true)) {
                afw_object_set_property(test_script_object,
                    key, afw_value_true, parser->xctx);
            }
            else if (!afw_utf8_equal(string, &afw_self_s_false)) {
                AFW_COMPILE_THROW_ERROR_Z(
            ...
            if (afw_utf8_equal(key, &afw_self_s_sourceType)) {
    ...
    if (!global_source_type) {
        global_source_type = &afw_self_s_script;
        afw_object_set_property_as_string(test_script_object,
            &afw_self_s_sourceType, global_source_type, parser->xctx);
    }
    afw_object_set_property_as_integer(test_script_object,
        &afw_self_s_upToTestsUTF8OctetOffsetInTestScript,
    ...
    for (test_object = NULL; ;) {
        ...
        if (!key || afw_utf8_equal(key, &afw_self_s_test)) {
            if (test_object) {
                if (!afw_object_has_property(test_object,
                    &afw_self_s_source, parser->xctx))
                {
                    AFW_COMPILE_THROW_ERROR_Z("'source:' missing");
                }
                if (!afw_object_has_property(test_object,
                    &afw_self_s_expect, parser->xctx))
                {
            ...
            afw_object_set_property_as_string(test_object,
                &afw_self_s_test, string, parser->xctx);
        }

        else if (afw_utf8_equal(key, &afw_self_s_skip)) {
            if (afw_utf8_equal(string, &afw_self_s_true)) {
                if (!test_object) {
        ...
        if (afw_utf8_equal(key, &afw_self_s_expect)) {
            ...
            afw_object_set_property_as_string(test_object,
                &afw_self_s_expectLocation, expect_location, parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_expectLineNumberInTestScript,
                    afw_safe_cast_size_to_integer(source_line, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_expectColumnNumberInTestScript,
                    afw_safe_cast_size_to_integer(source_column, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_expectCodepointOffsetInTestScript,
                    afw_safe_cast_size_to_integer(string_offset, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_expectCodepointLengthInTestScript,
                    afw_safe_cast_size_to_integer(string_length, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_expectUTF8OctetOffsetInTestScript,
                    afw_safe_cast_size_to_integer(string_offset, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_expectUTF8OctetLengthInTestScript,
                    afw_safe_cast_size_to_integer(string_length, parser->xctx),
                    parser->xctx);
        }

        if (afw_utf8_equal(key, &afw_self_s_source)) {
            afw_utf8_line_column_of_offset(
                &source_line, &source_column,
                parser->full_source,
                string_offset, 4, parser->xctx);
            source_location = afw_utf8_printf(parser->p, parser->xctx,
                ...
                string_offset, source_line, source_column);
            afw_object_set_property_as_string(test_object,
                &afw_self_s_sourceLocation, source_location, parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_sourceLineNumberInTestScript,
                    afw_safe_cast_size_to_integer(source_line, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_sourceColumnNumberInTestScript,
                    afw_safe_cast_size_to_integer(source_column, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_sourceCodepointOffsetInTestScript,
                    afw_safe_cast_size_to_integer(string_offset, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_sourceCodepointLengthInTestScript,
                    afw_safe_cast_size_to_integer(string_length, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_sourceUTF8OctetOffsetInTestScript,
                    afw_safe_cast_size_to_integer(string_offset, parser->xctx),
                    parser->xctx);
            afw_object_set_property_as_integer(test_object,
                &afw_self_s_sourceUTF8OctetLengthInTestScript,
                    afw_safe_cast_size_to_integer(string_length, parser->xctx),
                    parser->xctx);
        }
    ...
    string = afw_utf8_create_copy(
        ...
        parser->p, parser->xctx);
    afw_object_set_property_as_string(test_script_object,
        &afw_self_s_source, string, parser->xctx);
