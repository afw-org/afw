...
AFW_UTF8_LITERAL("Adapter type for using models to access objects.");


/* Model adapter factory instance. */
AFW_DEFINE_CONST_DATA(afw_adapter_factory_t)
...
static void
impl_add_adapted_properties_from_adapter(
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    object = afw_object_create_unmanaged_cede_p(
        p, xctx);
    AFW_TRY {
        ...
        impl_add_adapted_properties_from_adapter(object,
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    *to_value = from_value;
    ...
    case afw_model_adapt_to_adapter:
    ...
    case afw_model_adapt_from_adapter:
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    adapter = afw_adapter_impl_create_cede_p(
            ...
            sizeof(afw_model_internal_adapter_self_t),
            properties, p, xctx);
    self = (afw_model_internal_adapter_self_t *)adapter;

    source_location = adapter->impl->source_location;
    ...
    /* Don't allow model to be updated view a model adapter. */
        ...
        obj++, i++)
    {
        ...
        if (!obj->pub.inf) {
            AFW_THROW_ERROR_Z(general,
                "Internal error afw_model_adapter_create_cede_p()",
                xctx);
        }
    ...
    /* Make sure model location adapter is started and modelId is loaded. */
    ...
    AFW_FINALLY{
        if (model_location_adapter) {
            afw_adapter_release(model_location_adapter, xctx);
        }
    }
...
impl_afw_adapter_factory_create_adapter_cede_p (
    ...
    afw_xctx_t *xctx)
{
    return afw_model_adapter_create_cede_p(properties, p, xctx);
}
    ...
    afw_xctx_t *xctx)
{
    ...
    session->model_location_adapter = afw_adapter_get_reference(
            self->model_location_adapter_id, xctx);
            ...
            if (!cb_ctx.model_object_type) {
                ...
                if (cb_ctx.model_object_type->onRetrieveObjects) {
                    ...
                    AFW_TRY{
                        ...
                        ctx->mapBackObject_value = afw_value_function_thunk_create(
                            ...
                            adapter->mapBackObject_signature,
                        ...
                        ctx->returnObject_value = afw_value_function_thunk_create(
                            ...
                            adapter->returnObject_signature,
                ...
                if (use_default_processing) {
                    ...
                    afw_adapter_retrieve_objects(self->adapter->mapped_adapter_id,
                        ...
                        afw_object_create_unmanaged_new_p(p, xctx),
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_AUTHORIZATION_INTERMEDIATE_ACCESS_BEGIN {
        ...
        AFW_TRY {
            ...
            /* Prime context for "to adapter" (current property to mapped is superset). */
            ...
            if (use_default_processing) {
                ...
                journal_entry = afw_object_create_unmanaged_new_p(ctx->p, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    /* Process all entries. */
    for (entry = ctx->modify_entries; *entry; entry++)
    {
        ...
        {
            AFW_THROW_ERROR_Z(general,
                "Model adapter only supports modify at the entity level",
                xctx);
        }

        /* Fill the tuple, then add it. Do not push an empty wrapper first. */
        mapped_entry = afw_array_create_unmanaged(ctx->p, xctx);

        /* Add type to entry. */
        if ((*entry)->type < 0 ||
            (*entry)->type >= afw_adapter_modify_entry_type_invalid)
        {
    ...
    AFW_AUTHORIZATION_INTERMEDIATE_ACCESS_BEGIN {
        ...
        AFW_TRY {
            ...
            if (ctx->model_object_type->onModifyObject) {
                use_default_processing = false;
                entries_list = afw_adapter_modify_entries_to_list(entry,
                    ctx->p, xctx);
            ...
            if (use_default_processing) {
                ...
                afw_adapter_modify_object(
                    ...
                    ctx->mapped_object_id,
                    ctx->mapped_entries,
                    journal_entry, adapter_type_specific,
                    xctx);
            }
...
{  
    /* Create basic mapped object. */
    ctx->mapped_object = afw_model_internal_create_basic_to_adapter_mapped_object(ctx, xctx);
}
    ...
    afw_xctx_t *xctx)
{
    ...
    /* Return NULL since cached mapped adapter session handle cleanup. */
    return NULL;
}
