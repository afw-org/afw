        ...
        if (parser->criteria->object_type && entry->property_name) {
            ...
            if (!entry->pt || !entry->pt->allow_query) {
                IMPL_STRING_THROW_ERROR_FZ(
                    "Property " AFW_UTF8_K_FMT_Q " cannot be queried",
                    (entry->property_name));
            }
        }
    ...
    const afw_query_criteria_filter_entry_t *on_false)
{
    ...
    if (!rql_op) {
        IMPL_STRING_THROW_ERROR_FZ(
            "Operator " AFW_UTF8_K_FMT_Q " is not valid",
            (&parser->token));
    }
    ...
    const afw_query_criteria_filter_entry_t *on_false)
{
    ...
    if (!rql_op) {
        AFW_THROW_ERROR_FZ(general, parser->xctx,
            "Property \"op\" value is not valid: " AFW_UTF8_K_FMT_Q,
            (s));
    }
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    if (url_encoded_rql_string) {
        ...
        for (iterator = NULL;;) {
            ...
            if (!afw_value_equal(property_name, afw_v_urlEncodedRQLString, xctx)) {
                AFW_THROW_ERROR_Z(general,
        ...
        ;)
    {
        ...
        /* unknown property */
            ...
            AFW_THROW_ERROR_FZ(general, xctx,
                "Unknown query criteria property " AFW_UTF8_K_FMT_Q,
    ...
    afw_xctx_t *xctx)
{
    ...
    result = afw_array_create_unmanaged(p, xctx);
    ...
    afw_xctx_t *xctx)
{
    ...
    /* Comparision */
        ...
        if (!rql_op || !rql_op->can_be_property) {
            AFW_THROW_ERROR_FZ(general, xctx,
                "Invalid op name " AFW_UTF8_K_FMT_Q,
                (entry->op_name));
        }
