    ...
    if (!stream_anchor->streams[n]) {
        ...
        case afw_stream_number_raw_response_body:
            if (xctx->request) {
                ...
                *stream = afw_stream_fd_create(xctx->env->stdout_fd,
                    &afw_self_s_raw_response_body, false, true, true, false,
                    xctx->p, xctx);
            }
            break;
        ...
        case afw_stream_number_response_body:
            if (afw_content_type_is_application_afw(response_content_type))
            {
                *stream = afw_content_type_application_afw_stream_create(
                    ...
                    &afw_self_s_response, NULL, xctx->p, xctx);
            }
            else {
                *stream = afw_stream_standard(raw_response_body, xctx);
            }
        ...
        case afw_stream_number_console:
            if (afw_flag_by_id_is_active(
                &afw_self_s_a_flag_response_console_stream, xctx))
            {
                if (afw_content_type_is_application_afw(response_content_type))
                {
                    *stream = afw_content_type_application_afw_stream_create(
                        afw_stream_standard(raw_response_body, xctx),
                        &afw_self_s_console, NULL, xctx->p, xctx);
                }
                else {
                    AFW_THROW_ERROR_Z(general,
                ...
                &afw_self_s_a_flag_response_console, xctx))
            {
                *stream = afw_utf8_stream_create(&afw_self_s_console,
                    xctx->p, xctx);
            }
            else {
                *stream = afw_stream_fd_create(xctx->env->stdout_fd,
                    &afw_self_s_console, false, true, true, false, xctx->p, xctx);
            }
            break;

        case afw_stream_number_stderr:
            if (afw_flag_by_id_is_active(
                &afw_self_s_a_flag_response_stderr_stream, xctx))
            {
                if (afw_content_type_is_application_afw(response_content_type))
                {
                    *stream = afw_content_type_application_afw_stream_create(
                        afw_stream_standard(raw_response_body, xctx),
                        &afw_self_s_stderr, NULL, xctx->p, xctx);
                }
                else {
                    AFW_THROW_ERROR_Z(general,
                ...
                &afw_self_s_a_flag_response_stderr, xctx))
            {
                *stream = afw_utf8_stream_create(&afw_self_s_stderr,
                    xctx->p, xctx);
            }
            else {
                *stream = afw_stream_fd_create(xctx->env->stdout_fd,
                    &afw_self_s_stderr, false, true, true, false, xctx->p, xctx);
            }
            break;

        case afw_stream_number_stdout:
            if (afw_flag_by_id_is_active(
                &afw_self_s_a_flag_response_stdout_stream, xctx))
            {
                if (afw_content_type_is_application_afw(response_content_type))
                {
                    *stream = afw_content_type_application_afw_stream_create(
                        afw_stream_standard(raw_response_body, xctx),
                        &afw_self_s_stdout, NULL, xctx->p, xctx);
                }
                else {
                    AFW_THROW_ERROR_Z(general,
                ...
                &afw_self_s_a_flag_response_stdout, xctx))
            {
                *stream = afw_utf8_stream_create(&afw_self_s_stdout,
                    xctx->p, xctx);
            }
            else {
                *stream = afw_stream_fd_create(xctx->env->stdout_fd,
                    &afw_self_s_stdout, false, true, true, false, xctx->p, xctx);
            }
            break;
