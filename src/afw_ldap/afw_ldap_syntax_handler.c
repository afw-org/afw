    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    if (afw_utf8_equal(&value_data_type->cType, &afw_self_s_afw_memory_t)) {
        size = ((afw_value_hexBinary_t *)value)->internal.size;
        ptr = ((afw_value_hexBinary_t *)value)->internal.ptr;
    }
    else if (afw_utf8_equal(&value_data_type->cType, &afw_self_s_afw_utf8_t)) {
    ...
    const afw_pool_t *p, afw_xctx_t *xctx)
{
    ...
    if (afw_value_is_array(value)) {
        ...
        if (afw_utf8_equal(&data_type->cType, &afw_self_s_afw_memory_t)) {
        ...
        else if (afw_utf8_equal(&data_type->cType, &afw_self_s_afw_utf8_t)) {
            for (iterator = NULL, bv = result; ;bv++) {
                afw_array_get_next_internal(
    ...
    afw_xctx_t *xctx)
{
    ...
    if (ldap_syntax_object) {
        ...
        x_nds_syntax = afw_object_old_get_property_as_string(
            ldap_syntax_object,
            &afw_ldap_self_s_a_X_NDS_SYNTAX, xctx);
        if (x_nds_syntax) {
            impl_x_nds_syntax_handler(
