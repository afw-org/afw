    ...
    afw_xctx_t *xctx)
{
    ...
    bind_parameters = ((const afw_value_object_t *)bind_parameters_value)
        ->internal;
    self->bind_dn_z = afw_object_get_property_convert_to_utf8_z(bind_parameters,
        afw_ldap_v_dn, p, xctx);
    self->bind_password_z = afw_object_get_property_convert_to_utf8_z(bind_parameters,
