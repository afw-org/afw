...
/*
 * Implementation of afw_adapter_session interface LDAP
 *
...
/**
 * @file afw_ldap_adapter_session.c
 * @brief Implementation of afw_adapter_session interface for LDAP
 */
    ...
    afw_xctx_t *xctx)
{
    ...
    AFW_TRY {
        for (;;) {
            rv = ldap_result(self->ld, msgid, 0, &adapter->timeout, &res);
            if (rv < 0) {
                ldap_get_option(self->ld, LDAP_OPT_ERROR_NUMBER, &err);
                AFW_THROW_ERROR_RV_Z(general, ldap, err,
    ...
    afw_xctx_t *xctx)
{
    ...
    /* This adapter required object_id to be specified */
    ...
    afw_xctx_t *xctx)
{
     ...
     */
    AFW_THROW_ERROR_Z(general,
        "LDAP adapter does not support replace_object()",
        xctx);
}
