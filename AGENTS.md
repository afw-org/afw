...
- **#2 working story:** [`designs/issue-2-hold-in-inf.md`](designs/issue-2-hold-in-inf.md) (rails); two worlds [`designs/experiment-brainstorm.md`](designs/experiment-brainstorm.md) ([#277](https://github.com/afw-org/afw/issues/277)); 08-21 story [`designs/issue-2-lifetime.md`](designs/issue-2-lifetime.md). Slot protocol **landed**. Pool two-impls **landed**. Unmanaged = caller `p` / tracker; managed = this `xctx->p` + RC. Creates: `create_unmanaged` / `_new_p` / `_cede_p`; frames `create_managed`. Scope create/clone start at RC 1 (creator `release`s); `activate` / `deactivate` only push/pop. Compile facts `parent_scope_block` / `scope_depth` after the unit is complete. `get_reference` is a bump (`instance, xctx`); `get_assignable_value` fills a slot (no dest `p`). `script_result` ([#62](https://github.com/afw-org/afw/issues/62)): `return` is `as_assignable` only; statement list starts void; block finish stores a non-void last before deactivate. Eval `p` is `scope->p` when `{ }` has a frame ([PR #287](https://github.com/afw-org/afw/pull/287) / `experiment/eval-p-scope`; still caller `p` on `develop` until that lands). Pad: [`designs/experiment-eval-p.md`](designs/experiment-eval-p.md). Closures / throw-path rewind (**#35**). Archaeology: [`designs/memory-management.md`](designs/memory-management.md). Bindings still generate managed/unmanaged/slice infs — [`.cursor/rules/afw-value-memory.mdc`](.cursor/rules/afw-value-memory.mdc).
