...
- **Pools**: two impls — **heap** and **tracker** (mt = lock wrappers on heap or tracker). Files: `afw_pool.c` shared lifetime (glossary in the `@file` comment), `afw_pool_heap.c` store/scope, `afw_pool_tracker.c` tracker. Heap store is 64k-min, 4k-aligned chunks from the thread **`afw_memory_region`** (capped free list; `memoryRegionFreeListMaxBytes`, default 256KiB, **0** = posix_memalign/free every get/free). Live **block** stores `chunk *`; heap free list is LIFO with forward coalesce. One ST job heap per xctx (thread handoff off `env->p`, including base). `{ }` is `scope_create` of dest `p`. Compile omits empty `{ }` (no names, no statements). `heap_create` / `scope_create` follow parent ST/MT. `afw_pool_create()` is a heap like the parent (ST or MT), inherits `managed_p`. Tracker is `afw_pool_tracker_create()` (MT parent → lock wrappers). `afw_pool_multithread_create_as_managed_p(env->p, chunk_min)` for conf/server/log/adapter (`0` = 64k; those callers pass `small_chunk_min`, 4k). Compile units `afw_pool_heap_create` (own chunks, inherit `managed_p`; optional smaller `chunk_min`). Unmarked heap/MT create inherits `managed_p`; `*_as_managed_p` is a job heap (`managed_p = self`). Parent/child is lifetime only; store is the ancestor heap. Evaluation `{ }` is `afw_pool_scope_create` of `xctx->p` (ST heap, 4k chunks, inherits `managed_p`, last-release delay while a throw is handled). Tracker `free_memory` marks; `garbage_collect` returns marked to the heap. MT methods take the **region mutex** (not an env pool lock). `malloc_no_throw` / `calloc_no_throw` / `free_memory_no_throw` on the inf. Managed values use `p->managed_p`. `add_reference` / `release` for escaping values; bulk free when the pool or xctx is destroyed.
- **`compiled_value`**: owns a pool; script/template/test_script compile returns it **managed** (RC 1). **Everything in that unit is immutable**. Evaluation uses scope-stack discipline and `statement_flow` for leave paths; the result is pinned on dest `p`.
- **#2 working story:** [`designs/issue-2-hold-in-inf.md`](designs/issue-2-hold-in-inf.md) (rails); two worlds [`designs/experiment-brainstorm.md`](designs/experiment-brainstorm.md) ([#277](https://github.com/afw-org/afw/issues/277)); 08-21 story [`designs/issue-2-lifetime.md`](designs/issue-2-lifetime.md). Slot protocol **landed**. Pool two-impls **landed**. Unmanaged = caller `p`; managed = dest `p->managed_p` + RC. Creates: `create_unmanaged` / `_new_p` / `_cede_p`; frames `create_managed(p, xctx)`. Scope create/clone start at RC 1 (creator `release`s); `activate` / `deactivate` only push/pop. Compile facts `parent_scope_block` / `scope_depth` after the unit is complete. `get_reference` is a bump (`instance, xctx`); `get_assignable_value` / `slot_store` take dest `p`. Every remaining `{ }` is a scope ([PR #306](https://github.com/afw-org/afw/pull/306)); compile omits empty `{ }` (no names, no statements). `last_result` on the running frame; deactivate `script_result_set`s unless cloned; extra-hold at nested `{ }` adopt (unmanaged that must survive a dying scope); `return()` is `set_last_result` (pointer) — script return pin is `get_assignable_for_p_lifetime` on the caller. Clone isolates `script_result_set(original last)` then clone last stays void ([PR #307](https://github.com/afw-org/afw/pull/307)); loop `{ }` bodies do not extra-hold onto the clone. Unbraced loop bodies wrap after parse in the current block. Eval `p` is `scope->p` when `{ }` has a frame ([PR #287](https://github.com/afw-org/afw/pull/287)). Pad: [`designs/experiment-eval-p.md`](designs/experiment-eval-p.md). Leave / isolate-at-clone landed ([PR #307](https://github.com/afw-org/afw/pull/307)). Built-in lifetime hold landed ([PR #308](https://github.com/afw-org/afw/pull/308)): `get_assignable_for_lifetime` (no `last_result`) vs `set_last_result_for_lifetime`; hold the instance first then mutate; new array results `create_managed` then fill; `array()` / `create_array()` stay script wrappers in `x->p`. Managed `pop`/`shift` transfer then `afw_pool_release_value_at_cleanup` on the current scope (temp; contract on `afw_array_create_managed`). Script return pin on caller (`get_assignable_for_p_lifetime`); **no FRV wrapper**. Script/template/test_script compile returns a **managed** `compiled_value`; evaluate **pins** `script_result` on dest `p` (no `clone_unmanaged`). Pad: [`designs/compile-unit-and-frv-next.md`](designs/compile-unit-and-frv-next.md). Closures / throw-path rewind (**#35**). Archaeology: [`designs/memory-management.md`](designs/memory-management.md). Bindings still generate managed/unmanaged/slice infs — [`.cursor/rules/afw-value-memory.mdc`](.cursor/rules/afw-value-memory.mdc).
