...
- **Pools**: two impls — **heap** and **tracker** (mt = lock wrappers on heap). Heap store is 64k-min, 4k-aligned `posix_memalign` chunks. One ST job heap per xctx (`afw_pool_heap_create_as_managed_p`). `afw_pool_create()` of a ST parent is a tracker; of an MT parent, an MT heap that inherits `managed_p`. `afw_pool_multithread_create_as_managed_p(env->p)` for conf/server/log/adapter. Compile units `afw_pool_heap_create` (own chunks, inherit `managed_p`; optional smaller `chunk_min`). Unmarked heap/MT create inherits `managed_p`; `*_as_managed_p` is a job heap (`managed_p = self`). Parent/child is lifetime only; store is the ancestor heap. Evaluation `{ }` is `afw_pool_scope_create` of `xctx->p` (ST heap, 4k chunks, inherits `managed_p`, last-release delay while a throw is handled). Tracker `free_memory` marks; `garbage_collect` returns marked to the heap. `malloc_no_throw` / `calloc_no_throw` / `free_memory_no_throw` on the inf. Managed values use `p->managed_p`. `add_reference` / `release` for escaping values; bulk free when the pool or xctx is destroyed.
