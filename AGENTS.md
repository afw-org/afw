...
- **#2 working story** (not coded): [`designs/issue-2-lifetime.md`](designs/issue-2-lifetime.md) — managed = hold protocol under `xctx->p`; unmanaged = optional holds (zero does not destroy); scalar `add_reference` boxes in `xctx->p`; objects/arrays hold the instance; script mutates a face. Archaeology: [`designs/memory-management.md`](designs/memory-management.md). **Current tree** still has generated managed/unmanaged/slice infs — see [`.cursor/rules/afw-value-memory.mdc`](.cursor/rules/afw-value-memory.mdc).
