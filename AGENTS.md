...
- **#2 working story:** [`designs/issue-2-hold-in-inf.md`](designs/issue-2-hold-in-inf.md) (rails); 08-21 story [`designs/issue-2-lifetime.md`](designs/issue-2-lifetime.md). Slot protocol **landed**. Pool two-impls + last_return frame-end store **landed** (PR **#267**). `FIXME_GET_IT_WORKING`: eval still on caller `p` (ripping hung BMP comment loops). Closures / throw-path rewind (**#35**). Archaeology: [`designs/memory-management.md`](designs/memory-management.md). Bindings still generate managed/unmanaged/slice infs — [`.cursor/rules/afw-value-memory.mdc`](.cursor/rules/afw-value-memory.mdc).
