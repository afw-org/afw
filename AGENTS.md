...
- **Pools**: two impls — **heap** and **tracker** (mt = lock wrappers on heap). Heap store is 64k-min, 4k-aligned `posix_memalign` chunks. One ST heap per xctx (`afw_pool_create_xctx_p`). `afw_pool_create()` of a ST parent is a tracker; of an MT parent, an MT heap. `afw_pool_multithread_create(env->p)` for conf/server/log/adapter. Parent/child is lifetime only; store is the ancestor heap. Scopes are trackers of `xctx->p`. `add_reference` / `release` for escaping values; bulk free when the pool or xctx is destroyed.
