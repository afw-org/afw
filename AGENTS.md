...
- **Pools**: hierarchical allocation (including per-scope subpools). **Destroy is lifetime; optional `free` is reuse.** `add_reference` / `release` for escaping values (e.g. closures, assigned slots); bulk free when the pool or xctx is destroyed.
...
- **#2 working story:** [`designs/issue-2-lifetime.md`](designs/issue-2-lifetime.md) — destroy is lifetime; optional `free` is reuse. Slot protocol **landed** (`issue-2-slot-protocol`): assign / last-`release` walk / `for` and for-of `let`/`const` clone holds; scalar boxing in `xctx->p`; arrays match objects; overlay `set` on faces. **Live pool is still develop**; wrap-APR parked as `afw_pool_apr_*`. Archaeology: [`designs/memory-management.md`](designs/memory-management.md). Bindings still generate managed/unmanaged/slice infs — [`.cursor/rules/afw-value-memory.mdc`](.cursor/rules/afw-value-memory.mdc).
