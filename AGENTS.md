...
- **Pools**: two impls — **heap** and **tracker** (mt = lock wrappers on heap). Heap store is 4k-aligned `posix_memalign` chunks. `afw_pool_create()` of a heap parent is a heap; of a tracker parent is a tracker (extra rule). `xctx->p` is a single-thread heap; `env->p` is mt heap. Scopes use trackers. `add_reference` / `release` for escaping values; bulk free when the pool or xctx is destroyed.
