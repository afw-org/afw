...
| **Pool-centric lifetime** | Memory is hierarchical pools (request/`xctx`/scope). **Destroy of a pool is lifetime; optional `free` is reuse.** Two worlds: unmanaged in caller `p`; managed in dest `p->managed_p` + RC. Slot fill uses `get_assignable_value` (dest `p`). Rails: [`issue-2-hold-in-inf.md`](issue-2-hold-in-inf.md). |
...
- **Two worlds.** **Unmanaged** lives in caller `p` / `scope->p` and dies with that pool. **Managed** lives in dest `p->managed_p` and dies on last RC, then `free_memory` of the header via the stored p. Unmanaged object/array **value** `get_reference` / `release` throw; isolate with `get_assignable_value` (dest `p`).
- **Creates.** Pool-world object/array: `create_unmanaged` / `_new_p` / `_cede_p`. **`create_managed(p, xctx)`** is a frame in `p->managed_p`. Do not stamp `managed_*_inf` on a pool-world header.
- **Permanent** / **compiled unit** = immutable; holds are no-ops on permanents. A script/template/test_script unit is a **managed** `compiled_value`. **Everything in a compiled unit is immutable.** Script mutates a **face** over literals, not the compiled instance.
- **Eval `p`** is `scope->p` when `{ }` has a frame. Nested empty `{ }` is not a frame. Temps land on that pool and die with last-release. `{ }` is `scope_create` (ST heap, 4k, inherit `managed_p`).
...
Scopes (`afw_pool_scope_t`) are the scope pool: `scope->p` is that object, and **`frame_slots[]`** are the tail of the pool self, sized from the block and indexed by `afw_value_block_symbol_t.index`. Compile facts `parent_scope_block` / `scope_depth` (after the unit is complete) are the frame tree, not syntax `parent_block` / `depth`. Create/clone start at reference count 1. Activate bumps that count and pushes; deactivate pops and releases. Every `{ }` is a scope. Resolution indexes `frame_slots[]` by `scope_depth` + index. Assignment (`let` / `const` / assign) is `slot_store` (`get_assignable_value` then `release` occupant); **read** is a pointer copy. Last `release` at 0 walks `frame_slots[]`, releases the parent lexical scope, then releases the pool. A scope holds its parent from create until teardown. Closures keep a reference to the scope. **`last_result`** is a pointer on the running frame; nested `{ }` is void (parent extra-holds); `return()` extra-holds on current; deactivate `script_result_set`s unless the scope was cloned.
...
**Lifetime (two worlds, 2026-09):** almost everything dies with a **pool**. Request/`afw` command: **destroy `xctx->p`** is the safety net. Unmanaged temps live in caller `p` / `scope->p`. Managed values live in dest `p->managed_p` and last-release `free_memory`. Frames are `create_managed(p, xctx)`. Child pools hold their parent; **no reparent** on destroy. Long-running scripts need **`get_reference` / `get_assignable_value` / `release`** on slots — not “the request was long enough.” Unmanaged object/array **value** methods throw; isolate with `get_assignable_value`. Eval `p` is `scope->p` when `{ }` has a frame. Evaluate of a compiled unit **pins** the result on dest `p`. Script faces isolate compiled/adapter bases. Detail: [`experiment-brainstorm.md`](experiment-brainstorm.md), [`experiment-eval-p.md`](experiment-eval-p.md), [`issue-2-hold-in-inf.md`](issue-2-hold-in-inf.md).
