...
**Built-in returns ([PR #308](https://github.com/afw-org/afw/pull/308)):** `afw_pool_scope_get_assignable_for_lifetime` is `get_assignable` plus release on current `scope->p`. It does **not** write `last_result`. Mutating builtins (`push` / `unshift` / `add_entries` / `add_properties` / `freeze`): hold the **instance** first, write that `internal`, return that value. New array results (`bag` / `filter` / `map` / `sort` / `slice` / `reverse` / …): `create_managed`, hold, fill, return; stay **mutable**. Language constructors `array()` / `create_array()` stay `create_script_wrapper` in `x->p` (temps; `[i]` compiles to `array()`). `pop`/`shift` still return the occupant, not the array. Do not wrap unmanaged at the return of compiler `test_*` / `qualifier()` / `parse_uri` (those are temps in `x->p`).
