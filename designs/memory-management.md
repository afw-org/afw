...
**#2 is a retrofit of that model**, not a green-field allocator. Adaptive Script added **scope** lifetimes (nested frames, assign, leave paths, closures that escape a scope) **inside** a request — and long-running hosts also need values that **survive** a request (registries, permanents, retained results). Request-end bulk free still works for the outer shell; it is not enough for scope-correct hold/release. Work lands as **many feature branches** off `develop` (faces, catalog, permanent `afw_v_*`, dual `->value`, …); there will be more. Pools discussion next; server/request framing is the outer shell.
...
| 2026-08-20 | **Another #2 feature branch (discussed, not coded):** object **property names** become `const afw_value_t *`; implementations use `afw_value_equal`; store-as-is (clone of names is later #2). Script + JSON/YAML/UBJSON **string names only** (throw, no `as_utf8`). Pad: [`issue-2-property-name-values.md`](issue-2-property-name-values.md). | Not the first #2 branch and not the last. Values-first: names join the same world as property values. Generated `afw_v_*` already exist for the common call sites. |
...
| **Names as values** | Object property names `const afw_value_t *` (script/JSON string-only). Another #2 feature branch; more will follow | **discussed** 2026-08-20 — [`issue-2-property-name-values.md`](issue-2-property-name-values.md); execute when asked |
...
### 2026-08-20 — property names as values (another #2 feature branch)

- #2 continues as many feature branches; this is not a first cut. The campaign is a **retrofit**: request-lifetime bulk free plus Adaptive Script **scope** lifetimes.
...
- Pad: [`issue-2-property-name-values.md`](issue-2-property-name-values.md). No code until execute is requested.
