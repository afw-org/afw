...
**Payloads are not values.** `afw_utf8_t` / `afw_memory_t` / `afw_integer_t` do not know a pool. `create` / `set` / `no_copy` / `ks` naming for those structs: [`c-naming-and-payloads.md`](c-naming-and-payloads.md). Do not put `managed` on utf8/memory payloads; that word stays on value infs and object/array instances (#2).
