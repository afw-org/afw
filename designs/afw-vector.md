...
- **Next APR sitting** (not a new container): `apr_filepath_*` (`afw_file_path.c`, `afw_file.c`, `afw_vfs_adapter*.c` — `SECUREROOT`); `apr_strerror` (`afw_file.c`, `afw_environment_register_core.c`) until file/VFS stop returning APR `rv`.
- Portability layer (not a drop-in like vector): pools (`apr_pool_*`, `apr_palloc` / `apr_pcalloc`), files/dirs, threads/locks, getopt, LDAP apr-util, curl brigades. Time/UUID/random/DSO/fnmatch/signal/cwd are `afw_os_*` (or portable C on those doors).
