...
- **Next APR sitting: pools.** Heap/tracker still allocate from an APR reservoir (`apr_pool_create` / `apr_palloc` / `apr_pool_destroy`). `apr_initialize` / `apr_terminate` at env create. `afw_pool_get_apr_pool()` is the leftover-APR door (xctx/stream/vector-unhandled/backtrace still `apr_pcalloc`). `apr_strerror` stays while those `rv`s remain. That sitting is what can drop `apr-1`.
