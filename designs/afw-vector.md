...
- **Threads/locks** on branch `reduce-apr-threads`: `afw_os_mutex_*` / `rwlock_*` / `thread_*` in `os/nix` (pthreads); public names stay `afw_thread_mutex_*` / `afw_thread_rwlock_*`. Create takes `afw_pool` (cleanup destroys the pthread object). Nested = recursive; unnested = errorcheck. C11 `threads.h` is not the path. LMDB `dbLock` uses the AFW rwlock door (same lock order).
- **Next APR sitting:** GNU `getopt_long` (`afw` + command skeleton) **or** leftover `apr_strerror` (only while some APR `rv` remains). Does not unlock dropping `apr-1`.
- Portability layer (not a drop-in like vector): pools (`apr_pool_*`, `apr_palloc` / `apr_pcalloc`), getopt, LDAP apr-util, curl brigades. Time/UUID/random/DSO/fnmatch/signal/cwd/realpath/mutex/rwlock/thread are `afw_os_*`. Path join is `afw_file_path_*`; host file I/O is `afw_file_*`.
