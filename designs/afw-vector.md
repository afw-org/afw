...
- **Filepath + file/dir I/O:** `apr_filepath_merge` is `afw_file_path_*`; `apr_file_*` / `apr_dir_*` / `apr_stat` are `afw_file_open` / `read` / `write_full` / `stat` / `mkdir_p` / dir listing in `afw_file_io.c`. Adapter impl is `afw_file_adapter.c`. **Next:** `apr_strerror` stays for lock/thread APR `rv` until those move.
