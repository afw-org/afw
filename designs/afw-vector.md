...
- **Next APR sitting:** leftover `apr_strerror` (only while some APR `rv` remains). Does not unlock dropping `apr-1`.
- Portability layer (not a drop-in like vector): pools (`apr_pool_*`, `apr_palloc` / `apr_pcalloc`), LDAP apr-util. Time/UUID/random/DSO/fnmatch/signal/cwd/realpath/mutex/rwlock/thread are `afw_os_*`. Path join is `afw_file_path_*`; host file I/O is `afw_file_*`. Curl default body is the memory writer. Command argv is `afw_getopt_*`.
