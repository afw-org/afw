...
- **Next APR sitting** (not a new container): `apr_filepath_*` (`afw_file_path.c`, `afw_file.c`, `afw_vfs_adapter*.c`, `afw_environment.c` cwd); `apr_fnmatch` (`afw_vfs_adapter_session.c` — documented glob semantics, do not silently change); `apr_strerror` (`afw_file.c`, `afw_environment_register_core.c`); `log_deprecated.c` island (`apr_pstrdup` / `pvsprintf` + APR files). Pools stay the portability layer.
