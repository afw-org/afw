...
- **Next APR sitting** (not a new container): `apr_filepath_*` (`afw_file_path.c`, `afw_file.c`, `afw_vfs_adapter*.c`, `afw_environment.c` cwd); `apr_fnmatch` (`afw_vfs_adapter_session.c` — documented glob semantics, do not silently change); `apr_strerror` (`afw_file.c`, `afw_environment_register_core.c`). Pools stay the portability layer. `log_deprecated` island removed (no callers; prune still denylists leftover installs).
- Portability layer (not a drop-in like vector): pools (`apr_pool_*`, `apr_palloc` / `apr_pcalloc`), files/dirs, threads/locks, time, UUID/random/atomics, DSO, getopt/signal, LDAP apr-util, curl brigades.
