...
- **Filepath** sitting: `apr_filepath_merge` replaced by `afw_file_path_canonicalize` / `afw_file_path_join_under_root` (`realpath` via `afw_os_realpath`). Shared by rootFilePaths and VFS `vfsMap`. **Next:** `apr_strerror` (`afw_file.c`, `afw_environment_register_core.c`) until file/VFS stop returning APR `rv`.
- Portability layer (not a drop-in like vector): pools (`apr_pool_*`, `apr_palloc` / `apr_pcalloc`), files/dirs, threads/locks, getopt, LDAP apr-util, curl brigades. Time/UUID/random/DSO/fnmatch/signal/cwd/realpath are `afw_os_*` (or portable C on those doors). Path join/containment is `afw_file_path_*`, not APR.
