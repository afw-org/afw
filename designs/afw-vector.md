...
- **Next APR sitting** (not a new container): `apr_filepath_*` (`afw_file_path.c`, `afw_file.c`, `afw_vfs_adapter*.c`); `apr_strerror` (`afw_file.c`, `afw_environment_register_core.c`). Pools stay the portability layer. `log_deprecated` island removed (no callers; prune still denylists leftover installs).
