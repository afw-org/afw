...
| **A new error gets a test that would have caught it** | Mike, over and over: if we **have tests** and then **discover a new error**, the fix is not done until the gate has a case that fails without the fix — **and anything related you can think of** (sibling functions, empty/limit/end paths, the other impl of the same interface). A green suite that never saw the bug will not catch it next time. Pairs with *Tests are pretend mode…*. Day-to-day: `.cursor/rules/afw-tests.mdc` (*Regression tests*). |
...
| **Decompile/compile round-trip when the change can break it** | Compiled-form fidelity, not pretty source recovery. If parse, decompile, a value kind, type syntax, or `#Name` accept/emit could change the trip, run `d1 = decompile(compile(src)); d2 = decompile(compile(d1)); assert(d1 == d2)` and evaluate when the original had a result. Keep that case in the gate. Inventory: [`decompile-compiler-internal-inventory.md`](decompile-compiler-internal-inventory.md). Known non-round-trip: `#closure_binding`, `#function_thunk`. |
...
| **Don’t let the right way get in the way of getting work done** | Compiler professor, last day of class: you learned the right way to write a compiler; do not let that stop you from shipping a compiler. **Use patterns; break them when the simpler or clearer thing is the work.** People stick so close to a methodology that they take the complicated path; someone writes a book about avoiding that trap; the next loop treats the book as law. Same spirit as *Process should make you more efficient…*. Assistants: follow a house pattern when it is the simple story; do not add a second inf, layer, or ceremony just so the naming rule stays pretty. |
...
- **Methodology as law** — taking the more complicated path so it matches the pattern, when a simpler judgement is already the work (see *Don’t let the right way get in the way…*).
