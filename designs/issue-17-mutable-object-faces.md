...
- **Managed face pin (2026-08, #2-adjacent):** managed object faces `get_reference(wrapped)` at create; drop when face pool destroyed (`afw_pool_release` returns NULL). Unmanaged faces still borrow. Array faces still pool-owned/noop release — same pin when arrays get real managed RC.
