...
- Cards stay **drafts** so we do not mint one GitHub issue per finding. Public `afw` gets **one** issue (and later PR) when that card is `public`. Put those numbers in the card’s **Public** field as **`owner/repo#N`** (`afw-org/afw#190` / `afw-org/afw#191`) so Board links resolve to public `afw`, not the private how-to repo. Bare `#N` is this-repo. Keep Public visible on the Board — drafts cannot use Linked pull requests.
