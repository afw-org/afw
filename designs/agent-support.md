...
- Cards stay **drafts** so we do not mint one GitHub issue per finding. Public `afw` gets **one** issue (and later PR) when that card is `public`. Put a **full URL** in the card’s **Public** field (`https://github.com/afw-org/afw/issues/N` and/or `/pull/M`). Project text fields do not run issue autolinks (`#N` / `owner/repo#N` go to the wrong place). Keep Public visible on the Board — drafts cannot use Linked pull requests.
