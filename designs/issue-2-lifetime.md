...
4. **Script wrapper overlay holds** — `set` / last-`release`; wrap every script-mutable path we missed. (Started on `issue-2-slot-protocol`: memory object `set_property` and array push/insert/set/remove use slot protocol on the **stored overlay** pointer, not look-through get. pop/shift still transfer. Missed wraps are a later pass.)
