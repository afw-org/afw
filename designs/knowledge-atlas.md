...
| **Probe** | Targeted `.as` + `afwdev test -j --env-mode valgrind`; orchestrated multi-request leaves; **#2 hard-loop RSS lab** `src/afw/tests-extra/issue-2/` (`afwdev test -T src/afw/tests-extra/issue-2 --show-all`, gdb helpers in that README); never “fix memory” without a metric/story |
| **Open** | Umbrella **#2**. Slot protocol landed. Pool split landed. **Names as values** landed (PR **#220**). Closures / throw-path rewind (**#35**) store-time bind. Script wrapper overlay holds + create-at-0 landed (`issue-2-script-wrapper-holds`). Remaining: optional `free`, first-fit/P3. Debug probes ([#242](https://github.com/afw-org/afw/issues/242)): `AFW_DEBUG_EVALUATION` / `LOCK` / `POOL` (`--cdev`/`--fulldev`); flags `debug:evaluation` / `debug:lock` / `debug:pool`; `env->pool_bytes_allocated` (includes heap/tracker APR skeletons); `>debug pool` `rss` is current KB (`afw_os_get_rss` / `/proc/self/statm`), not `ru_maxrss`. Do not `flag_set` them on RSS soaks. |
