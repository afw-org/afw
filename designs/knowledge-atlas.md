...
| **Open** | Umbrella **#2**. Slot protocol landed. Pool split landed (`issue-2-pool-heap`: general APR vs heap/tracker, `managed_p`). **Names as values** landed (PR **#220**). Remaining: closures / throw-path scope rewind (**#35**), object-literal method closures, optional `free`. |
