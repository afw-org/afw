...
| **Open** | Umbrella **#2** still open. **Scoreboard (how close):** [`issue-2-hold-in-inf.md`](issue-2-hold-in-inf.md) *Live status (2026-09-16)*. Lab table **2026-09-16**: [`01-rss-hard-loops/README.md`](../src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md). `function_return` **flat**. Nested `{ }` that wrote `script_result` **clears** parent last (do not plant). `try_catch` ~0.25 MiB/s is OS backtrace. Two worlds pad / FRV history are maps, not the scoreboard. |
