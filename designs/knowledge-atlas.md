...
| Heap chunk reuse | Thread `afw_memory_region`; `memoryRegionFreeListMaxBytes` ([#358](https://github.com/afw-org/afw/issues/358)). Record is the issue body. Pool impl split: `afw_pool.c` shared, `afw_pool_heap.c` store, `afw_pool_scope.c` the `{ }` frame (the scope object is that pool), `afw_pool_tracker.c` tracker. A heap or tracker pins its parent only while `get_reference` holds the child; a scope holds its parent from create until teardown. A thread pool (`afw_pool_thread_create`, inf `impl_afw_pool_thread_inf`) holds its parent the same way, and its `get_reference` does not pin that parent. Live **block** stores `chunk *`; heap free list is LIFO with forward coalesce (no `first_chunk` walk). Glossary in `afw_pool.c`. `afw_pool_multithread_create_as_managed_p(parent, chunk_min, xctx)`: `0` is `env->default_chunk_min` (64k). Service, adapter, log, and conf pass `small_chunk_min` (4k). `env->p` and `xctx->p` stay on the 64k floor. `afwdev test -j` harvests `poolChunkBytes`. Compile omits empty `{ }` (no names, no statements). Process-wide region hits/misses/free-list on `process::` and `_AdaptiveServer_/current`. Dual-ended carve of a chunk (small low, large high) is later, if `managed_p` fragmentation shows. |
| Crypto | #74 pad |
...
| Field | Content |
|-------|---------|
| **Settled map** | Values first. **Two worlds [#277](https://github.com/afw-org/afw/issues/277) closed** (PR **#278**): [`experiment-brainstorm.md`](experiment-brainstorm.md) — unmanaged in caller `p`; managed in dest `p->managed_p` + RC; evaluate of a compiled value **pins** on dest `p`. **Eval `p` = `scope->p` when `{ }` has a frame** ([PR #287](https://github.com/afw-org/afw/pull/287) / [`experiment-eval-p.md`](experiment-eval-p.md)). Inf methods: [`issue-2-hold-in-inf.md`](issue-2-hold-in-inf.md) (`get_reference` bump, `xctx` only; `get_assignable_value` / `slot_store` take dest `p`). 08-21 story (history): [`issue-2-lifetime.md`](issue-2-lifetime.md). Payloads: [`c-naming-and-payloads.md`](c-naming-and-payloads.md) |
...
| **Probe** | Targeted `.as` + `afwdev test -j --env-mode valgrind`; orchestrated multi-request leaves; **#2 extra lab** `src/afw/tests-extra/issue-2/` — live soak table [`01-rss-hard-loops/README.md`](../src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md); dest-p pin `eval-pin` / model-on-file `model-lifecycle` / overnight smoke `overnight-soak`; BMP comment sweeps in default `test -j` (`src/afw/tests/test262/comments-bmp-*.as`); `03-eval-p` probes; never “fix memory” without a metric/story |
| **Open** | Umbrella **#2** still open. **Scoreboard:** [`issue-2-hold-in-inf.md`](issue-2-hold-in-inf.md) *Live status (2026-09-19)*. Isolate sitting [PR #340](https://github.com/afw-org/afw/pull/340). [#341](https://github.com/afw-org/afw/issues/341) OS backtrace leftover **closed** ([PR #354](https://github.com/afw-org/afw/pull/354); managed hexBinary; `try_catch` soak flat). Compiled-value managed + dest `p` + evaluate pin: [PR #355](https://github.com/afw-org/afw/pull/355). Lab table **2026-09-17**. Thread display/workers is [#343](https://github.com/afw-org/afw/issues/343) (not a #2 close gate). |
...
| **Open** | Index create/txn residuals (#54/#57); more adapter types; RQL paging if we want it (#49); catalog `composite`/`normalize` cost **[#331](https://github.com/afw-org/afw/issues/331)** (`check_object_errors.as` / `object_options.as`). **[#366](https://github.com/afw-org/afw/issues/366)** repeating sequential model-swap pool step is gone on `service-polish` (metrics object in the adapter pool, `afw_environment_get_lock` reads the lock registry, file journal anchor lock is reused). The first swap still adds about 42 KiB once: the location pool grows from the boot `SoakSwap.json` compile (209072 bytes) to `soak_swap_model()` (238464, +29392) because that script adds `onModifyObject` and `onDeleteObject`. Later swaps compile the same shape. Memory-region chunks are mmap/munmap; after the per-thread free lists warm, that probe stops adding RSS. Issue body is the record. Do not put that swap in the firehose. |
...
| UTF-8 / memory C doors | [`c-naming-and-payloads.md`](c-naming-and-payloads.md); **`src/afw/utf8/afw_utf8.h`** (printf) | `create` copies; `no_copy` points; `ks` `^hex^`; printf `%ku` / `%ks` / `create` not whole-buffer `ks` ([#314](https://github.com/afw-org/afw/issues/314)); error dump `printf_ks` (`%s`/`%ku` encode); `src/afw/code_point/`. |
...
|-----|----------------|----------|
| Leak / long-run “first 15 minutes” support card | agent-support + `src/afw/tests-extra/issue-2/` RSS/gdb lab | Medium (#2 active); lab leaf landed |
