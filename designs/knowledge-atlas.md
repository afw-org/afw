...
| **Open** | Umbrella **#2** still open. **Landed on `develop`:** slot protocol; pool two-impls (**#267**); two worlds **#277** closed; compile-literal + intern **#280**; scope RC 1 + `parent_scope_block` / `scope_depth`; dest `p` ripped; `script_result` ([#62](https://github.com/afw-org/afw/issues/62) closed); names as values (PR **#220**); closures / throw-path rewind (**#35** closed); eval `p` **#287**; compile() is a unit **#305**; every `{ }` is a scope + `last_result` on the running frame **#306**. RSS soaks for assign / overlay / rebind / empty `{ }` are **flat** (2026-09-07); live table [`src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md`](../src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md). **Still climbing:** `function_return` (FRV unique consume leaves wrapper in caller `p`) and `array_push_pop` (managed vector slot in `xctx->p` + pop transfer). **Next:** FRV stack leftover, then runtime managed-return inf — [`compile-unit-and-frv-next.md`](compile-unit-and-frv-next.md). **Parked (not those soaks):** skip `double_free_throws`; adapter clones; clone-of-unmanaged meta; Adaptive `clone()` still the old function; unevaluated clone-out; `qualifier("current")` snapshot tail. Debug flags `AFW_DEBUG_EVALUATION` / `LOCK` / `POOL` (**#242** closed). Heap free-list mixed sizes is **not** the remaining eval win (re-measured after #287: ~0.05s). Overlay `o.x = i` / `o = { n: i }` are not parked leak verticals. Timings: [`experiment-brainstorm.md`](experiment-brainstorm.md). |
