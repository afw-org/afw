...
| **Open** | Umbrella **#2** — retrofit of request-lifetime MM for script **scope** lifetimes; many feature branches, more to come; parent of closed #149. **Names as values** coded on `issue-2-property-name-values` (script/JSON string-only; later `afw_value_string_t` wrap pass) — [`issue-2-property-name-values.md`](issue-2-property-name-values.md). Also: array managed/unmanaged pools; `create_array` still ignores `options` (see memory pad 2026-08-16). Allocator overflow shipped as a local check. Optional free-list splice waits on **#2** (naive `else` livelocks first-fit). Candidate wrap-APR pool: memory pad **Candidate: back toward old afw_pool**. |
