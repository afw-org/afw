...
| UTF-8 / memory C doors | [`c-naming-and-payloads.md`](c-naming-and-payloads.md); **`src/afw/utf8/afw_utf8.h`** (printf) | `create` copies; `no_copy` points; `forced_safe` `^hex^`; printf `%ku` / `%ks` / `create` not whole-buffer `forced_safe` ([#314](https://github.com/afw-org/afw/issues/314)); `src/afw/code_point/` |
