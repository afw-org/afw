...
| **Open** | Umbrella **#2**. Slot protocol landed. Pool two-impls + last_return frame-end store on `develop` (PR **#267**). **Names as values** landed (PR **#220**). Closures / throw-path rewind (**#35**). Script wrapper overlay holds. Return temps (`function_return_value`; `evaluate()` consumes; parameter slot parks occupant). **`FIXME_GET_IT_WORKING`:** eval still on caller `p` — ripping it hung `comments-bmp-slash-0.as`. Skip `double_free_throws`. Do not revive `issue-2-managed-p`. Debug probes ([#242](https://github.com/afw-org/afw/issues/242)): `AFW_DEBUG_EVALUATION` / `LOCK` / `POOL`. |
