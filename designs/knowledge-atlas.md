...
| **Open** | Umbrella **#2**. Slot protocol landed. Pool two-impls **landed**. Two worlds **#277** (`create_unmanaged` / `create_managed` frames; last_return is the slot). **Names as values** landed (PR **#220**). Closures / throw-path rewind (**#35**). **`FIXME_GET_IT_WORKING`:** eval still on caller `p` — `scope->p` hung `comments-bmp-slash-0.as`. Skip `double_free_throws`. Debug probes ([#242](https://github.com/afw-org/afw/issues/242)): `AFW_DEBUG_EVALUATION` / `LOCK` / `POOL`. **Later:** heap free-list mixed sizes (concat string + integer last_return in one loop — [`experiment-brainstorm.md`](experiment-brainstorm.md) timings). |
