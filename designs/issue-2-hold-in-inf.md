...
Hard-loop soaks remeasured **2026-09-16**; isolate sitting landed on `develop` as [PR #340](https://github.com/afw-org/afw/pull/340) (`d9cb1583`). Assign / overlay / rebind / empty `{ }` / unbraced loop body / `array_push_pop` / **`function_return`** / `let`/`const` empty `f()` **flat**. Nested `{ }` that wrote `script_result` **clears** parent last instead of planting the occupant. Try returns current last; `keep_return` extra-holds a real `return`. `try_catch` still ~0.25 MiB/s in_use — OS backtrace on `xctx->p` every `throw` ([#341](https://github.com/afw-org/afw/issues/341)). Table: [`src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md`](../src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md). Gate 2026-09-16: `./afwdev build --fulldev` + `afwdev test -j` + valgrind **4512 passed**, 71 skipped.
...
|------|---------------------|
| **Evidence** | **2026-09-16** on `develop` ([PR #340](https://github.com/afw-org/afw/pull/340)). Lab table rewritten. `function_return` **flat**. Nested `{ }` last plant dropped. Server soaks not remeasured (`02` / `07` / `07b` still 2026-09-15). |
| **OS backtrace leftover** | [#341](https://github.com/afw-org/afw/issues/341). `try_catch` ~0.25 MiB/s in_use. Release-on-overwrite; do **not** skip capture. |
