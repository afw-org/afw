...
- Unique FRV consume leaves the wrapper in `self->p`. Every `{ }` is a frame now, so `i = f()` leftover sits in the body tracker until last-release — still climbs (`function_return` soak). Do not paper over it with a helper around assign or with extra-hold at `return()`. Product tests (`return_temps.as`) stay; the soak is the leak lab. Next: [`compile-unit-and-frv-next.md`](compile-unit-and-frv-next.md).
