...
**How close (scoreboard):** [Live status](#live-status-2026-09-17--how-close-is-2) below. If this pad’s older “Order / Parked” lists, the RSS lab README, the two-worlds pad, or the GitHub issue body disagree with that section, **the live status wins** until someone updates it.
...
## Live status (2026-09-17) — how close is #2?
...
Adaptive `clone()` still calls `afw_value_clone()` (script `clone()` already extra-holds; aligning with `clone_unmanaged` / `clone_managed` is C-surface). Adapter clones / clone-of-unmanaged meta. `qualifier("current")` snapshot tail. `double_free_throws` skip (wrong throw prefix). mmap / per-chunk free lists. Heap mixed-size rewrite **withdrawn** (14k concat nest ~0.05s after #287). Do not add `get_base`. Do not mint a mutable face over `environment::`, `process::`, `request::`, `application::`, or adapter/custom conf objects. Overlay `o.x = i` / `o = { n: i }` are **not** parked leak verticals — those soaks were flat.
