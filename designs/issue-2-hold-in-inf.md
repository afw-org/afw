...
**last_return** (code today: `xctx->script_result`). Last non-void statement. Not every assign poking a slot. for init/increment are not body statements. A small `replace` / `afw_xctx_set_last_return` is enough: release occupant if appropriate, `*place = get_assignable_value(value, dest_p, xctx)`. Dest for last_return is **`xctx->p`**. Dest for a frame name is **that frame’s `scope->p`**. No extra word “box.” `release` of void/permanent is a no-op. If the **incoming** is void, **do not replace** `&place` (keep previous) — same idea as ES `UpdateEmpty`. `{ let x = 1; { add(1,1) } }` last_return is `2`; `{ add(1,1); let x = 1 }` stays `2`. Empty `{ }` writes nothing. `let`/`const` are void; `x = 1` as a statement is not (completion is `1`). Jeremy-script / ES — confirm if we ever disagree.
