...
- Managed array `push` calloc’s a ring entry in `xctx->p` and `slot_store`s. `pop`/`shift` transfer (no `release`, no `free_memory` of the entry). `array_push_pop` soak ~50 MiB/s. Same class as the splice extra-hold worry. **Next soak.**
