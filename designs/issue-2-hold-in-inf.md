...
**How close (scoreboard):** [Live status](#live-status-2026-09-16--how-close-is-2) below. If this pad’s older “Order / Parked” lists, the RSS lab README, the two-worlds pad, or the GitHub issue body disagree with that section, **the live status wins** until someone updates it.
...
## Live status (2026-09-16) — how close is #2?
...
Hard-loop soaks remeasured **2026-09-16** on branch `issue-2-stale-maps` (not yet on `develop`): assign / overlay / rebind / empty `{ }` / unbraced loop body / `array_push_pop` / **`function_return`** / `let`/`const` empty `f()` **flat**. `try_catch` still ~0.25 MiB/s in_use — OS backtrace on `xctx->p` every `throw`, not script RC (release-on-overwrite is a later sitting). Nested `{ }` that wrote `script_result` **clears** parent last instead of planting the occupant (`set_last_result` ignores void — that plant was the `try.as` SIGSEGV). Try returns current last; `keep_return` extra-holds a real `return` so a normal finally does not drop it. Table: [`src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md`](../src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md). Gate 2026-09-16: `./afwdev build --fulldev` + `afwdev test -j` + valgrind **4512 passed**, 71 skipped.
...
|------|---------------------|
| **Evidence** | **2026-09-16** (this branch). Lab table rewritten. `function_return` **flat**. `try_catch` ~0.25 MiB/s in_use is OS backtrace leftover, not the engine. Nested `{ }` last plant dropped (clear parent last). Server soaks not remeasured this sitting (`02` / `07` / `07b` still 2026-09-15). |
...
| **Lab** | [`src/afw/tests-extra/issue-2/`](../src/afw/tests-extra/issue-2/) — table in `01-rss-hard-loops/README.md` (2026-09-16) |
...
Tests: `language/script/return_temps.as`. `function_return` soak is **flat** on this branch; `array_push_pop` is **flat** (temp on current scope).
...
- FRV leftover wrapping **dropped** ([PR #326](https://github.com/afw-org/afw/pull/326)): no `function_return_value`; pin on caller. Do not reopen unique consume or a call-result leftover inf. Product tests (`return_temps.as`) stay; the soak lab is [`src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md`](../src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md). `function_return` is **flat** on this branch (2026-09-16). Map: [`compile-unit-and-frv-next.md`](compile-unit-and-frv-next.md).
