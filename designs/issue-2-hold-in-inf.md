...
Hard-loop soaks remeasured **2026-09-15** on `develop` `63831efa` (after [PR #326](https://github.com/afw-org/afw/pull/326) / [PR #327](https://github.com/afw-org/afw/pull/327)): assign / overlay / rebind / empty `{ }` / unbraced loop body / `array_push_pop` **flat**. `function_return` still grows **~1.5 MiB/s** (under the 8 MiB/s RSS / 2 MiB/s in_use fail line) — not flat; no leftover wrapper type. Table: [`src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md`](../src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md). Gate 2026-09-14: `./afwdev build --fulldev` + `afwdev test -j` + valgrind **4484 passed**, 71 skipped.
...
|------|---------------------|
| **Evidence** | **Done 2026-09-15.** Lab table rewritten. `function_return` still ~1.5 MiB/s (under bar). `02-pool-eval-soak` `afwfcgi` RSS flat 20 s; `07` ~39–41 MB/30 s; `07b` 40 requests / ~0.3 s (too short for a slope). |
...
| **Lab** | [`src/afw/tests-extra/issue-2/`](../src/afw/tests-extra/issue-2/) — table in `01-rss-hard-loops/README.md` (2026-09-15) |
...
**`xctx->p`:** one ST heap for that run (request, CLI). Base xctx `p` is `env->p` (MT process heap). Tracker = leftover list on the ancestor heap. Job pools (`multithread_create`) set `managed_p = self`; managed values allocate there. Compile units are their own ST heap (`heap_create`, 4k min). Live pool map: [`remaining-apr.md`](remaining-apr.md).
...
- FRV leftover wrapping **dropped** ([PR #326](https://github.com/afw-org/afw/pull/326)): no `function_return_value`; pin on caller. Do not reopen unique consume or a call-result leftover inf. Product tests (`return_temps.as`) stay; the soak lab is [`src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md`](../src/afw/tests-extra/issue-2/01-rss-hard-loops/README.md). `function_return` still ~1.5 MiB/s (2026-09-15), under the bar. Map: [`compile-unit-and-frv-next.md`](compile-unit-and-frv-next.md).
