...
**How close (scoreboard):** [Live status](#live-status-2026-09-19--how-close-is-2) below. If this pad’s older “Order / Parked” lists, the RSS lab README, the two-worlds pad, or the GitHub issue body disagree with that section, **the live status wins** until someone updates it.
...
## Live status (2026-09-19) — how close is #2?
...
| **Managed dest / compiled results** | [#342](https://github.com/afw-org/afw/issues/342). Evaluated results are **pinned** on dest `p` (not `clone_unmanaged`). Functions/closures as the eval result still alias the unit; the unit is a managed `compiled_value`. Managed alloc is dest `p->managed_p`, not a hardcoded `xctx->p`. |
...
**`xctx->p`:** one ST job heap for that run (request, CLI), created `*_as_managed_p`. Base xctx `p` is `env->p` (MT process heap). Tracker = leftover list on the ancestor heap. Unmarked heap/MT create **inherits** `managed_p`; `*_as_managed_p` is `managed_p = self`. Managed values allocate in dest `p->managed_p`. Compile units are their own ST heap (`heap_create`, inherit, 4k min). Evaluation `{ }` is `scope_create` (ST heap, 4k, inherit, throw delay). Live pool map: [`remaining-apr.md`](remaining-apr.md).
