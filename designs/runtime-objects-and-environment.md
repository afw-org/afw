...
Discover/filter: `retrieve_objects("afw", "_AdaptiveRuntimeValueAccessor_")` (default `maxObjects` **0** = unlimited) and query on `copiesUnderLock` / `returnsLiveReference` / `key` / text fields.
