...
| `apr_thread_mutex_*` / `apr_thread_rwlock_*` / `apr_thread_create` via `afw_thread_mutex_*` aliases and `get_apr_pool` | **`afw_thread_mutex_create(flags, p, xctx)`** (and lock/unlock/destroy) on **`afw_pool`**. Nested is recursive; unnested is not. Same for **`afw_thread_rwlock_*`**. Native thread is **`afw_os_thread_*`** in `os/`. Do not pass an APR pool. |
