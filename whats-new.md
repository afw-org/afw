...
| Curl default response `apr_brigade_write` / `length` / `flatten` | **`afw_memory_create_writer`**, write addr+len via `writer->callback`, **`retrieve_and_release`**. Script `writeFunction` still skips the buffer. |
| `apr_getopt_long` in `afw` / command skeleton | **`afw_getopt_long`** with an `afw_getopt_option_t` table (help from the same table). No APR pool. `afwfcgi` still uses its strcmp loop. A **make-command** package’s `find_package(afw)` picks the **newest** installed cmake config (versioned `lib/cmake/afw-*` dirs accumulate). |
| LDAP `apr_ldap_url_parse` / `apr_ldap_init` / `apr_ldap_set_option` | OpenLDAP **`ldap_url_parse`**, **`ldap_initialize`**, **`ldap_set_option`**. Search/bind/CRUD were already `ldap_*`. |
...
| `error->backtrace` as a pool utf8 / `afw_os_backtrace` returning a buffer | **`const afw_value_hexBinary_t *`** (NULL if none). Captured when this xctx has **`response:error:backtrace`** on, except a memory error and **trace-all-only** codes (`throw`, `syntax`), which need **`response:error:backtrace:all`**. **`afw_error_release_backtrace`**. Error object property is still a **`ks`** string of those octets. A request can set it with **`_flags_`** / **`flag_set`**. The `afw` command leaves **`response:error`** off. Turn it on with **`--conf`** (`defaultFlags`) or **`flag_set`**, including from interactive mode and from a test. That still does not turn `:all` on. |
...
If you are bringing a **sibling AFW package** or other C that linked old libafw up to this line, work the [C programmers](#c-programmers) table **in order**, then the checklists below. You do **not** need APR to build or link AFW.
...
3. **Pools:** `afw_pool_heap_create` / `afw_pool_create` (heap like the parent) / `afw_pool_tracker_create` / `afw_pool_scope_create` (`{ }`) / `afw_pool_multithread_create`. `create_managed` takes **`p`** and allocates in **`p->managed_p`**. `destroy` is storage-only; **`run_cleanups`** first. [Value lifetime](#value-lifetime--memory-management-issue-2--alphabeta)
...
| [**`afw` CLI**](#interactive-afw-line-editing-and-history) | Optional interactive line editing and history ([#30](https://github.com/afw-org/afw/issues/30)); **`response:error` stays off** unless **`--conf`** or **`flag_set`** turns it on; **`--allow` / `-a`** for result content type (YAML block strings, issue **[#14](https://github.com/afw-org/afw/issues/14)**) — see also [YAML / `--allow`](#afw---allow-and-yaml-value-output-issue-14) |
...
The rule is unchanged. A run of ordinary ASCII is copied in one write, in `ks` and in JSON strings. An error object's `backtraceEvaluation` property holds that `ks` string; the temporary used to build it is released. On the [#363](https://github.com/afw-org/afw/issues/363) profile, `for-of.as` pool bytes in use fell from about 106MB to 55MB, and the suite ceiling from about 129MB to 55MB. Line and column counts for that dump walk ASCII bytes in place, and the source-listing prefix is written without `snprintf`. `for-of.as` fell from about 11.2 billion instructions to about 5.6 billion while **`response:error`** was on. The `afw` command now leaves that flag off, so a test script no longer keeps an evaluation listing on each caught error. With the flag off, `for-of.as` is about 0.04s and about 4MB of pool, and the suite ceiling is about 23MB xctx and 25MB chunk. [#363](https://github.com/afw-org/afw/issues/363) stays open.
...
Compile errors / runtime NFC throws on error paths look like: `AFW_UTF8_FMT` in `afw_utf8_printf` still followed by `FMT_ARG` (int + pointer — **va_list desync** / SIGSEGV); `%s` of OS/LDAP/backtrace bytes throwing **`%s is not valid UTF-8`** on **default** printf (error `*_fz` / `error_to_utf8` already encode); missing `apr_strings.h`; unknown `AFW_UTF8_FMT_OPTIONAL_*`.
...
This section is for **someone supporting another repository that uses this one** (extension DSO, command, sibling AFW package). After you rebuild that tree against a libafw that has this change, cmake will fail until object **property name** arguments are values. Use this as the source-fix checklist.

Same [C rebuild rule](#c-programmers) as the rest of this note: rebuild and reinstall the other package against **this** AFW install. Then regenerate (`./afwdev build --cdev` in **that** tree) so its `*_v_*` strings exist.

### Script / JSON in that repository

Adaptive Script and JSON/YAML/UBJSON are **string names only**. Integer/boolean object keys **throw** (no convert of `1` to `"1"`). Array and string indexes stay integer.
...
Fix tests and samples in the other repo that used `obj[1]` or `{ [1]: x }` as an object key. JSON `{"1": true}` is still the string `"1"` — lookup with `o["1"]`, not `o[1]`.

### What C actually changed

`afw_object` / `afw_object_setter` (and helpers that are really a property name) take **`const afw_value_t *`**. Implementations compare with **`afw_value_equal`**.

Includes: `afw_object_get_property`, `set_property` / `set_property_as_*`, `has_property`, `remove_property`, `create_embedded`, `get_next_property` (the name out-parameter is now `const afw_value_t **`), `afw_object_meta_set_property*`, `afw_adapter_impl_throw_property_invalid` / `throw_property_required`.

**Not** a property name (leave utf8): object type ids, adapter/object ids, environment **registry type** ids, RQL/query-criteria filter and sort keys, `afw_object_get_property_extended` (still a dotted **utf8** path).

### How to fix C in the other repository

Compile errors look like: passing `const afw_utf8_t *` to get/set; `AFW_UTF8_FMT_ARG(property_name)` when the name is now a value (`no member named 'len'`); `afw_writer_write_utf8` of a value; path entry `.s` / `.len`.

| In the other repo you have | Change to |
|----------------------------|-----------|
| `afw_object_get_property(obj, afw_s_foo, xctx)` | `…(obj, afw_v_foo, xctx)` |
| `afw_object_set_property(obj, afw_s_foo, v, xctx)` | `…(obj, afw_v_foo, v, xctx)` |
...
afw_adapter_impl_throw_property_
afw_s_
```

Do **not** blindly change every `afw_s_` — only arguments that are **object property names**. `afw_s__AdaptiveObjectType_` as an object **type** id stays utf8.

### Path dotted names (`afw_object_path_property_name_entry_t`)

The entry is no longer an embedded `afw_utf8_t`. It is **`afw_value_string_t property_name`**.

| Old | New |
|-----|-----|
| `&entry->property_name` as utf8 | `&entry->property_name.internal` |
| `entry->property_name.s` / `.len` | `entry->property_name.internal.s` / `.len` |
| Pass the name to get/set | `&entry->property_name.pub` |

### Object implementations in the other repo

If you implement `afw_object` / setter: store the name as `const afw_value_t *`, compare with **`afw_value_equal`**. No extra “is it a string?” in the impl. Script/JSON string-only is the compiler/content-type helper `afw_object_require_string_property_name`.

### Macros (in `afw_value.h`)

```c
static const afw_value_string_t impl_v_sysname =
    AFW_VALUE_STRING_LITERAL("sysname");
/* object API: &impl_v_sysname.pub
   utf8:       &impl_v_sysname.internal */

const afw_value_string_t n = AFW_VALUE_STRING_UNMANAGED(utf8);
afw_object_get_property(obj, &n.pub, xctx);  /* get/has only */
```
...
- **`create_unmanaged_new_p`** — new child of `p->managed_p`
- **`create_unmanaged_cede_p`** — you pass the pool
- **`create_managed(p, xctx)`** — **frame** (slots + RC in **`p->managed_p`**)
...
| Type | Use |
|------|-----|
| **`afw_vector`** | Growable contiguous elements. Header stays put; `entries` may move on grow. Declare a typed overlay with **`AFW_VECTOR_STRUCT`**. The in-memory **`afw_array`** store is a vector of value pointers. Remaining in-tree **`apr_array`** uses (env registry/method tables, LDAP mods, adapter index cursor lists) now use this type. |
...
| [Value / memory](#value-lifetime--memory-management-issue-2--alphabeta) ([#2](https://github.com/afw-org/afw/issues/2)) | Pool/value lifetime and faces; **no APR in libafw** |
...
| APR | **Gone from libafw.** No `apr-1` to link; no `get_apr_pool`. Heap is AFW chunks. |
...
- **`vfsMap` multi-entry:** longest matching `objectId_prefix` wins. Host roots
  ...
  A symlink whose **realpath** leaves the map root is rejected (same as
  `rootFilePaths`). A sibling under the map is still followed.
...
**`response:error` stays off** for every `afw` run, including interactive and test scripts. The terminal still prints the error type, the message, and the short contextual lines (source name, offset, cursor). The flag is what adds the evaluation listing and the code backtrace to the error object. Turn it on with **`--conf`** (`defaultFlags`) or **`flag_set`**. That does not turn **`response:error:backtrace:all`** on. `afwfcgi` was already off unless conf or the request sets it.
